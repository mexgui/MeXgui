...
JobProcessorFactory *const x264Encoder::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "x264Encoder");
	...
	x264Encoder::x264Encoder(const QString &encoderPath) : CommandlineVideoEncoder()
	{
		executable = encoderPath;
	#if defined(x86)
		if (OSInfo::isWow64() && MainForm::Instance->getSettings()->getUse64bitX264())
		{
			QString x264Path = Path::Combine(Path::GetDirectoryName(encoderPath), "avs4x264mod.exe");
	...
	void x264Encoder::ProcessLine(const QString &line, StreamType stream, ImageType oType)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
	...
	QString x264Encoder::genCommandline(const QString &input, const QString &output, Nullable<Dar> d, int hres, int vres, int fps_n, int fps_d, x264Settings *_xs, Zone zones[], LogItem *log)
	{
		int qp;
		bool display = false;
		...
		MeXgui::packages::video::x264::x264SettingsHandler *oSettingsHandler = new packages::video::x264::x264SettingsHandler(xs, log);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	const QString &x264Encoder::getCommandline() const
	{
	...
	void x264Encoder::doExitConfig()
	{
		if (proc->ExitCode != 0 && !su->getWasAborted() && OSInfo::isWow64() && MainForm::Instance->getSettings()->getUse64bitX264())
			log->LogEvent("The 64 bit mode of x264 is enabled. Depending on the error it may help to disable it in the MeXgui settings.", Warning);
