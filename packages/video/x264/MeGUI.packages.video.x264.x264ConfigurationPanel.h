...
namespace MeXgui
{
	namespace packages
	{
		namespace video
		{
			namespace x264
			{
				class x264ConfigurationPanel : public MeXgui::core::details::video::VideoConfigurationPanel, Editable<x264Settings*>
				{
				public:
				...
				private:
					...
					QVector<x264Device*> x264DeviceList;
...
//C# TO C++ CONVERTER WARNING: Unlike C#, there is no automatic call to this finalizer method in native C++:
				...
				private:
					...
					void logfileOpenButton_Click();
					QString SelectHelpText(const QString &node);
					void SetToolTips();
					void x264DialogTriStateAdjustment();

					void cqmComboBox1_SelectionChanged(object *sender, const QString &val);

					void VisitLink();

					void cbAQMode_SelectedIndexChanged();

					void useQPFile_CheckedChanged();

					void qpfileOpenButton_Click();

					void tbx264Presets_Scroll();

					void advancedSettings_CheckedChanged();

					void dSettings_Click();

					void btPresetSettings_Click();

					void x264Tunes_SelectedIndexChanged();

					void avcProfile_SelectedIndexChanged();

					void cbTarget_SelectionChangeCommitted();

					void pictureBox1_Click();

					void targetDevice_SelectedIndexChanged();

					x264Settings::x264PsyTuningModes getPsyTuning();

					AVCLevels::Levels getAVCLevel();

					void avcLevel_SelectedIndexChanged();

					void x264VBVBufferSize_ValueChanged();

					void x264VBVMaxRate_ValueChanged();

					void ch10BitsEncoder_CheckedChanged();
					...
					MeXgui::core::gui::FileSCBox *cqmComboBox1;
