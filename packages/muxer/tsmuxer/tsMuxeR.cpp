...
JobProcessorFactory *const tsMuxeR::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "TSMuxer");
	...
	tsMuxeR::tsMuxeR(const QString &executablePath)
	{
	...
	const QString &tsMuxeR::getCommandline() const
	{
		return " \"" + metaFile + "\"" + " \"" + job->Output + "\"";
	}

	void tsMuxeR::ProcessLine(const QString &line, StreamType stream, ImageType oType)
	{
	...
	Nullable<decimal> tsMuxeR::getPercentage(const QString &line)
	{
		try
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
			QString *strPercentage = line.Split('%')[0]->Split('.');
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to this .NET Convert class method:
	...
	bool tsMuxeR::isEmptyLine(const QString &line)
	{
...
//ORIGINAL LINE: case "AVCHD":
				...
				{
					QString chapterTimeLine = VideoUtil::getChapterTimeLine(settings->getChapterFile());
