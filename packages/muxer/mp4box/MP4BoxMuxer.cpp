...
JobProcessorFactory *const MP4BoxMuxer::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "MP4BoxMuxer");
	...
	MP4BoxMuxer::MP4BoxMuxer(const QString &executablePath)
	{
	...
	MeXgui::MP4BoxMuxer::LineType MP4BoxMuxer::getLineType(const QString &line)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
	...
	void MP4BoxMuxer::ProcessLine(const QString &line, StreamType stream, ImageType oType)
	{
	...
	Nullable<decimal> MP4BoxMuxer::getPercentage(const QString &line)
	{
		try
		{
			int start = line.find("(") + 1;
			int end = line.find("/");
			QString perc = line.substr(start, end - start);
	...
	bool MP4BoxMuxer::isEmptyLine(const QString &line)
	{
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
	...
	const QString &MP4BoxMuxer::getCommandline() const
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
				QString fpsString = settings->getFramerate().Value->ToString(ci);
		...
		for (QVector<MuxStream*>::const_iterator o = settings->getAudioStreams().begin(); o != settings->getAudioStreams().end(); ++o)
		{
			...
			{
				for (QMap<QString, QString>::const_iterator strLanguage = LanguageSelectionContainer::getLanguagesTerminology().begin(); strLanguage != LanguageSelectionContainer::getLanguagesTerminology().end(); ++strLanguage)
				{
