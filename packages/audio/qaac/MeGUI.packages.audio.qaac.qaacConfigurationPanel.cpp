	...
	namespace packages
	{
		namespace audio
		{
			namespace qaac
			{
				...
				qaacConfigurationPanel::qaacConfigurationPanel() : MeXgui::core::details::audio::AudioConfigurationPanel()
				{
				...
				const MeXgui::AudioCodecSettings &qaacConfigurationPanel::getCodecSettings() const
				{
				...
				const MeXgui::QaacSettings &qaacConfigurationPanel::getSettings() const
				{
				...
				void qaacConfigurationPanel::trackBar1_ValueChanged()
				{
					switch (static_cast<QaacMode>((dynamic_cast<EnumProxy*>(cbMode->SelectedItem))->getRealValue()))
					{
						case TVBR:
							...
							encoderGroupBox->Text = QString::Format("QAAC Options - (Q={0})", trackBar->Value);
							break;
						case CVBR:
							trackBar->Minimum = 0;
							trackBar->Maximum = 320;
							trackBar->TickFrequency = 20;
							encoderGroupBox->Text = QString::Format("QAAC Options - Constrained Variable Bitrate @ {0} kbit/s", trackBar->Value);
							break;
						case ABR:
							trackBar->Minimum = 0;
							trackBar->Maximum = 320;
							trackBar->TickFrequency = 20;
							encoderGroupBox->Text = QString::Format("QAAC Options - Average Bitrate @ {0} kbit/s", trackBar->Value);
							break;
						case CBR:
							trackBar->Minimum = 0;
							trackBar->Maximum = 320;
							trackBar->TickFrequency = 20;
							encoderGroupBox->Text = QString::Format("QAAC Options - Constant Bitrate  @ {0} kbit/s", trackBar->Value);
							break;
					}
					if (cbProfile->SelectedIndex == 2)
						encoderGroupBox->Text = QString::Format("QAAC Options");
				}

				void qaacConfigurationPanel::cbMode_SelectedIndexChanged()
				{
					trackBar1_ValueChanged(sender, e);
				}

				void qaacConfigurationPanel::cbProfile_SelectedIndexChanged()
				{
