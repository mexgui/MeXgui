	...
	namespace packages
	{
		namespace audio
		{
			namespace naac
			{
				...
				neroConfigurationPanel::neroConfigurationPanel() : MeXgui::core::details::audio::AudioConfigurationPanel()
				{
					//
				...
				const MeXgui::AudioCodecSettings &neroConfigurationPanel::getCodecSettings() const
				{
				...
				void neroConfigurationPanel::rbtnABR_CheckedChanged()
				{
					vBitrate->Enabled = !(vQuality->Enabled = rbtnVBR->Checked);
					vBitrate_ValueChanged(sender, e);
				}

				void neroConfigurationPanel::vBitrate_ValueChanged()
				{
					if (rbtnVBR->Checked)
					{
						...
						rbtnVBR->Text = QString::Format("Variable Bitrate (Q={0}) ", q);
					}
					else if (rbtnABR->Checked)
					{
						rbtnABR->Text = QString::Format("Adaptive Bitrate @ {0} kbit/s", vBitrate->Value);
						rbtnCBR->Text = "Constant Bitrate";
					...
					{
						rbtnABR->Text = "Adaptive Bitrate";
						rbtnCBR->Text = QString::Format("Constant Bitrate @ {0} kbit/s", vBitrate->Value);
						rbtnVBR->Text = "Variable Bitrate";
					}
				}

				const MeXgui::NeroAACSettings &neroConfigurationPanel::getSettings() const
				{
