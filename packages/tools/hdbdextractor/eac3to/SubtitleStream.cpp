...
namespace eac3to
{
	...
	const QString &SubtitleStream::getLanguage() const
	{
		return privateLanguage;
	}

	void SubtitleStream::setLanguage(const QString &value)
	{
...
//ORIGINAL LINE: case "Vob":
	...
	SubtitleStream::SubtitleStream(const QString &s) : Stream(s)
	{
		if (s.empty())
	...
	Stream *SubtitleStream::Parse(const QString &s)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
	...
	QString SubtitleStream::ToString()
	{
