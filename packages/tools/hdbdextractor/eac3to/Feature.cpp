...
namespace eac3to
{
	...
	const QString &Feature::getName() const
	{
		return privateName;
	}

	void Feature::setName(const QString &value)
	{
		privateName = value;
	}

	const QString &Feature::getDescription() const
	{
		return privateDescription;
	}

	void Feature::setDescription(const QString &value)
	{
	...
	const QVector<File*> &Feature::getFiles() const
	{
		return privateFiles;
	}

	void Feature::setFiles(const QVector<File*> &value)
	{
		privateFiles = value;
	}

	const QVector<Stream*> &Feature::getStreams() const
	{
		return privateStreams;
	}

	void Feature::setStreams(const QVector<Stream*> &value)
	{
	...
	Feature::Feature()
	{
		...
		setFiles(QVector<File*>());
		setStreams(QVector<Stream*>());
	}

	Feature *Feature::Parse(const QString &s)
	{
		/*
		// Blu-Ray
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
			for (unknown::const_iterator file = s.substr(s.find(",") + 1, s.rfind(',') - s.find(",") - 1)->Split((QString("+"))->ToCharArray()).begin(); file != s.substr(s.find(",") + 1, s.rfind(',') - s.find(",") - 1)->Split((QString("+"))->ToCharArray()).end(); ++file)
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
			for (unknown::const_iterator file = s.substr(s.find(")") + 1, s.find(',') - s.find(")") - 1)->Split((QString("+"))->ToCharArray()).begin(); file != s.substr(s.find(")") + 1, s.find(',') - s.find(")") - 1)->Split((QString("+"))->ToCharArray()).end(); ++file)
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
				feature->getFiles().push_back(new File((*file).Trim(), feature->getFiles().size() + 1));
	...
	QString Feature::ToString()
	{
		...
		for (QVector<File*>::const_iterator file = getFiles().begin(); file != getFiles().end(); ++file)
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		...
		if (getStreams().empty())
			...
			for (QVector<Stream*>::const_iterator stream = getStreams().begin(); stream != getStreams().end(); ++stream)
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
