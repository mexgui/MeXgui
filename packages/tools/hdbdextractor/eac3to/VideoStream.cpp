...
namespace eac3to
{
	...
	const QString &VideoStream::getResolution() const
	{
		return privateResolution;
	}

	void VideoStream::setResolution(const QString &value)
	{
	...
	VideoStream::VideoStream(const QString &s) : Stream(s)
	{
		if (s.empty())
	...
	Stream *VideoStream::Parse(const QString &s)
	{
