...
JobProcessorFactory *const D2VIndexer::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "D2VIndexer");
	...
	D2VIndexer::D2VIndexer(const QString &executableName)
	{
		executable = executableName;
	}

	void D2VIndexer::ProcessLine(const QString &line, StreamType stream, ImageType oType)
	{
	...
	const QString &D2VIndexer::getCommandline() const
	{
		StringBuilder *sb = new StringBuilder();
		int idx = 0;
		QString projName = Path::Combine(Path::GetDirectoryName(job->Output), Path::GetFileNameWithoutExtension(job->Output));
		...
		else if (job->getDemuxMode() == 1)
		{
			...
			for (QVector<AudioTrackInfo*>::const_iterator ati = job->getAudioTracks().begin(); ati != job->getAudioTracks().end(); ++ati)
			{
				if (idx > 0)
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	bool D2VIndexer::applyForceFilm(const QString &fileName)
	{
		try
		{
			StringBuilder *sb = new StringBuilder();
	...
	void D2VIndexer::doStatusCycleOverrides()
	{
		try
		{
			if (su->getTimeElapsed() < TwoSeconds) // give it some time to start up, otherwise MainWindowHandle remains null
				return;
			QString text = proc->MainWindowTitle;
