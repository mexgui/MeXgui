...
namespace MeXgui
{
	...
	void FileIndexerWindow::setConfig(const QString &input, const QString &projectName, int demuxType, bool showCloseOnQueue, bool closeOnQueue, bool loadOnComplete, bool updateMode)
	{
		openVideo(input);
	...
	FileIndexerWindow::FileIndexerWindow(MainForm *mainForm, const QString &fileName)
	{
		InitializeInstanceFields();
		CheckDGIIndexer();
		openVideo(fileName);
	...
	FileIndexerWindow::FileIndexerWindow(MainForm *mainForm, const QString &fileName, bool autoReturn)
	{
		InitializeInstanceFields();
		CheckDGIIndexer();
		openVideo(fileName);
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
	...
	void FileIndexerWindow::pickOutputButton_Click()
	{
		...
		if (saveProjectDialog->show() == System::Windows::Forms::DialogResult::OK)
		{
	...
	void FileIndexerWindow::openVideo(const QString &fileName)
	{
	...
	void FileIndexerWindow::generateAudioList()
	{
		...
		for (QVector<AudioTrackInfo*>::const_iterator atrack = audioTracks.begin(); atrack != audioTracks.end(); ++atrack)
	...
	void FileIndexerWindow::setOutputFileName()
	{
		...
		{
			QString projectPath = "";
			QString fileNameNoPath = Path::GetFileName(this->input->getFilename());
	...
	void FileIndexerWindow::queueButton_Click()
	{
		if (dialogMode)
			return;
		...
		if (cbPGC->SelectedIndex > 0 && Path::GetExtension(input->getFilename().ToUpper(System::Globalization::CultureInfo::InvariantCulture)) == ".VOB")
		{
			...
			if (File::Exists(videoIFO))
			{
				...
				for (int i = 1; i < 10; i++)
				{
					QString file = Path::Combine(Path::GetDirectoryName(output->Text), "VTS_01_" + i + ".VOB");
					if (File::Exists(file))
					{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
					if (txtContainerInformation->Text->Trim()->ToUpper(System::Globalization::CultureInfo::InvariantCulture)->Equals("MATROSKA") && job->getDemuxMode() > 0 && job->getAudioTracks().size() > 0)
					{
						...
						job->setAudioTracks(QVector<AudioTrackInfo*>());
	...
	D2VIndexJob *FileIndexerWindow::generateD2VIndexJob(const QString &videoInput)
	{
		int demuxType = 0;
		if (demuxTracks->Checked)
			demuxType = 1;
		...
		QVector<AudioTrackInfo*> audioTracks = QVector<AudioTrackInfo*>();
	...
	DGIIndexJob *FileIndexerWindow::generateDGNVIndexJob(const QString &videoInput)
	{
		int demuxType = 0;
		if (demuxTracks->Checked)
			demuxType = 1;
	...
	DGAIndexJob *FileIndexerWindow::generateDGAIndexJob(const QString &videoInput)
	{
		int demuxType = 0;
		if (demuxTracks->Checked)
			demuxType = 1;
	...
	FFMSIndexJob *FileIndexerWindow::generateFFMSIndexJob(const QString &videoInput)
	{
		int demuxType = 0;
		if (demuxTracks->Checked)
			demuxType = 1;
	...
	void FileIndexerWindow::rbtracks_CheckedChanged()
	{
	...
	void FileIndexerWindow::btnFFMS_Click()
	{
		changeIndexer(FFMS);
	}

	void FileIndexerWindow::btnDGI_Click()
	{
		changeIndexer(DGI);
	}

	void FileIndexerWindow::btnDGA_Click()
	{
		changeIndexer(DGA);
	}

	void FileIndexerWindow::btnD2V_Click()
	{
		changeIndexer(D2V);
	}
	...
	void FileIndexerWindow::InitializeComponent()
	{
		...
		this->pickOutputButton = new QPushButton;
		...
		this->input->setTitle("");
		this->input->FileSelected += new MeXgui::FileBarEventHandler(this, &FileIndexerWindow::input_FileSelected);
		// 
		// inputLabel
		// 
