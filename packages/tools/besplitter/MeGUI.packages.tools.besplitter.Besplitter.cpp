	...
	namespace packages
	{
		namespace tools
		{
			namespace besplitter
			{
				...
				void Besplitter::goButton_Click()
				{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
					...
					QString prefix = getAcceptableFilename(output->getFilename(), c->getAllCuts().size()*2);
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
					QString *tempfiles = generateNumberedFilenames(prefix, Path::GetExtension(output->getFilename()), c->getAllCuts().size() * 2);
				...
				QString *Besplitter::generateNumberedFilenames(const QString &prefix, const QString &ext, int num)
				{
					QString ans[num];
					for (int i = 1; i <= num; i++)
					{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
				...
				QString Besplitter::getAcceptableFilename(const QString &p, int p_2)
				{
					QString ext = Path::GetExtension(p);
					QString name = Path::Combine(Path::GetDirectoryName(p), Path::GetFileNameWithoutExtension(p));
					int suffix = 0;
					while (true)
					{
						QString test = name + "_" + suffix + "_";
						bool failed = false;
						for (QString::const_iterator s = generateNumberedFilenames(test, ext, p_2)->begin(); s != generateNumberedFilenames(test, ext, p_2)->end(); ++s)
							if (File::Exists(*s))
							{
								failed = true;
								break;
				...
				{
					for (MeXgui::OutputFileType::const_iterator type = filters->begin(); type != filters->end(); ++type)
					{
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
				...
				void Besplitter::InitializeComponent()
				{
					...
					this->input->setTitle("");
					this->input->FileSelected += new MeXgui::FileBarEventHandler(this, &Besplitter::input_FileSelected);
					// 
					// cuts
					// 
