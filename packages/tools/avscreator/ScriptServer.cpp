...
const QVector ScriptServer::ListOfResizeFilterType = EnumProxy::CreateArray(ResizeFilterType::typeid);
const QVector ScriptServer::ListOfDenoiseFilterType = EnumProxy::CreateArray(DenoiseFilterType::typeid);
const QVector ScriptServer::ListOfSourceTypes = EnumProxy::CreateArray(UserSourceType::typeid);
const QVector ScriptServer::ListOfFieldOrders = EnumProxy::CreateArray(UserFieldOrder::typeid);
const QVector ScriptServer::ListOfNvDeIntType = EnumProxy::CreateArray(NvDeinterlacerType::typeid);

	QString ScriptServer::CreateScriptFromTemplate(const QString &template_Renamed, const QString &inputLine, const QString &cropLine, const QString &resizeLine, const QString &denoiseLines, const QString &deinterlaceLines)
	{
		QString newScript = template_Renamed;
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Replace' method:
	...
	QString ScriptServer::GetInputLine(const QString &input, const QString &indexFile, bool interlaced, PossibleSources sourceType, bool colormatrix, bool mpeg2deblock, bool flipVertical, double fps, bool dss2)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
				...
				{
					if (dss2 && File::Exists(Path::Combine(MeXguiSettings::getHaaliMSPath(), "avss.dll")))
					{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
						inputLine = "LoadPlugin(\"" + MeXguiSettings::getHaaliMSPath() + "\\avss.dll" + "\")\r\ndss2(\"" + input + "\"" + ((fps > 0) ? ", fps=" + fps.ToString("F3", new CultureInfo("en-us")) : "") + ")" + VideoUtil::getAssumeFPS(fps, input);
					}
					else
					{
						inputLine = "";
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	QString ScriptServer::GetCropLine(bool crop, CropValues *cropValues)
	{
		QString cropLine = "#crop";
		if (crop & cropValues->isCropped())
		{
			cropLine = QString::Format("crop({0}, {1}, {2}, {3})", cropValues->left, cropValues->top, -cropValues->right, -cropValues->bottom);
		}
		return cropLine;
	}

	QString ScriptServer::GetResizeLine(bool resize, int hres, int vres, int hresWithBorder, int vresWithBorder, ResizeFilterType type, bool crop, CropValues *cropValues, int originalHRes, int originalVRes)
	{
		...
		if (p->getTag() != 0)
			if (hresWithBorder > hres || vresWithBorder > vres)
				return QString::Format(p->getTag() + "." + getAddBorders(hres, vres, hresWithBorder, vresWithBorder) + " # {2}", hres, vres, p);
			else
				return QString::Format(p->getTag() + " # {2}", hres, vres, p);
		else
			return "#resize - " + p;
	}

	QString ScriptServer::getAddBorders(int hres, int vres, int hresWithBorder, int vresWithBorder)
	{
		...
		return QString::Format("AddBorders({0},{1},{2},{3})", borderValues->left, borderValues->top, borderValues->right, borderValues->bottom);
	}

	QString ScriptServer::GetDenoiseLines(bool denoise, DenoiseFilterType type)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
				...
				denoiseLines = QString::Format(strPath + p->getTag() + " # " + p);
			}
			else
	...
	QString ScriptServer::GetNvDeInterlacerLine(bool deint, NvDeinterlacerType type)
	{
		QString nvDeInterlacerLine = "";
		if (deint)
		{
			EnumProxy *p = EnumProxy::Create(type);
			if (p->getTag() != 0)
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	QVector<DeinterlaceFilter*> ScriptServer::GetDeinterlacers(SourceInfo *info)
	{
		QVector<DeinterlaceFilter*> filters = QVector<DeinterlaceFilter*>();
	...
	void ScriptServer::AddYadif(FieldOrder order, QVector<DeinterlaceFilter*> &filters, bool bobber)
	{
		filters.push_back(new DeinterlaceFilter(bobber ? "Yadif (with Bob)" : "Yadif", QString::Format("Load_Stdcall_Plugin(\"{0}\"){1}Yadif({2}order={3})", MainForm::Instance->getSettings()->getYadifPath(), "\r\n", bobber ? "mode=1, " : "", Order(order))));
	}

	void ScriptServer::AddLeakDeint(FieldOrder order, QVector<DeinterlaceFilter*> &filters)
	{
		filters.push_back(new DeinterlaceFilter("LeakKernelDeint", QString::Format("LoadPlugin(\"{0}\"){1}LeakKernelDeint(order={2},sharp=true)", Path::Combine(MainForm::Instance->getSettings()->getAvisynthPluginsPath(), "LeakKernelDeint.dll"), "\r\n", Order(order))));
	}

	void ScriptServer::AddTDeint(FieldOrder order, QVector<DeinterlaceFilter*> &filters, bool processAll, bool eedi2, bool bob)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void ScriptServer::AddFieldDeint(FieldOrder order, QVector<DeinterlaceFilter*> &filters, bool processAll, bool blend)
	{
		QString name = "FieldDeinterlace";
		if (!blend)
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void ScriptServer::AddTMC(FieldOrder order, QVector<DeinterlaceFilter*> &filters)
	{
		filters.push_back(new DeinterlaceFilter("TomsMoComp", QString::Format("LoadPlugin(\"{0}\"){1}TomsMoComp({2},5,1)", Path::Combine(MainForm::Instance->getSettings()->getAvisynthPluginsPath(), "TomsMoComp.dll"), "\r\n", Order(order))));
	}

	void ScriptServer::Portionize(QVector<DeinterlaceFilter*> &filters, const QString &trimLine)
	{
		for (int i = 0; i < filters.size(); i++)
		{
			QString script = filters[i]->getScript();
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void ScriptServer::AddTIVTC(const QString &d2vFile_Renamed, bool anime, bool hybrid, bool mostlyFilm, bool advancedDeinterlacing, FieldOrder fieldOrder, QVector<DeinterlaceFilter*> &filters)
	{
		StringBuilder *script = new StringBuilder();
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void ScriptServer::AddIVTC(FieldOrder order, bool hybrid, bool mostlyFilm, QVector<DeinterlaceFilter*> &filters)
	{
		StringBuilder *script = new StringBuilder();
		script->Append("LoadPlugin(\"" + Path::Combine(MainForm::Instance->getSettings()->getAvisynthPluginsPath(), "Decomb.dll") + "\")\r\n");
		if (order == TFF)
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void ScriptServer::AddTDecimate(int decimateM, QVector<DeinterlaceFilter*> &filters)
	{
		filters.push_back(new DeinterlaceFilter("Tritical Decimate", QString::Format("LoadPlugin(\"{0}\"){1}TDecimate(cycleR={2})", Path::Combine(MainForm::Instance->getSettings()->getAvisynthPluginsPath(), "TIVTC.dll"), "\r\n", decimateM)));
	}

const QString ScriptServer::DetectionScript = "{0} #original script" + "\r\n" + "{1} #trimming" + "\r\n" + "{5} #LoadPlugin" + "\r\n" + "global unused_ = blankclip(pixel_type=\"yv12\", length=10).TFM()" + "\r\n" + "file=\"{2}\\" + "\r\n" + "global sep=\"-\\" + "\r\n" + "function IsMoving() {{" + "\r\n" + "  global b = (diff < 1.0) ? false : true}}" + "\r\n" + "ConvertToYV12()" + "\r\n" + "c = last" + "\r\n" + "global clip = last" + "\r\n" + "c = WriteFile(c, file, \"a\", \"sep\", \"b\")" + "\r\n" + "c = FrameEvaluate(c, \"global a = IsCombedTIVTC(clip, cthresh=9)\")" + "\r\n" + "c = FrameEvaluate(c, \"IsMoving\")" + "\r\n" + "c = FrameEvaluate(c,\"global diff = 0.50*YDifferenceFromPrevious(clip) + 0.25*UDifferenceFromPrevious(clip) + 0.25*VDifferenceFromPrevious(clip)\")" + "\r\n" + "crop(c,0,0,16,16)" + "\r\n" + "SelectRangeEvery({3},{4},0)";
const QString ScriptServer::FieldOrderScript = "{0} # original script" + "\r\n" + "{1} #trimming" + "\r\n" + "file=\"{2}\\" + "\r\n" + "global sep=\"-\\" + "\r\n" + "ConvertToYV12()" + "\r\n" + "d = last" + "\r\n" + "global abff = d.assumebff().separatefields()" + "\r\n" + "global atff = d.assumetff().separatefields()" + "\r\n" + "c = d.loop(2)" + "\r\n" + "c = WriteFile(c, file, \"diffa\", \"sep\", \"diffb\")" + "\r\n" + "c = FrameEvaluate(c,\"global diffa = 0.50*YDifferenceFromPrevious(abff) + 0.25*UDifferenceFromPrevious(abff) + 0.25*VDifferenceFromPrevious(abff)\")" + "\r\n" + "c = FrameEvaluate(c,\"global diffb = 0.50*YDifferenceFromPrevious(atff) + 0.25*UDifferenceFromPrevious(atff) + 0.25*VDifferenceFromPrevious(atff)\")" + "\r\n" + "crop(c,0,0,16,16)" + "\r\n" + "SelectRangeEvery({3},{4},0)" + "\r\n" + "";

	QString ScriptServer::getScript(int scriptType, const QString &originalScript, const QString &trimLine, const QString &logFileName, int selectEvery, int selectLength)
	{
		if (scriptType == 0) // detection
			return QString::Format(DetectionScript, originalScript, trimLine, logFileName, selectEvery, selectLength, "LoadPlugin(\"" + Path::Combine(MainForm::Instance->getSettings()->getAvisynthPluginsPath(), "TIVTC.dll") + "\")");
		else if (scriptType == 1) // field order
			return QString::Format(FieldOrderScript, originalScript, trimLine, logFileName, selectEvery, selectLength);
		else
			return 0;
	}
	...
	{
		int mod = MeXgui::core::util::Resolution::GetModValue(mValue, undercrop, true);
	...
	{
		int mod = MeXgui::core::util::Resolution::GetModValue(mValue, overcrop, true);
