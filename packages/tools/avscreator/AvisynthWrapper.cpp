...
namespace MeXgui
{
	...
	AviSynthException::AviSynthException(const QString &message) : ApplicationException(message)
	{
	}
	...
	AviSynthException::AviSynthException(const QString &message, std::exception &innerException) : ApplicationException(message, innerException)
	{
	}

	QString AviSynthScriptEnvironment::GetLastError()
	{
		return 0;
	}
	...
	AviSynthClip *AviSynthScriptEnvironment::OpenScriptFile(const QString &filePath, AviSynthColorspace forceColorspace)
	{
		return new AviSynthClip("Import", filePath, forceColorspace, this);
	}

	AviSynthClip *AviSynthScriptEnvironment::ParseScript(const QString &script, AviSynthColorspace forceColorspace)
	{
		return new AviSynthClip("Eval", script, forceColorspace, this);
	}

	AviSynthClip *AviSynthScriptEnvironment::OpenScriptFile(const QString &filePath)
	{
		return OpenScriptFile(filePath, RGB24);
	}

	AviSynthClip *AviSynthScriptEnvironment::ParseScript(const QString &script)
	{
...
#if defined(dimzon)
	MeXgui::AviSynthClip::EnvRef *AviSynthClip::createNewEnvRef(int threadId)
	{
		//TODO:
...
#if defined(dimzon)
	MeXgui::AviSynthClip::EnvRef *AviSynthClip::addRef()
	{
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	QString AviSynthClip::getLastError()
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	const qint64 &AviSynthClip::getSamplesCount() const
	{
	...
	int AviSynthClip::GetIntVariable(const QString &variableName, int defaultValue)
	{
		int v = 0;
		int res = 0;
	...
	void AviSynthClip::ReadAudio(IntPtr addr, qint64 offset, int count)
	{
		if (0 != dimzon_avs_getaframe(_avs, addr, offset, count))
			throw new AviSynthException(getLastError());
	}

	void AviSynthClip::ReadAudio(unsigned char buffer, qint64 offset, int count)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	AviSynthClip::AviSynthClip(const QString &func, const QString &arg, AviSynthColorspace forceColorspace, AviSynthScriptEnvironment *env)
	{
		_vi = AVSDLLVideoInfo();
		_avs = new void*(0);
		_colorSpace = Unknown;
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	const qint64 &AviSynthClip::getAudioSizeInBytes() const
	{
