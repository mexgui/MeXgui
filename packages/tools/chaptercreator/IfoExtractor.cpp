...
namespace MeXgui
{
	...
	const QString &IfoExtractor::getExtensions() const
	{
		return new QString[] {"ifo"};
	}
	...
	QVector<ChapterInfo*> IfoExtractor::GetStreams(const QString &ifoFile)
	{
		QVector<ChapterInfo*> oList = QVector<ChapterInfo*>();
	...
	ChapterInfo *IfoExtractor::GetChapterInfo(const QString &location, int titleSetNum)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
	...
	QVector<Chapter> IfoExtractor::GetChapters(const QString &ifoFile, int programChain, TimeSpan *&duration, double &fps)
	{
		QVector<Chapter> chapters = QVector<Chapter>();
		duration = TimeSpan::Zero;
		fps = 0;

		qint64 pcgITPosition = IFOparser::GetPCGIP_Position(ifoFile);
		...
		int programQMapOffset = IFOparser::ToInt16(IFOparser::GetFileBlock(ifoFile, (pcgITPosition + longestChainOffset) + 230, 2));
		...
		{
			int entryCell = IFOparser::GetFileBlock(ifoFile, ((pcgITPosition + longestChainOffset) + programQMapOffset) + currentProgram, 1)[0];
			int exitCell = entryCell;
			if (currentProgram < (programChainPrograms - 1))
				exitCell = IFOparser::GetFileBlock(ifoFile, ((pcgITPosition + longestChainOffset) + programQMapOffset) + (currentProgram + 1), 1)[0] - 1;
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
			...
			if (currentProgram + 1 < programChainPrograms)
				chapters.push_back(Chapter(new object[] {Name = QString::Format("Chapter {0:D2}", currentProgram + 2), Time = duration}));
		}
		return chapters;
	}
}
