...
namespace BDInfo
{
	...
	public:
		...
		QVector<TSStreamClip*> StreamClips;
		QVector<QMap<unsigned short, TSStream*>*> AngleStreams;
		int AngleCount;
	  /*
	  ...
	  */
		...
		const quint64 &getFileSize() const;
		const double &getTotalLength() const;

		const double &getAngleLength() const;

		const quint64 &getTotalSize() const;

		const quint64 &getAngleSize() const;

		const quint64 &getTotalBitRate() const;

		const quint64 &getAngleBitRate() const;
