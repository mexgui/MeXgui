...
namespace BDInfo
{
	...
	QString TSStream::ToString()
	{
		return QString::Format("{0} ({1})", getCodecShortName(), PID);
	}

	const quint64 &TSStream::getPacketSize() const
	{
		return PacketCount * 192;
	}

	const QString &TSStream::getLanguageCode() const
	{
		return _LanguageCode;
	}

	void TSStream::setLanguageCode(const QString &value)
	{
		_LanguageCode = value;
	...
	const QString &TSStream::getCodecName() const
	{
		switch (StreamType)
		{
			case MPEG1_VIDEO:
	...
	const QString &TSStream::getCodecAltName() const
	{
		switch (StreamType)
		{
			case MPEG1_VIDEO:
	...
	const QString &TSStream::getCodecShortName() const
	{
		switch (StreamType)
		{
			case MPEG1_VIDEO:
	...
	const QString &TSStream::getDescription() const
	{
		return "";
	}
	...
	void TSStream::CopyTo(TSStream *stream)
	{
		...
		{
			stream->Descriptors = QVector<TSDescriptor*>();
			for (QVector<TSDescriptor*>::const_iterator descriptor = Descriptors.begin(); descriptor != Descriptors.end(); ++descriptor)
			{
	...
	const QString &TSVideoStream::getDescription() const
	{
		QString description = "";

		if (Height > 0)
		{
			description += QString::Format("{0:D}{1} / ", Height, IsInterlaced ? "i" : "p");
		}
		if (FrameRateEnumerator > 0 && FrameRateDenominator > 0)
		{
			if (FrameRateEnumerator % FrameRateDenominator == 0)
			{
				description += QString::Format("{0:D} fps / ", FrameRateEnumerator / FrameRateDenominator);
			}
			else
			{
				description += QString::Format("{0:F3} fps / ", static_cast<double>(FrameRateEnumerator) / FrameRateDenominator);
			}
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
	...
	const QString &TSAudioStream::getChannelDescription() const
	{
		QString description = "";
		if (ChannelCount > 0)
		{
			description += QString::Format("{0:D}.{1:D}", ChannelCount, LFE);
		}
		else
		{
	...
	const QString &TSAudioStream::getDescription() const
	{
		QString description = getChannelDescription();

		if (SampleRate > 0)
		{
			description += QString::Format(" / {0:D} kHz", SampleRate / 1000);
		}
		if (BitRate > 0)
		{
			description += QString::Format(" / {0:D} kbps", static_cast<unsigned int>(Math::Round(static_cast<double>(BitRate) / 1000)));
		}
		if (BitDepth > 0)
		{
			description += QString::Format(" / {0:D}-bit", BitDepth);
		}
		if (ChannelCount == 2)
		{
			switch (AudioMode)
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
		...
		if (CoreStream != 0)
		{
			...
			description += QString::Format(" ({0} Core: {1})", codec, CoreStream->getDescription());
		}
		return description;
	}
