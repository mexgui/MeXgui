...
namespace BDInfo
{
	...
	void TSStreamClipFile::Scan()
	{
		...
		try
		{
			...
			{
				throw std::exception(QString::Format("Clip info file {0} has an unknown file type {1}.", FileInfo->Name, FileType));
			}
	#if defined(DEBUG)                
	...
	#if defined(DEBUG)
			Debug::WriteLine(QString::Format("\tStreamCount: {0}", streamCount));
	#endif
			int streamOffset = 10;
			for (int streamIndex = 0; streamIndex < streamCount; streamIndex++)
			{
				...
				switch (streamType)
				{
	...
	#if defined(DEBUG)
						Debug::WriteLine(QString::Format("\t{0} {1} {2} {3} {4}", PID, streamType, videoFormat, frameRate, aspectRatio));
	#endif
					}
					break;
					...
					case MPEG2_AUDIO:
					{
						...
						QString languageCode = ASCIIEncoding::ASCII::GetString(languageBytes);
						...
						(static_cast<TSAudioStream*>(stream))->setLanguageCode(languageCode);
	#if defined(DEBUG)
						Debug::WriteLine(QString::Format("\t{0} {1} {2} {3} {4}", PID, streamType, languageCode, channelLayout, sampleRate));
	#endif
					}
					break;
	...
	#if defined(DEBUG)
						Debug::WriteLine(QString::Format("\t{0} {1} {2}", PID, streamType, languageCode));
	#endif
					}
					break;
