...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	const QVector<ChapterInfo*> &frmStreamSelect::getSelectedMultipleChapterInfo() const
	{
		return listBox1->SelectedItems->Count > 0 ? QVector<ChapterInfo*>(listBox1->SelectedItems->Cast<ChapterInfo*>()) : 0;
	}
	...
	void frmStreamSelect::btnOK_Click()
	{
	...
	void frmStreamSelect::btnSortName_CheckedChanged()
	{
		if (!btnSortName->Checked || listBox1->Items->Count == 0)
			return;

		QVector<ChapterInfo*> oSelectedList = QVector<ChapterInfo*>(listBox1->SelectedItems->Cast<ChapterInfo*>());

		QVector<ChapterInfo*> list = QVector<ChapterInfo*>(listBox1->Items->Cast<ChapterInfo*>());
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
		...
		if (oSelectedList.size() > 0)
		{
			for (int i = 0; i < listBox1->Items->Count; i++)
			{
				for (QVector<ChapterInfo*>::const_iterator oSelectedItem = oSelectedList.begin(); oSelectedItem != oSelectedList.end(); ++oSelectedItem)
				{
	...
	void frmStreamSelect::btnSortDuration_CheckedChanged()
	{
