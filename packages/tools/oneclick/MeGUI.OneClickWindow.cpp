...
namespace MeXgui
{
	...
	{
		audioTracks = QVector<OneClickStreamControl*>();
		...
		subtitleTracks = QVector<OneClickStreamControl*>();
	...
	void OneClickWindow::OneClickProfileChanged()
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		...
		bLock = true;
		cbGUIMode->DataSource = EnumProxy::CreateArray(new object[] {MeXguiSettings::Basic, MeXguiSettings::Default, MeXguiSettings::Advanced});
		bLock = false;
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	void OneClickWindow::cbGUIMode_SelectedIndexChanged()
	{
		...
		if (bLock)
			...
			MainForm::Instance->getSettings()->setOneClickGUIMode(static_cast<MeXguiSettings::OCGUIMode>(o->getRealValue()));

		if (MainForm::Instance->getSettings()->getOneClickGUIMode() == MeXguiSettings::Advanced)
		{
			audioTab->Height = 175;
		...
		else if (MainForm::Instance->getSettings()->getOneClickGUIMode() == MeXguiSettings::Basic)
		{
			audioTab->Height = 90;
	...
	void OneClickWindow::containerFormat_SelectedIndexChanged()
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void OneClickWindow::updateWorkingName(const QString &strInputFile)
	{
		int iPGCNumber = 0;
		...
		if (iPGCNumber > 1)
		{
			QString strTempName = Path::GetFileNameWithoutExtension(strInputFile);
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
	...
	void OneClickWindow::updateFilename()
	{
		if (!outputChosen)
		{
			QString strVideoInput = input->getSelectedText();
	...
	void OneClickWindow::input_SelectionChanged(object *sender, const QString &val)
	{
	...
	void OneClickWindow::setOutput(const QString &strFileName)
	{
	...
	void OneClickWindow::setWorkingDirectory(const QString &strFolder)
	{
	...
	void OneClickWindow::workingName_TextChanged()
	{
		updateFilename();
	}

	void OneClickWindow::setInput(const QString &strFileorFolderName)
	{
		input->AddCustomItem(strFileorFolderName);
		input->setSelectedObject(strFileorFolderName);
	}

	void OneClickWindow::setInput(QString& strFileorFolderName[])
	{
		QVector<OneClickFilesToProcess*> arrFilesToProcess = QVector<OneClickFilesToProcess*>();
		for (QString::const_iterator strFile = strFileorFolderName->begin(); strFile != strFileorFolderName->end(); ++strFile)
	...
	void OneClickWindow::openInput(const QString &fileName)
	{
	...
	void OneClickWindow::setBatchProcessing(QVector<OneClickFilesToProcess*> &arrFilesToProcess, OneClickSettings *oSettings)
	{
	...
	void OneClickWindow::setInputData(MediaInfoFile *iFile, QVector<OneClickFilesToProcess*> &arrFilesToProcess)
	{
		if (iFile == 0)
			return;
		...
		if (!bAutomatedProcessing && arrFilesToProcess.size() > 0)
		{
			...
			if (dr == System::Windows::Forms::DialogResult::Yes)
			{
				...
				SetOneClickProfile(static_cast<OneClickSettings*>(oneclickProfile->getSelectedProfile()->BaseSettings));
				_oSettings->setLeadingName(MeXgui::core::gui::InputBox::Show("If desired please enter a leading name", "Please enter a leading name", _oSettings->getLeadingName()));
			}
		}
		bLock = true;
		if (input->getSelectedSCItem() == 0 || !iFile->getFileName().Equals(static_cast<QString>(input->getSelectedObject())))
		{
		...
		QVector<OneClickStream*> arrAudioTrackInfo = QVector<OneClickStream*>();
		for (QVector<AudioTrackInfo*>::const_iterator oInfo = iFile->getAudioInfo()->getTracks().begin(); oInfo != iFile->getAudioInfo()->getTracks().end(); ++oInfo)
			arrAudioTrackInfo.push_back(new OneClickStream(*oInfo));
		AudioResetTrack(arrAudioTrackInfo, _oSettings);

		QVector<OneClickStream*> arrSubtitleTrackInfo = QVector<OneClickStream*>();
		for (QVector<SubtitleTrackInfo*>::const_iterator oInfo = iFile->getSubtitleInfo()->getTracks().begin(); oInfo != iFile->getSubtitleInfo()->getTracks().end(); ++oInfo)
		...
		{
			QString chapterFile = VideoUtil::getChapterFile(iFile->getFileName());
		...
		{
			QString strPath = Path::GetDirectoryName(iFile->getFileName());
	...
	void OneClickWindow::updatePossibleContainers()
	{
		...
		QVector<AudioEncoderType*> audioCodecs = QVector<AudioEncoderType*>();
		QVector<MuxableType*> dictatedOutputTypes = QVector<MuxableType*>();
		...
		QVector<ContainerType*> tempSupportedOutputTypes = this->muxProvider->GetSupportedContainers(getVideoSettings()->getEncoderType(), audioCodecs.ToArray(), dictatedOutputTypes.ToArray());

		QVector<ContainerType*> supportedOutputTypes = QVector<ContainerType*>();

		for (MeXgui::ContainerType::const_iterator c = acceptableContainerTypes->begin(); c != acceptableContainerTypes->end(); ++c)
		...
		if (supportedOutputTypes.empty())
		{
			if (tempSupportedOutputTypes.size() > 0 && !ignoreRestrictions)
			{
				if (!bAutomatedProcessing)
				{
					QString message = QString::Format("No container type could be found that matches the list of acceptable types " + "in your chosen one click profile. {0}" + "Your restrictions are now being ignored.", "\r\n");
	...
	void OneClickWindow::SetOneClickProfile(OneClickSettings *settings)
	{
		...
		for (QVector<OneClickStreamControl*>::const_iterator oStream = audioTracks.begin(); oStream != audioTracks.end(); ++oStream)
			(*oStream)->setDefaultLanguage(_oSettings->getDefaultLanguage());

		for (QVector<OneClickStreamControl*>::const_iterator oStream = subtitleTracks.begin(); oStream != subtitleTracks.end(); ++oStream)
			(*oStream)->setDefaultLanguage(_oSettings->getDefaultLanguage());

		if (_videoInputInfo != 0)
		{
			QVector<OneClickStream*> arrAudioTrackInfo = QVector<OneClickStream*>();
			for (QVector<AudioTrackInfo*>::const_iterator oInfo = _videoInputInfo->getAudioInfo()->getTracks().begin(); oInfo != _videoInputInfo->getAudioInfo()->getTracks().end(); ++oInfo)
				arrAudioTrackInfo.push_back(new OneClickStream(*oInfo));
			AudioResetTrack(arrAudioTrackInfo, settings);

			QVector<OneClickStream*> arrSubtitleTrackInfo = QVector<OneClickStream*>();
			for (QVector<SubtitleTrackInfo*>::const_iterator oInfo = _videoInputInfo->getSubtitleInfo()->getTracks().begin(); oInfo != _videoInputInfo->getSubtitleInfo()->getTracks().end(); ++oInfo)
				arrSubtitleTrackInfo.push_back(new OneClickStream(*oInfo));
		...
		QVector<ContainerType*> temp = QVector<ContainerType*>();
		QVector<ContainerType*> allContainerTypes = muxProvider->GetSupportedContainers();
		for (QString::const_iterator s = settings->getContainerCandidates()->begin(); s != settings->getContainerCandidates()->end(); ++s)
		{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void OneClickWindow::goButton_Click()
	{
		goButton->Enabled = false;
		createOneClickJob(0);
	}

	void OneClickWindow::createOneClickJob(QVector<OneClickFilesToProcess*> &arrFilesToProcess)
	{
		...
		if (_videoInputInfo->isEac3toDemuxable())
		{
			...
			sb->Append(QString::Format("{0}:\"{1}\" ", _videoInputInfo->getVideoInfo()->getTrack()->getTrackID(), dpp->getVideoInput()));
			...
			if (dpp->getChapterFile().Equals("<internal chapters>"))
			{
				...
				if (iTrackNumber > -1)
				{
					...
					sb->Append(QString::Format("{0}:\"{1}\" ", iTrackNumber, dpp->getChapterFile()));
					dpp->getFilesToDelete().push_back(dpp->getChapterFile());
				}
			}

			for (QVector<OneClickStreamControl*>::const_iterator oStreamControl = audioTracks.begin(); oStreamControl != audioTracks.end(); ++oStreamControl)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
				...
				sb->Append(QString::Format("{0}:\"{1}\" ", (*oStreamControl)->getSelectedStream()->TrackInfo->TrackID, Path::Combine(dpp->getWorkingDirectory(), (*oStreamControl)->getSelectedStream()->TrackInfo->DemuxFileName)));
				if (bCoreOnly)
					sb->Append("-core ");
			...
			for (QVector<OneClickStreamControl*>::const_iterator oStreamControl = subtitleTracks.begin(); oStreamControl != subtitleTracks.end(); ++oStreamControl)
			{
				if (!(*oStreamControl)->getSelectedItem()->IsStandard)
					continue;
				...
				QString strDemuxFilePath = Path::Combine(dpp->getWorkingDirectory(), (*oStreamControl)->getSelectedStream()->TrackInfo->DemuxFileName);
				sb->Append(QString::Format("{0}:\"{1}\" ", (*oStreamControl)->getSelectedStream()->TrackInfo->MMGTrackID, strDemuxFilePath));
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			{
				const QString tempVector[] = {_videoInputInfo->getFileName()};
				prepareJobs = new SequentialChain(prepareJobs, new HDStreamsExJob(QVector<QString>(tempVector, tempVector + sizeof(tempVector) / sizeof(tempVector[0])), dpp->getWorkingDirectory(), 0, sb->ToString(), 2));
			}
		}
	...
	dpp->setAutoDeinterlace(dpp->getPrerenderJob());
		...
		QVector<TrackInfo*> oExtractMKVTrack = QVector<TrackInfo*>();

		// get audio information
		JobChain *audioJobs = 0;
		QVector<AudioTrackInfo*> arrAudioTrackInfo = QVector<AudioTrackInfo*>();
		for (QVector<OneClickStreamControl*>::const_iterator oStreamControl = audioTracks.begin(); oStreamControl != audioTracks.end(); ++oStreamControl)
		{
			...
			else
			{
				...
				if (!(*oStreamControl)->getSelectedItem()->IsStandard || !dpp->getEac3toDemux())
				{
					if (strAudioCodec.ToLower(System::Globalization::CultureInfo::InvariantCulture)->Contains("truehd"))
					{
						...
						const QString tempVector2[] = {strFileName};
						HDStreamsExJob *oJob = new HDStreamsExJob(QVector<QString>(tempVector2, tempVector2 + sizeof(tempVector2) / sizeof(tempVector2[0])), aInput, 0, "\"" + aInput + "\"", 2);
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
					else if (strAudioCodec.StartsWith("DTS-HD", StringComparison::InvariantCultureIgnoreCase))
					{
						...
						const QString tempVector3[] = {strFileName};
						HDStreamsExJob *oJob = new HDStreamsExJob(QVector<QString>(tempVector3, tempVector3 + sizeof(tempVector3) / sizeof(tempVector3[0])), aInput, 0, "\"" + aInput + "\" -core", 2);
						audioJobs = new SequentialChain(audioJobs, new SequentialChain(oJob));
						dpp->getFilesToDelete().push_back(FileUtil::AddToFileName(Path::ChangeExtension(aInput, "txt"), " - Log"));
						dpp->getFilesToDelete().push_back(aInput);
					}
					else if ((*oStreamControl)->getSelectedItem()->IsStandard && strAudioCodec.Equals("PCM", StringComparison::InvariantCultureIgnoreCase))
					{
						if (_videoInputInfo->getContainerFileType() == ContainerType::MKV)
						{
							...
							const QString tempVector4[] = {dpp->getVideoInput()};
							HDStreamsExJob *oJob = new HDStreamsExJob(QVector<QString>(tempVector4, tempVector4 + sizeof(tempVector4) / sizeof(tempVector4[0])), aInput, 0, (*oStreamControl)->getSelectedStream()->TrackInfo->TrackID + ":\"" + aInput + "\"", 2);
							audioJobs = new SequentialChain(audioJobs, new SequentialChain(oJob));
							dpp->getFilesToDelete().push_back(FileUtil::AddToFileName(Path::ChangeExtension(aInput, "txt"), " - Log"));
							dpp->getFilesToDelete().push_back(aInput);
						}
		...
		for (QVector<OneClickStreamControl*>::const_iterator oStream = subtitleTracks.begin(); oStream != subtitleTracks.end(); ++oStream)
		{
			...
			{
				QString strExtension = Path::GetExtension((*oStream)->getSelectedStream()->TrackInfo->SourceFileName->ToLower(System::Globalization::CultureInfo::InvariantCulture));
				if (strExtension.Equals(".ifo") || strExtension.Equals(".vob"))
				{
					QString strInput = (*oStream)->getSelectedStream()->TrackInfo->SourceFileName;
					...
					QString outputFile = Path::Combine(dpp->getWorkingDirectory(), Path::GetFileNameWithoutExtension(strInput)) + "_" + (*oStream)->getSelectedStream()->TrackInfo->MMGTrackID + ".idx";
					SubtitleIndexJob *oJob = new SubtitleIndexJob(strInput, outputFile, false, QVector<int>(new *oStream[] {(*oStream)->getSelectedStream()->TrackInfo->MMGTrackID}), _videoInputInfo->getVideoInfo()->PGCNumber);
		...
		for (QVector<OneClickAudioTrack*>::const_iterator oTrack = dpp->getAudioTracks().begin(); oTrack != dpp->getAudioTracks().end(); ++oTrack)
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		...
		for (QVector<OneClickStream*>::const_iterator oTrack = dpp->getSubtitleTracks().begin(); oTrack != dpp->getSubtitleTracks().end(); ++oTrack)
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	bool OneClickWindow::verifyInputSettings(MediaInfoFile *oVideoInputInfo, const QString &strWorkingDirectory)
	{
	...
	QString OneClickWindow::verifyStreamSettings()
	{
		for (int i = 0; i < audioTracks.size(); ++i)
		{
			...
			QString r = MainForm::verifyInputFile(audioTracks[i]->getSelectedFile());
			if (r != "")
				return r;
		}
		for (int i = 0; i < subtitleTracks.size(); ++i)
		{
			...
			QString r = MainForm::verifyInputFile(subtitleTracks[i]->getSelectedFile());
			if (r != "")
				return r;
		}
		return 0;
	...
	void OneClickWindow::audio1_SomethingChanged()
	{
		updatePossibleContainers();
	}

	void OneClickWindow::ProfileChanged()
	{
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
	...
	void OneClickWindow::keepInputResolution_CheckedChanged()
	{
	...
	void OneClickWindow::chkDontEncodeVideo_CheckedChanged()
	{
		if (chkDontEncodeVideo->Checked)
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
	...
	void OneClickWindow::subtitleAddTrack_Click()
	{
		SubtitleAddTrack(true);
	}

	void OneClickWindow::subtitleRemoveTrack_Click()
	{
	...
	void OneClickWindow::chkDefaultStream_CheckedChanged()
	{
		if ((static_cast<CheckBox*>(sender))->Checked == false)
			return;

		for (QVector<OneClickStreamControl*>::const_iterator oTrack = subtitleTracks.begin(); oTrack != subtitleTracks.end(); ++oTrack)
		{
	...
	void OneClickWindow::oneClickSubtitleStreamControl_FileUpdated()
	{
		if (bLock)
			return;
		...
		for (QVector<OneClickStreamControl*>::const_iterator oControl = subtitleTracks.begin(); oControl != subtitleTracks.end(); ++oControl)
		{
			if (*oControl == track)
				continue;
	...
	void OneClickWindow::SubtitleResetTrack(QVector<OneClickStream*> &arrSubtitleTrackInfo, OneClickSettings *settings)
	{
		// generate track names
		QVector<object*> trackNames = QVector<object*>();
		trackNames.push_back("None");
		for (QVector<OneClickStream*>::const_iterator o = arrSubtitleTrackInfo.begin(); o != arrSubtitleTrackInfo.end(); ++o)
		...
		for (QVector<QString>::const_iterator strLanguage = settings->getDefaultSubtitleLanguage().begin(); strLanguage != settings->getDefaultSubtitleLanguage().end(); ++strLanguage)
	...
	void OneClickWindow::audioAddTrack_Click()
	{
	   AudioAddTrack(true);
	}

	void OneClickWindow::audioRemoveTrack_Click()
	{
	...
	void OneClickWindow::oneClickAudioStreamControl_FileUpdated()
	{
		if (bLock)
			return;
		...
		for (QVector<OneClickStreamControl*>::const_iterator oControl = audioTracks.begin(); oControl != audioTracks.end(); ++oControl)
		{
			if (*oControl == track)
				continue;
	...
	void OneClickWindow::AudioResetTrack(QVector<OneClickStream*> &arrAudioTrackInfo, OneClickSettings *settings)
	{
		// generate track names
		QVector<object*> trackNames = QVector<object*>();
		trackNames.push_back("None");
		for (QVector<OneClickStream*>::const_iterator o = arrAudioTrackInfo.begin(); o != arrAudioTrackInfo.end(); ++o)
			trackNames.push_back(*o);
		...
		for (QVector<QString>::const_iterator strLanguage = settings->getDefaultAudioLanguage().begin(); strLanguage != settings->getDefaultAudioLanguage().end(); ++strLanguage)
			if ((*strLanguage).Equals("[none]"))
				return;
		...
		for (QVector<QString>::const_iterator strLanguage = settings->getDefaultAudioLanguage().begin(); strLanguage != settings->getDefaultAudioLanguage().end(); ++strLanguage)
		{
			for (int i = 0; i < arrAudioTrackInfo.size(); i++)
			{
				if (arrAudioTrackInfo[i]->getLanguage()->ToLower(System::Globalization::CultureInfo::InvariantCulture)->Equals((*strLanguage).ToLower(System::Globalization::CultureInfo::InvariantCulture)))
				{
					...
					for (QVector<OneClickAudioSettings*>::const_iterator oAudioSettings = settings->getAudioSettings().begin(); oAudioSettings != settings->getAudioSettings().end(); ++oAudioSettings)
					{
					...
					{
						for (QVector<OneClickStreamControl*>::const_iterator oAudioControl = audioTracks.begin(); oAudioControl != audioTracks.end(); ++oAudioControl)
						{
	...
	{
		for (QVector<OneClickStreamControl*>::const_iterator a = audioTracks.begin(); a != audioTracks.end(); ++a)
		{
			bool bFound = false;
	...
	void OneClickWindow::OneClickWindow_Shown()
	{
	...
	void OneClickWindow::audioTab_SelectedIndexChanged()
	{
	...
	void OneClickWindow::subtitlesTab_SelectedIndexChanged()
	{
	...
	void OneClickWindow::InitializeComponent()
	{
		...
		this->oneclickProfile = new MeXgui::core::gui::ConfigableProfilesControl();
		this->output = new MeXgui::FileBar();
		this->input = new MeXgui::core::gui::FileSCBox();
		this->oneClickAudioStreamControl1 = new MeXgui::OneClickStreamControl();
		this->oneClickSubtitleStreamControl1 = new MeXgui::OneClickStreamControl();
		this->avsProfile = new MeXgui::core::gui::ConfigableProfilesControl();
		this->ar = new MeXgui::core::gui::ARChooser();
		this->chapterFile = new MeXgui::FileBar();
		...
		this->output->FileSelected += new MeXgui::FileBarEventHandler(this, &OneClickWindow::output_FileSelected);
		// 
		// input
		// 
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
		...
		this->input->setType(MeXgui::core::gui::FileSCBox::OC_FILE_AND_FOLDER);
		this->input->SelectionChanged += new MeXgui::StringChanged(this, &OneClickWindow::input_SelectionChanged);
		// 
		...
		this->ar->setCustomDARs(new MeXgui::core::util::Dar[0]);
		...
		this->workingDirectory->FileSelected += new MeXgui::FileBarEventHandler(this, &OneClickWindow::workingDirectory_FileSelected);
		// 
		// fileSize
		// 
		...
		this->Text = "MeXgui - One Click Encoder";
