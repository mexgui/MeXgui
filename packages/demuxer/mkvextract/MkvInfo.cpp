...
namespace MeXgui
{
	...
	MkvInfo::MkvInfo(const QString &strFile, LogItem *&oLog)
	{
...
//		using (Process mkvinfo = new Process())
		...
		try
		{
			...
			mkvinfo->StartInfo->Arguments = QString::Format("--ui-language en --identify-verbose \"{0}\"", _strFile);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	bool MkvInfo::extractChapters(const QString &strChapterFile)
	{
		_strResult = "";
		bool bResult = false;
//C# TO C++ CONVERTER NOTE: The following 'using' block is replaced by its C++ equivalent:
//		using (Process mkvinfo = new Process())
		...
		try
		{
			...
			mkvinfo->StartInfo->Arguments = QString::Format("chapters \"{0}\" --ui-language en --simple", _strFile);
			mkvinfo->StartInfo->CreateNoWindow = true;
			mkvinfo->StartInfo->UseShellExecute = false;
			mkvinfo->StartInfo->RedirectStandardOutput = true;
			mkvinfo->StartInfo->RedirectStandardError = true;
