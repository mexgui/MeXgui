...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ template equivalent to generic constraints:
	...
	protected:
		...
		QString executable; // path and filename of the commandline encoder to be used
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	private:
		...
		{
			for (QVector<QString>::const_iterator filePath = tempFiles.begin(); filePath != tempFiles.end(); ++filePath)
				safeDelete(*filePath);
	...
	protected:
		...
		virtual const QString &getCommandline() const = 0;
		...
		void proc_Exited()
		{
			...
			{
				QString strError = WindowUtil::GetErrorText(proc->ExitCode);
				if (!su->getWasAborted())
				{
					su->setHasError(true);
	...
	public:
		...
		void start()
		{
			...
			try
			{
				...
				stdoutLog = log->Info(QString::Format("[{0:G}] {1}", DateTime::Now, "Standard output stream"));
				stderrLog = log->Info(QString::Format("[{0:G}] {1}", DateTime::Now, "Standard error stream"));
