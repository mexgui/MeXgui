...
namespace MeXgui
{
	...
	baseMuxWindow::baseMuxWindow()
	{
		...
		audioTracks = QVector<MuxStreamControl*>();
		audioTracks.push_back(muxStreamControl2);
		subtitleTracks = QVector<MuxStreamControl*>();
		subtitleTracks.push_back(muxStreamControl1);

		splitting->setMinimumFileSize(FileSize(MB, 1));
		subtitleTracks[0]->input->FileSelected += new MeXgui::FileBarEventHandler(this, &baseMuxWindow::Subtitle_FileSelected);
		audioTracks[0]->input->FileSelected += new MeXgui::FileBarEventHandler(this, &baseMuxWindow::Audio_FileSelected);
	}
	...
	void baseMuxWindow::setConfig(const QString &videoInput, const QString &videoName, Nullable<decimal> framerate, MuxStream audioStreams[], MuxStream subtitleStreams[], const QString &chapterFile, const QString &output, Nullable<FileSize> splitSize, Nullable<Dar> dar, const QString &deviceType)
	{
		this->dar = dar;
		...
		int index = 0;
		for (MeXgui::core::details::MuxStream::const_iterator stream = audioStreams->begin(); stream != audioStreams->end(); ++stream)
		{
		...
		index = 0;
		for (MeXgui::core::details::MuxStream::const_iterator stream = subtitleStreams->begin(); stream != subtitleStreams->end(); ++stream)
		{
	...
	void baseMuxWindow::getAdditionalStreams(MuxStream aStreams[], MuxStream sStreams[], QString &chapterFile)
	{
	...
	MuxStream *baseMuxWindow::getStreams(QVector<MuxStreamControl*> &controls)
	{
		QVector<MuxStream*> streams = QVector<MuxStream*>();
		for (QVector<MuxStreamControl*>::const_iterator t = controls.begin(); t != controls.end(); ++t)
		{
			if ((*t)->getStream() != 0)
	...
	void baseMuxWindow::InitializeComponent()
	{
		...
		this->chapters = new MeXgui::FileBar();
		...
		this->muxStreamControl2 = new MeXgui::core::details::mux::MuxStreamControl();
		...
		this->muxStreamControl1 = new MeXgui::core::details::mux::MuxStreamControl();
		...
		this->fps->TabIndex = 3;
		this->fps->SelectionChanged += new MeXgui::StringChanged(this, &baseMuxWindow::fps_SelectionChanged);
		// 
		// videoNameLabel
		// 
		...
		this->vInput->FileSelected += new MeXgui::FileBarEventHandler(this, &baseMuxWindow::vInput_FileSelected);
		// 
		// MuxFPSLabel
		// 
		...
		this->chapters->FileSelected += new MeXgui::FileBarEventHandler(this, &baseMuxWindow::chapters_FileSelected);
		// 
		// outputGroupbox
		// 
	...
	void baseMuxWindow::Subtitle_FileSelected()
	{
		for (QVector<MuxStreamControl*>::const_iterator oTrack = subtitleTracks.begin(); oTrack != subtitleTracks.end(); ++oTrack)
		{
	...
	void baseMuxWindow::Audio_FileSelected()
	{
		for (QVector<MuxStreamControl*>::const_iterator oTrack = audioTracks.begin(); oTrack != audioTracks.end(); ++oTrack)
		{
			if ((*oTrack)->input->Filename->empty())
				return;
		}
	...
	void baseMuxWindow::chkDefaultStream_CheckedChanged()
	{
	...
	{
		QString projectPath;
		QString fileNameNoPath = Path::GetFileName(vInput->getFilename());
	...
	void baseMuxWindow::fps_SelectionChanged(object *sender, const QString &val)
	{
		checkIO();
	}
	...
	void baseMuxWindow::audioAddTrack_Click()
	{
		AudioAddTrack();
	}
	...
	void baseMuxWindow::AudioAddTrack()
	{
		...
		a->input->FileSelected += new MeXgui::FileBarEventHandler(this, &baseMuxWindow::Audio_FileSelected);
	...
	void baseMuxWindow::audioRemoveTrack_Click()
	{
		AudioRemoveTrack();
	}
	...
	void baseMuxWindow::subtitleAddTrack_Click()
	{
		SubtitleAddTrack();
	}
	...
	void baseMuxWindow::SubtitleAddTrack()
	{
		...
		a->input->FileSelected += new MeXgui::FileBarEventHandler(this, &baseMuxWindow::Subtitle_FileSelected);
	...
	void baseMuxWindow::subtitleRemoveTrack_Click()
	{
		SubtitleRemoveTrack();
	}
	...
	void baseMuxWindow::muxStreamControl1_FileUpdated()
	{
		fileUpdated();
	}

	void baseMuxWindow::muxStreamControl2_FileUpdated()
	{
		fileUpdated();
	}

	void baseMuxWindow::cbContainer_SelectedIndexChanged()
	{
		upDeviceTypes();
	}

	void baseMuxWindow::output_Click()
	{
		checkIO();
		fileUpdated();
	}

	void baseMuxWindow::removeVideoTrack_Click()
	{
		vInput->setFilename("");
		videoName->Text = "";
		fps->setValue(Nullable<decimal>());
	...
	void baseMuxWindow::cbType_SelectedIndexChanged()
	{
