...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
		info->VideoInputChanged += new StringChanged(delegate(object _, QString val)
		{
			videoInput->setFilename(val);
		}
	   );
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
		info->VideoOutputChanged += new StringChanged(delegate(object _, QString val)
		{
	...
	const QString &VideoEncodingComponent::getFileType() const
	{
		return fileType->Text;
		;
	}

	void VideoEncodingComponent::setFileType(const QString &value)
	{
		fileType->Text = value;
	}
	...
	const QString &VideoEncodingComponent::getVideoInput() const
	{
		return info->getVideoInput();
	}

	void VideoEncodingComponent::setVideoInput(const QString &value)
	{
		info->setVideoInput(value);
	}

	const QString &VideoEncodingComponent::getVideoOutput() const
	{
		return info->getVideoOutput();
	}

	void VideoEncodingComponent::setVideoOutput(const QString &value)
	{
	...
	void VideoEncodingComponent::openAvisynthScript(const QString &fileName)
	{
	...
	void VideoEncodingComponent::VideoInput_DoubleClick()
	{
	...
	void VideoEncodingComponent::queueVideoButton_Click()
	{
		fileType_SelectedIndexChanged(sender, e); // to select always correct output file extension
		QString settingsError = verifyVideoSettings(); // basic input, logfile and output file settings are okay
		if (settingsError != "")
		{
		...
		QString videoOutput = info->getVideoOutput();
	...
	void VideoEncodingComponent::fileType_SelectedIndexChanged()
	{
	...
	QString VideoEncodingComponent::verifyVideoSettings()
	{
		// test for valid input filename
		if (this->getVideoInput().empty())
			return "Please specify a video input file";

		QString fileErr = MainForm::verifyInputFile(this->getVideoInput());
		if (fileErr != "")
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Replace' method:
	...
	void VideoEncodingComponent::openVideoFile(const QString &fileName)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
		...
		QString fileNameNoExtension = Path::GetFileNameWithoutExtension(fileName);
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	void VideoEncodingComponent::player_Closed()
	{
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	{
		for (MeXgui::Zone::const_iterator z = getInfo()->getZones()->begin(); z != getInfo()->getZones()->end(); ++z)
		{
	...
	const MeXgui::VideoEncoderProvider &VideoEncodingComponent::getVideoEncoderProvider() const
	{
	...
	void VideoEncodingComponent::addAnalysisPass_Click()
	{
	...
	void VideoEncodingComponent::videoProfile_SelectedProfileChanged()
	{
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
	...
	void VideoEncodingComponent::editZonesButton_Click()
	{
		...
		if (zw->show() == DialogResult::OK)
			getInfo()->setZones(zw->getZones());
	}

	void VideoEncodingComponent::videopreview_Click()
	{
	...
	void VideoEncodingComponent::InitializeComponent()
	{
		...
		this->videoInput->FileSelected += new MeXgui::FileBarEventHandler(this, &VideoEncodingComponent::videoInput_FileSelected);
		// 
		// videoOutput
		// 
		...
		this->videoOutput->FileSelected += new MeXgui::FileBarEventHandler(this, &VideoEncodingComponent::videoOutput_FileSelected);
		// 
		// fileType
		// 
