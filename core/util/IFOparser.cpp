...
namespace MeXgui
{
	namespace core
	{
		namespace util
		{
			...
			QString IFOparser::DetermineMovieIFO(const QString &inputPath)
			{
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
				QString *vobFiles = Directory::GetFiles(inputPath, "vts*.vob");
				if (sizeof(vobFiles) / sizeof(vobFiles[0]) == 0)
					return 0;

				// Look for the largest VOB set
				QString vtsNameCurrent;
				QString vtsNamePrevious = Path::GetFileName(vobFiles[0])->substr(0, 7);
				qint64 vtsSizeLargest = 0;
				qint64 vtsSize = 0;
				QString vtsNumber = "01";
				for (QString::const_iterator file = vobFiles->begin(); file != vobFiles->end(); ++file)
				{
				...
				QString ifoFile = inputPath + Path::DirectorySeparatorChar + "VTS_" + vtsNumber + "_0.IFO";
				// Name of largest VOB set is the name of the IFO, so we can now create the IFO file
				return ifoFile;
			}

			unsigned char *IFOparser::GetFileBlock(const QString &strFile, qint64 pos, int count)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
			...
			qint64 IFOparser::ToFilePosition(unsigned char bytes[])
			{
				return ToInt32(bytes) * 0x800LL;
			}

			qint64 IFOparser::GetTotalFrames(TimeSpan *time, int fps)
			{
				return static_cast<qint64>(Math::Round(fps * time->TotalSeconds));
			}

			QString IFOparser::TwoLong(int val)
			{
				return QString::Format("{0:D2}", val);
			}

			int IFOparser::AsHex(int val)
			{
				int ret;
				int::TryParse(QString::Format("{0:X2}", val), ret);
				return ret;
			}
			...
			qint64 IFOparser::GetPCGIP_Position(const QString &ifoFile)
			{
				return ToFilePosition(GetFileBlock(ifoFile, 0xCC, 4));
			}

			int IFOparser::GetProgramChains(const QString &ifoFile, qint64 pcgitPosition)
			{
				return ToInt16(GetFileBlock(ifoFile, pcgitPosition, 2));
			}

			unsigned int IFOparser::GetChainOffset(const QString &ifoFile, qint64 pcgitPosition, int programChain)
			{
				return ToInt32(GetFileBlock(ifoFile, (pcgitPosition + (8 * programChain)) + 4, 4));
			}

			int IFOparser::GetNumberOfPrograms(const QString &ifoFile, qint64 pcgitPosition, unsigned int chainOffset)
			{
				return GetFileBlock(ifoFile, (pcgitPosition + chainOffset) + 2, 1)[0];
			}

			Nullable<TimeSpan*> IFOparser::ReadTimeSpan(const QString &ifoFile, qint64 pcgitPosition, unsigned int chainOffset, double &fps)
			{
			...
			QString IFOparser::getAudioLanguage(const QString &FileName, int count)
			{
				...
				br->Read(buff, 0, 2);
				QString ShortLangCode = QString::Format("{0}{1}", static_cast<wchar_t>(buff[0]), static_cast<wchar_t>(buff[1]));
				QString audioLang = LanguageSelectionContainer::Short2FullLanguageName(ShortLangCode);
				fs->Close();
				return audioLang;
			}

			QString *IFOparser::GetSubtitlesStreamsInfos(const QString &FileName, int iPGC, bool bGetAllStreams)
			{
				unsigned char buff[4];
				unsigned char s = 0;
				QString subdesc[s];
				QString substreams[s];
					...
					for (int i = 0; i < 32; i++)
					{
						...
						if (buff[0] > 0)
							if ((substreams[buff[0]]).empty())
								substreams[buff[0]] = "[" + QString::Format("{0:00}", buff[0]) + "] - " + subdesc[i];
						if (buff[1] > 0)
							if ((substreams[buff[1]]).empty())
								substreams[buff[1]] = "[" + QString::Format("{0:00}", buff[1]) + "] - " + subdesc[i];
						if (buff[2] > 0)
							if ((substreams[buff[2]]).empty())
								substreams[buff[2]] = "[" + QString::Format("{0:00}", buff[2]) + "] - " + subdesc[i];
						if (buff[3] > 0)
							if ((substreams[buff[3]]).empty())
								substreams[buff[3]] = "[" + QString::Format("{0:00}", buff[3]) + "] - " + subdesc[i];
					...
					if (bGetAllStreams)
					{
						for (int i = 0; i < 32; i++)
							if ((substreams[i]).empty())
								substreams[i] = "[" + QString::Format("{0:00}", i) + "] - not detected";
					}
					else
					{
						ArrayList *arrList = new ArrayList();
						for (QString::const_iterator strItem = substreams->begin(); strItem != substreams->end(); ++strItem)
							if (!(*strItem).empty())
								arrList->Add(*strItem);
						substreams = new QString[arrList->Count];
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			unsigned int IFOparser::getPGCnb(const QString &FileName)
			{
				FileStream *fs = new FileStream(FileName, FileMode->Open, FileAccess::Read);
				BinaryReader *br = new BinaryReader(fs);
				Stream *sr = br->BaseStream;
				...
				qint64 VTS_PGCITI_start_position = sr->Position - 1;
			...
			QString IFOparser::GetVideoAR(const QString &FileName)
			{
				FileStream *fs = new FileStream(FileName, FileMode->Open, FileAccess::Read);
				BinaryReader *br = new BinaryReader(fs);
				Stream *sr = br->BaseStream;
