...
template<typename TException> where TException : Exception
			void Util::CatchAndTellUser(const QString &processDescription, Action action)
			{
				try
				{
					action();
...
template<typename TException> where TException : Exception
			...
			void Util::CatchExceptionsAndTellUser(const QString &processDescription, Action action)
			{
			...
			void Util::XmlSerialize(object *o, const QString &path)
			{
...
template<typename T> where T : class, new()
			T Util::XmlDeserializeOrDefault(const QString &path)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
			...
			object *Util::XmlDeserialize(const QString &path, Type *t, bool bSilentError)
			{
...
template<typename T> where T : class
			T Util::XmlDeserialize(const QString &path, bool bSilentError)
			{
				XmlSerializer *ser = new XmlSerializer(T::typeid);
				if (File::Exists(path))
				{
...
System::Text::RegularExpressions::Regex *const Util::_cleanUpStringRegex = new System::Text::RegularExpressions::Regex("\\n[^\\n]+\\r", System::Text::RegularExpressions::RegexOptions::Compiled | System::Text::RegularExpressions::RegexOptions::CultureInvariant);
			...
			QString Util::cleanUpString(const QString &s)
			{
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Replace' method:
			...
			void Util::ensureExists(const QString &file)
			{
				if (file == "" || !System::IO::File::Exists(file))
					throw new MissingFileException(file);
			}

			void Util::ensureExistsIfNeeded(const QString &file)
			{
				if (!file.empty())
					ensureExists(file);
			}

			QString Util::ToString(Nullable<TimeSpan*> t1)
			{
				if (!t1.HasValue)
					return 0;
				TimeSpan *t = t1.Value;
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			QString Util::ToString(Nullable<decimal> d, bool bNoDecimalPlaces)
			{
				if (!d.HasValue)
					return 0;
				if (bNoDecimalPlaces)
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ template equivalent to generic constraints:
template<typename T> where T : class
			QString Util::ToStringOrNull(T t)
			{
				if (t == 0)
					return 0;
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			int Util::CountStrings(const QString &src, wchar_t find)
			{
				int ret = 0;
				for (QString::const_iterator s = src.begin(); s != src.end(); ++s)
				{
					if (*s == find)
					{
						++ret;
...
template<typename T> where T : IIDable
			T Util::ByID(IEnumerable<T> *i, const QString &id)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ template equivalent to generic constraints:
template<typename T> where T : class
			QVector<T> Util::Unique(QVector<T> &l, Delegate<bool, T, T> cmp)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ template equivalent to generic constraints:
template<typename T> where T : class
			QVector<T> Util::Unique(QVector<T> &l)
			{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
...
template<typename T> where T : class, IIDable
			QVector<T> Util::UniqueByIDs(QVector<T> &l)
			{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				return Unique(l, delegate(T t1, T t2)
				{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ template equivalent to generic constraints:
template<typename T> where T : class, IIDable
			QVector<T> Util::RemoveDuds(QVector<T> &ps)
			{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ template equivalent to generic constraints:
template<typename From, typename To> where To : class
			QVector<To> Util::CastAll(QVector<From> &i)
			{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
...
template<typename T>
			...
			{
				return (QVector<T>(i))->ToArray();
			}

			object *Util::ToArray(IEnumerable *i)
			{
				QVector<object*> l = QVector<object*>();
...
//C# TO C++ CONVERTER TODO TASK: C++ does not have an equivalent to the C# 'yield' keyword:
			...
			quint64 Util::clamp(quint64 val, quint64 min, quint64 max)
			{
				return static_cast<quint64>(clamp(static_cast<decimal>(val), min, max));
			}

			qint64 Util::clamp(qint64 val, qint64 min, qint64 max)
			{
				return static_cast<qint64>(clamp(static_cast<decimal>(val), min, max));
			}

			quint64 Util::clamp(qint64 val, quint64 min, quint64 max)
			{
				return static_cast<quint64>(clamp(static_cast<decimal>(val), min, max));
			}

			quint64 Util::clampPositive(qint64 val)
			{
				if (val < 0)
					return 0u;
			...
			{
				return static_cast<unsigned int>(clampPositive(static_cast<qint64>(val)));
			}
...
template<typename T>
			...
			QString Util::converFrameNumberToTimecode(int frameNumber, double framerate)
			{
				double millisecondsPerFrame = static_cast<double>(1000 / framerate);
			...
			int Util::getTimeCode(const QString &timecode)
			{
				if (timecode.Equals(""))
					return -1;
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
					QString *subItems = timecode.Split(separator);
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
							QString *str = subItems[2].Split(separator);
