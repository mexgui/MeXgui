...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	FileBarEvent::FileBarEvent(const QString &oldName, const QString &newName) : QEvent(), OldFileName(oldName), NewFileName(newName)
	{
	}
	...
	const QString &FileBar::getTitle() const
	{
		return title;
	}

	void FileBar::setTitle(const QString &value)
	{
		title = value;
	}

	const QString &FileBar::getFilename() const
	{
		return filename->Text;
	}

	void FileBar::setFilename(const QString &value)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	const QString &FileBar::getFilter() const
	{
		return filter;
	}

	void FileBar::setFilter(const QString &value)
	{
		filter = value;
	}
	...
	void FileBar::openButton_Click(QObject *sender, QEvent *e)
	{
		if (folderMode)
		{
	...
	void FileBar::setFilename(const QString &filename)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void FileBar::triggerEvent()
	{
		if (raiseEvent->getReady())
			FileSelected(this, new FileBarQEvent(oldName, filename->Text));
		oldName = filename->Text;
	}

	void FileBar::filename_TextChanged(QObject *sender, QEvent *e)
	{
