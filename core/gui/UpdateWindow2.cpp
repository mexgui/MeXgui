...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	const QString &UpdateWindow::iUpgradeable::getSavePath() const
	{
		return savePath;
	}

	void UpdateWindow::iUpgradeable::setSavePath(const QString &value)
	{
		savePath = value;
	}

	const QString &UpdateWindow::iUpgradeable::getSaveFolder() const
	{
		return saveFolder;
	}

	void UpdateWindow::iUpgradeable::setSaveFolder(const QString &value)
	{
		saveFolder = value;
	}
	...
	const QString &UpdateWindow::iUpgradeable::getName() const
	{
		return this->name;
	}

	void UpdateWindow::iUpgradeable::setName(const QString &value)
	{
		this->name = value;
	}

	const QString &UpdateWindow::iUpgradeable::getDisplayName() const
	{
		return this->displayName;
	}

	void UpdateWindow::iUpgradeable::setDisplayName(const QString &value)
	{
	...
	const QString &UpdateWindow::iUpgradeable::getRequiredNET() const
	{
		return requiredNET;
	}

	void UpdateWindow::iUpgradeable::setRequiredNET(const QString &value)
	{
		requiredNET = value;
	}

	const QString &UpdateWindow::iUpgradeable::getTreeViewID() const
	{
		return this->treeViewID;
	}

	void UpdateWindow::iUpgradeable::setTreeViewID(const QString &value)
	{
...
//	}
	...
	UpdateWindow::AviSynthFile::AviSynthFile(const QString &treeviewid, const QString &name)
	{
		this->setName(name);
		this->setAllowUpdate(true);
		this->treeViewID = treeviewid;
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void UpdateWindow::ProgramFile::init()
	{
		...
		{
			QString extension = Path::GetExtension(getSavePath());
	...
	UpdateWindow::ProgramFile::ProgramFile(const QString &treeViewID, const QString &name)
	{
		this->setName(name);
		this->setAllowUpdate(true);
		this->treeViewID = treeViewID;
...
//ORIGINAL LINE: case ("neroaacenc"):
	...
	UpdateWindow::Version::Version(const QString &version, const QString &url)
	{
		this->fileVersion = version;
		this->url = url;
	}

	const QString &UpdateWindow::Version::getFileVersion() const
	{
		return fileVersion;
	}

	void UpdateWindow::Version::setFileVersion(const QString &value)
	{
		fileVersion = value;
	}

	const QString &UpdateWindow::Version::getUrl() const
	{
		return url;
	}

	void UpdateWindow::Version::setUrl(const QString &value)
	{
		url = value;
	}
	...
	const QString &UpdateWindow::Version::getWeb() const
	{
		return web;
	}

	void UpdateWindow::Version::setWeb(const QString &value)
	{
		web = value;
	}
...
MeXguiSettings *UpdateWindow::MeXguiSettings = 0;
	...
	void UpdateWindow::AddTextToLog(const QString &text, ImageType oLogType)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void UpdateWindow::UpdateWindow_FormClosing(QObject *sender, FormClosingQEvent *e)
	{
		SaveSettings();
	}
	...
	QVector<QString> UpdateWindow::getUpdateServerList(QString& serverList[])
	{
		QString lastUpdateServer = MainForm::Instance->getSettings()->getLastUpdateServer();
		QVector<QString> randomServerList = QVector<QString>();
		QVector<QString> sortedServerList = QVector<QString>(serverList);
	...
	void UpdateWindow::UpdateWindow_Load(QObject *sender, QEvent *e)
	{
	...
	{
		QString path = Path::Combine(Application::StartupPath, "AutoUpdate.xml");
		if (File::Exists(path))
		{
			try
			{
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	void UpdateWindow::ProcessUpdateXML()
	{
		...
		for (QVector<QString>::const_iterator serverName = serverList.begin(); serverName != serverList.end(); ++serverName)
		{
		...
		if (iUpdatesCount > 1)
			AddTextToLog(QString::Format("There are {0} files which can be updated.", iUpdatesCount), Information);
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	bool UpdateWindow::FindFileInUpdateXML(XmlNodeList *xnList, const QString &strFile)
	{
	...
	void UpdateWindow::ParseUpgradeXml(XmlNode *currentNode, XmlNode *groupNode, const QString &path)
	{
		for (unknown::const_iterator childnode = currentNode->ChildNodes.begin(); childnode != currentNode->ChildNodes.end(); ++childnode)
		{
			...
			QString newPath = path + "." + (*childnode)->Name;
	...
	void UpdateWindow::ParseFileData(XmlNode *node, XmlNode *groupNode, const QString &treePath)
	{
		iUpgradeable *file = 0;
...
//C# TO C++ CONVERTER TODO TASK: There is no equivalent to implicit typing in C++ unless the C++0x inferred typing option is selected:
	...
	void UpdateWindow::listViewDetails_ItemCheck(QObject *sender, ItemCheckQEvent *e)
	{
	...
	void UpdateWindow::listViewDetails_MouseClick(QObject *sender, MouseQEvent *e)
	{
	...
	void UpdateWindow::setIgnoreValue_Click(QObject *sender, QEvent *e)
	{
	...
	void UpdateWindow::btnUpdate_Click(QObject *sender, QEvent *e)
	{
	...
	void UpdateWindow::InstallFiles(SortedDictionary<unsigned int, QVector<iUpgradeable*>*> *groups)
	{
		continueUpdate = true;
		int currentFile = 1; //the first file we update is file 1.
		ErrorState result;
		QVector<iUpgradeable*> succeededFiles = QVector<iUpgradeable*>();
		QVector<iUpgradeable*> failedFiles = QVector<iUpgradeable*>();
		QVector<iUpgradeable*> missingFiles = QVector<iUpgradeable*>();

		// Count the number of files we can update before we restart
		int updateableFileCount = 0;
		bool needsRestart = false;
		for (SortedDictionary<unsigned int, QVector<iUpgradeable*>*>::const_iterator group = groups->begin(); group != groups->end(); ++group)
			updateableFileCount += (*group).Count;

		// Now update the files we can
		for (SortedDictionary<unsigned int, QVector<iUpgradeable*>*>::const_iterator group = groups->begin(); group != groups->end(); ++group)
		{
			for (QVector::const_iterator file = group.begin(); file != group.end(); ++file)
			{
				if (!continueUpdate)
				{
				...
				AddTextToLog(QString::Format("Updating {0}. File {1}/{2}.", (*file)->getName(), currentFile, updateableFileCount), Information);
				...
				else
				{
					...
					if (result != Successful)
					{
						failedFiles.push_back(*file);
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
						AddTextToLog(QString::Format("Failed to download file {0} with error: {1}.", (*file)->getName(), EnumProxy::Create(result)->ToString()), Error);
					}
					else
					{
						try
						{
							...
							if (state != Successful)
							{
								...
								{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
									AddTextToLog(QString::Format("Failed to install file {0} with error: {1}.", (*file)->getName(), EnumProxy::Create(state)->ToString()), Error);
									failedFiles.push_back(*file);
								}
								else
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	MeXgui::UpdateWindow::ErrorState UpdateWindow::Install(iUpgradeable *file, Stream *fileData)
	{
		...
		{
			AddTextToLog(QString::Format("Could not install module '{0}' as .NET {1} is required.", file->getName(), file->getRequiredNET()), Warning);
			return RequirementNotMet;
		}
		...
		AddTextToLog(QString::Format("Could not install module '{0}'.", file->getName()), Error);
		return state;
	}
	...
	MeXgui::UpdateWindow::ErrorState UpdateWindow::SaveNewFile(iUpgradeable *file, Stream *data)
	{
		...
		{
			AddTextToLog(QString::Format("Could not create directory {0}.", filepath), Error);
			return CouldNotSaveNewFile;
		}
