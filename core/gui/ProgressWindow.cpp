...
namespace MeXgui
{
	...
	void ProgressWindow::OnClosing(CancelQEvent *e)
	{
	...
	void ProgressWindow::UpdateStatus(StatusUpdate *su)
	{
		try
		{
			...
			// Current position
			QString tempVar = Util.ToString(su.getClipPosition());
			QString tempVar2 = Util.ToString(su.getClipLength());
			positionInClip->Text = ((tempVar != 0) ? tempVar : "---") + " / " + ((tempVar2 != 0) ? tempVar2 : "---");

			// Current frame
			QString tempVar3 = Util.ToString(su.getNbFramesDone(), true);
			QString tempVar4 = Util.ToString(su.getNbFramesTotal(), true);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			// Processing speed
			QString tempVar5 = su.getProcessingSpeed();
			fps->Text = (tempVar5 != 0) ? tempVar5 : "---";

			// Time elapsed 
			// Now we use TotalHours to avoid 24h+ resets...
			if (su->getTimeElapsed()->TotalHours > 24)
				timeElapsed->Text = QString::Format("{0:00}:{1:00}:{2:00}:{3:00}", static_cast<int>(su->getTimeElapsed()->TotalDays), su->getTimeElapsed()->Hours, su->getTimeElapsed()->Minutes, su->getTimeElapsed()->Seconds);
			else
				timeElapsed->Text = QString::Format("{0:00}:{1:00}:{2:00}", static_cast<int>(su->getTimeElapsed()->Hours), su->getTimeElapsed()->Minutes, su->getTimeElapsed()->Seconds);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			if ((Environment::OSVersion::Version->Major == 6 && Environment::OSVersion::Version::Minor >= 1) || Environment::OSVersion::Version::Major > 6)
				taskbarProgress->SetProgressValue(this->Handle, static_cast<quint64>(su->getPercentageDone()), 100);
		}
	...
	QString ProgressWindow::getTimeString(TimeSpan *span, decimal percentageDone)
	{
		if (percentageDone == 0)
			return "---";
		else
		{
			qint64 ratio = static_cast<qint64>(static_cast<decimal>(span->Ticks) / percentageDone * 100);
			TimeSpan *t = new TimeSpan(ratio - span->Ticks);
			QString retval = "";
			if (t->TotalHours > 24)
			{
				retval += QString::Format("{0:00}:{1:00}:{2:00}:{3:00}", static_cast<int>(t->TotalDays), t->Hours, t->Minutes, t->Seconds);
			}
			else
			{
				retval += QString::Format("{0:00}:{1:00}:{2:00}", static_cast<int>(t->Hours), t->Minutes, t->Seconds);
			}
			return retval;
		}
	}
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	void ProgressWindow::abortButton_Click(QObject *sender, System::QEvent *e)
	{
		Abort();
	}

	void ProgressWindow::priority_SelectedIndexChanged(QObject *sender, System::QEvent *e)
	{
