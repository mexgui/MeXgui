...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			const QString &JobWorker::getStatusString() const
			{
				if (status == Idle)
					return "idle";
				if (status == Stopped)
				...
				QString _status = "running";
				if (currentJob != 0)
					_status += QString::Format(" {0} ({1:P2})", currentJob->getName(), progress / 100);
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				jobQueue1->AddMenuItem("Return to main job queue", 0, delegate(QVector<TaggedJob*> jobs)
				{
			...
			{
				QVector<TaggedJob*> list = QVector<TaggedJob*>(localJobs.Values);
				for (QVector<TaggedJob*>::const_iterator j = list.begin(); j != list.end(); ++j)
					mainForm->getJobs()->ReleaseJob(*j);
			}
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
															new Thread(delegate()
															{
																WorkerFinishedJobs(this, QEvent::Empty);
															}
														   ).Start();
															break;
									}
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			{
				for (std::QMap<QString, TaggedJob*>::const_iterator j = localJobs.begin(); j != localJobs.end(); ++j)
			...
			bool JobWorker::startEncoding(TaggedJob *job)
			{
				try
				{
					log = mainForm->getLog()->Info(QString::Format("Log for {0} ({1}, {2} -> {3})", job->getName(), job->getJob()->EncodingMode, job->getInputFileName(), job->getOutputFileName()));
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
			...
			void JobWorker::OnClosing(CancelQEvent *e)
			{
				if (!alive)
				{
			...
			void JobWorker::UserRequestedRename()
			{
				try
				{
					QString name = InputBox::Show("Please enter the new name for this worker", "Please enter the new name", "New Worker Name");
					if (name != "")
						setName(name);
				}
			...
			void JobWorker::changeNameToolStripMenuItem_Click(QObject *sender, QEvent *e)
			{
				UserRequestedRename();
			}

			void JobWorker::setName(const QString &p)
			{
				mainForm->getJobs()->RenameWorker(this->name, p); // throws NameTakenException if it fails
			}

			void JobWorker::shutDownWhenFinishedLocalQueueToolStripMenuItem_Click(QObject *sender, QEvent *e)
			{
				ShutDownWhenFinished();
			}

			void JobWorker::shutDownWorkerNowToolStripMenuItem_Click(QObject *sender, QEvent *e)
			{
				UserRequestShutDown();
			}

			void JobWorker::showProgressWindowToolStripMenuItem_Click(QObject *sender, QEvent *e)
			{
			...
			void JobWorker::jobQueue1_AbortClicked(QObject *sender, QEvent *e)
			{
				UserRequestedAbort();
			}

			void JobWorker::jobQueue1_StartClicked(QObject *sender, QEvent *e)
			{
			...
			void JobWorker::jobQueue1_StopClicked(QObject *sender, QEvent *e)
			{
			...
			void JobWorker::progressWindowToolStripMenuItem_DropDownOpened(QObject *sender, QEvent *e)
			{
			...
			void JobWorker::JobWorker_FormClosed(QObject *sender, FormClosedQEvent *e)
			{
			}
			...
			{
				localJobs = std::QMap<QString, TaggedJob*>();
				alive = true;
				delete components;
			}
		}
