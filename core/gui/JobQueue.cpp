...
namespace MeXgui
{
	namespace core
	{
		namespace gui
		{
			...
			JobQueueQEvent::JobQueueEvent(Job *j)
			{
				this->job = j;
			}
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				...
				for (std::QMap<QString, TaggedJob*>::const_iterator j = jobs.begin(); j != jobs.end(); ++j)
				{
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				...
				{
					queueListView->Items->Add(new ListViewItem(new QString[] {(*job)->getName(), "", "", "", "", "", "", "", "", ""}));
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				...
				queueListView->Items->Add(new ListViewItem(new QString[] {j->getName(), "", "", "", "", "", "", "", "", ""}));
				jobs[j->getName()] = j;
				refreshQueue();
			}
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			void JobQueue::addItem(ToolStripMenuItem *item, const QString &parent)
			{
				if (InvokeRequired)
				{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			void JobQueue::AddDynamicSubMenu(const QString &name, const QString &parent, MultiJobMenuGenerator gen)
			{
			...
			void JobQueue::AddMenuItem(const QString &name, const QString &parent)
			{
				addItem(new ToolStripMenuItem(name), parent);
			}

			void JobQueue::AddMenuItem(const QString &name, const QString &parent, SingleJobHandler handler)
			{
				ToolStripMenuItem *item = new ToolStripMenuItem(name);
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				item->Click += delegate(object sender, QEvent e)
				{
			...
			void JobQueue::AddMenuItem(const QString &name, const QString &parent, MultiJobHandler handler)
			{
				ToolStripMenuItem *item = new ToolStripMenuItem(name);
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				item->Click += delegate(object sender, QEvent e)
				{
					...
					QVector<TaggedJob*> list = QVector<TaggedJob*>();
			...
			void JobQueue::AddButton(const QString &name, EventHandler *handler)
			{
				if (InvokeRequired)
				{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			QVector<TaggedJob*> JobQueue::removeAllDependantJobsFromQueue(TaggedJob *job)
			{
				removeJobFromQueue(job);
				QVector<TaggedJob*> list = QVector<TaggedJob*>();
			...
			void JobQueue::deleteJobButton_Click(QObject *sender, QEvent *e)
			{
			...
			void JobQueue::processUserRequestToDelete(const QString &name)
			{
			...
			void JobQueue::downButton_Click(QObject *sender, QEvent *e)
			{
				MoveListViewItem(Down);
			}

			void JobQueue::upButton_Click(QObject *sender, QEvent *e)
			{
				MoveListViewItem(Up);
			}
			...
			void JobQueue::swapContents(ListViewItem *a, ListViewItem *b)
			{
				...
				{
					QString cache = b->SubItems[i]->Text;
			...
			void JobQueue::queueListView_ItemSelectionChanged(QObject *sender, ListViewItemSelectionChangedQEvent *e)
			{
			...
			void JobQueue::editJobButton_Click(QObject *sender, QEvent *e)
			{
			...
			void JobQueue::queueListView_DoubleClick(QObject *sender, QEvent *e)
			{
			...
			void JobQueue::postponeMenuItem_Click(QObject *sender, QEvent *e)
			{
				for (unknown::const_iterator item = this->queueListView->SelectedItems.begin(); item != this->queueListView->SelectedItems.end(); ++item)
				{
			...
			void JobQueue::waitingMenuItem_Click(QObject *sender, QEvent *e)
			{
				for (unknown::const_iterator item = this->queueListView->SelectedItems.begin(); item != this->queueListView->SelectedItems.end(); ++item)
				{
					int position = (*item)->Index;
			...
			void JobQueue::queueContextMenu_Opened(QObject *sender, QEvent *e)
			{
				int count = queueListView->SelectedItems->Count;
				for (QVector<ToolStripItem*>::const_iterator i = multiJobHandlers.begin(); i != multiJobHandlers.end(); ++i)
					(*i)->Enabled = (count > 0);
				for (QVector<ToolStripItem*>::const_iterator i = singleJobHandlers.begin(); i != singleJobHandlers.end(); ++i)
					(*i)->Enabled = (count == 1);

				// here we generate our submenus as required, giving them the event handlers
				if (count > 0)
				{
					for (QVector<Pair<ToolStripMenuItem*, MultiJobMenuGenerator>*>::const_iterator p = menuGenerators.begin(); p != menuGenerators.end(); ++p)
					{
						...
						for (unknown::const_iterator pair = p->snd().begin(); pair != p->snd().end(); ++pair)
						{
							...
							(*i)->Tag = (*pair)->snd;
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
							(*i)->Click += delegate(object sender1, QEvent e2)
							{
								assert(queueListView->SelectedItems->Count > 0);
								QVector<TaggedJob*> list = QVector<TaggedJob*>();
			...
			void JobQueue::abortButton_Click(QObject *sender, QEvent *e)
			{
				AbortClicked(this, e);
			}

			void JobQueue::startStopButton_Click(QObject *sender, QEvent *e)
			{
			...
			void JobQueue::pauseButton_Click(QObject *sender, QEvent *e)
			{
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				...
				for (unknown::const_iterator item = queueListView->Items.begin(); item != queueListView->Items.end(); ++item)
				{
					...
					QString tempVar = job.getOwningWorker();
			...
			void JobQueue::stopButton_Click(QObject *sender, QEvent *e)
			{
				StopClicked(this, e);
			}
			...
			void JobQueue::queueListView_VisibleChanged(QObject *sender, QEvent *e)
			{
				refreshQueue();
			}

			void JobQueue::AbortMenuItem_Click(QObject *sender, QEvent *e)
			{
			...
			void JobQueue::queueListView_KeyDown(QObject *sender, KeyQEvent *e)
			{
			...
			void JobQueue::JobQueue_Load(QObject *sender, QEvent *e)
			{
			...
			void JobQueue::queueListView_ColumnWidthChanged(QObject *sender, ColumnWidthChangedQEvent *e)
			{
			...
			{
				QVector<QString> oList = QVector<QString>();
			...
			void JobQueue::getAllDependantJobs(TaggedJob *job, QVector<QString> &oList)
			{
			...
			void JobQueue::outputFolderToolStripMenuItem_Click(QObject *sender, QEvent *e)
			{
				for (unknown::const_iterator item = this->queueListView->SelectedItems.begin(); item != this->queueListView->SelectedItems.end(); ++item)
				{
					TaggedJob *job = jobs[(*item)->Text];
			...
			void JobQueue::outputFileToolStripMenuItem_Click(QObject *sender, QEvent *e)
			{
				for (unknown::const_iterator item = this->queueListView->SelectedItems.begin(); item != this->queueListView->SelectedItems.end(); ++item)
				{
					TaggedJob *job = jobs[(*item)->Text];
			...
			void JobQueue::inputFolderToolStripMenuItem_Click(QObject *sender, QEvent *e)
			{
				for (unknown::const_iterator item = this->queueListView->SelectedItems.begin(); item != this->queueListView->SelectedItems.end(); ++item)
				{
					TaggedJob *job = jobs[(*item)->Text];
			...
			void JobQueue::inputFileToolStripMenuItem_Click(QObject *sender, QEvent *e)
			{
				for (unknown::const_iterator item = this->queueListView->SelectedItems.begin(); item != this->queueListView->SelectedItems.end(); ++item)
				{
					TaggedJob *job = jobs[(*item)->Text];
			...
			{
				jobs = std::QMap<QString, TaggedJob*>();
				singleJobHandlers = QVector<ToolStripItem*>();
				multiJobHandlers = QVector<ToolStripItem*>();
				menuGenerators = QVector<Pair<ToolStripMenuItem*, MultiJobMenuGenerator>*>();
				delete components;
			}
		}
	}
