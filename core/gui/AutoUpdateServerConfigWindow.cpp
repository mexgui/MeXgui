...
namespace MeGUI
{
	namespace core
	{
		namespace gui
		{
			...
			void AutoUpdateServerConfigWindow::addServerButton_Click(QObject *sender, QEvent *e)
			{
				QString serverName = InputBox::Show("Please enter the server address", "Please enter the server address", "http://yourserver.org/path/to/update/folder/");
				if (serverName == "")
					return;
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
			...
			void AutoUpdateServerConfigWindow::removeSelectedServersButton_Click(QObject *sender, QEvent *e)
			{
			...
			const QString &*AutoUpdateServerConfigWindow::getServerList() const
			{
				subList_SelectedIndexChanged(0, 0);
				return serverLists;
			}

			void AutoUpdateServerConfigWindow::setServerList(const QString &value[][])
			{
				serverLists = value;
				oldIndex = -1;
				subList->Items->Clear();

				for (QString[][]::const_iterator sub = value->begin(); sub != value->end(); ++sub)
					subList->Items->Add(*sub[0]);

				subList_SelectedIndexChanged(0, 0);
			}

			const QString &AutoUpdateServerConfigWindow::getlittleServerList() const
			{
			...
			void AutoUpdateServerConfigWindow::setlittleServerList(const QString &value[])
			{
				oldTitle = value[0];
			...
			void AutoUpdateServerConfigWindow::subList_SelectedIndexChanged(QObject *sender, QEvent *e)
			{
				if (oldIndex > -1)
