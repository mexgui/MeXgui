...
CONFIG_HEADER = $(top_builddir)/auxial/header.h
CONFIG_CLEAN_FILES =
...
depcomp = $(SHELL) $(top_srcdir)/auxial/depcomp
...
SOURCES = $(ligui_SOURCES)
DIST_SOURCES = $(ligui_SOURCES)
ETAGS = etags
CTAGS = ctags
...
auxial/header.h: auxial/stamp-h1
	@if test ! -f $@; then rm -f auxial/stamp-h1; else :; fi
	@if test ! -f $@; then $(MAKE) $(AM_MAKEFLAGS) auxial/stamp-h1; else :; fi

auxial/stamp-h1: $(top_srcdir)/auxial/header.h.in $(top_builddir)/config.status
	@rm -f auxial/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status auxial/header.h
$(top_srcdir)/auxial/header.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f auxial/stamp-h1
	touch $@

distclean-hdr:
	-rm -f auxial/header.h auxial/stamp-h1
...
ligui$(EXEEXT): $(ligui_OBJECTS) $(ligui_DEPENDENCIES) $(EXTRA_ligui_DEPENDENCIES) 
	@rm -f ligui$(EXEEXT)
	$(LINK) $(ligui_OBJECTS) $(ligui_LDADD) $(LIBS)
