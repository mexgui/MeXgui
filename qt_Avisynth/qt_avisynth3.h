...
struct VideoInfo {
  ...
  void MulDivFPS(unsigned multiplier, unsigned divisor) {
    unsigned qint64 numerator   = UInt32x32To64(fps_numerator,   multiplier);
    unsigned qint64 denominator = UInt32x32To64(fps_denominator, divisor);

    unsigned qint64 x=numerator, y=denominator;
	while (y) {   // find gcd
      unsigned qint64 t = x%y; x = y; y = t;
	}
	numerator   /= x; // normalize
	denominator /= x;

    unsigned qint64 temp = numerator | denominator; // Just looking top bit
	unsigned u = 0;
