...
namespace MeGUI
{
	...
	SourceDetector::SourceDetector(const std::string &avsScript, const std::string &d2vFile_Renamed, bool isAnime, SourceDetectorSettings *settings, UpdateSourceDetectionStatus updateMethod, FinishedAnalysis finishedMethod)
	{
	...
	std::string SourceDetector::findPortions(std::vector<int[]> &portions, int selectEvery, int selectLength, int numPortions, int sectionCount, int inputFrames, const std::string &type, std::string &trimLine, int &frameCount)
	{
		frameCount = 0;
		trimLine = "";
		trimmedFilteredLine = "";
		std::string outputText = std::string::Format("There are {0} {1} portions.\r\n", numPortions, type);
		...
		for (int i = 0; i < numPortions; i++)
		{
			...
			trimLine += std::string::Format("trim({0},{1}) ++ ", startFrame, endFrame);
			outputText += std::string::Format("Portion number {0} goes from frame {1} to frame {2}.\r\n", i + 1, startFrame, endFrame);
			trimmedFilteredLine += std::string::Format("original.trim({0},{1}) ++ deintted.trim({2},{3}) ++ ", lastEndFrame + 1, startFrame - 1, startFrame, endFrame);
			lastEndFrame = endFrame;
		}
		if (lastEndFrame < inputFrames - 1)
			trimmedFilteredLine += std::string::Format("original.trim({0},{1})", lastEndFrame + 1, inputFrames);
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'TrimEnd' method:
	...
	std::string SourceDetector::getLogFileName(const std::string &logFileName)
	{
		return Path::Combine(Path::GetTempPath(), logFileName);
	}

	void SourceDetector::Process(const std::string &scriptBlock)
	{
		try
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void SourceDetector::InitializeInstanceFields()
	{
		...
		bffCount = 0;
		fieldOrder = UNKNOWN;
		filters = std::vector<DeinterlaceFilter*>();
	}

	void <missing_class_definition>::runScript(int scriptType, int frameCount, const std::string &trimLine)
	{
		int numFrames = 0;
		try
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		std::string logFileName = getLogFileName((scriptType == 1) ? "ff_interlace-" + Guid::NewGuid()->ToString("N") + ".log" : "interlace-" + Guid::NewGuid()->ToString("N") + ".log");

		if (File::Exists(logFileName))
			File::Delete(logFileName);

		std::string resultScript = ScriptServer::getScript(scriptType, script, trimLine, logFileName, selectEvery, selectLength);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void <missing_class_definition>::analyseFF(const std::string &filename)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
				std::string *contents = line.Split(new wchar_t[] {'-'});
				try
				{
	...
	void <missing_class_definition>::analyse(const std::string &logFileName, int selectEvery, int selectLength, int inputFrames)
	{
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
		std::vector<int[]> *portions = new std::vector<int[]>[2];
		portions[0] = std::vector<int[]>();
		portions[1] = std::vector<int[]>();
		...
		else
		{
			...
			else if (array_Renamed[0] == numInt)
			{
				...
				else
				{
					...
					int frameCount = -1;
					std::string trimLine = "#no trimming";
					std::string textLines = "The number of portions is " + numPortions[1] + ".\r\n";
			...
			else if (array_Renamed[0] == numTC)
			{
				...
				else
				{
					...
					int frameCount = -1;
					std::string trimLine = "#no trimming";
					std::string textLines = "The number of portions is " + numPortions[0] + ".\r\n";
