...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	bool VideoPlayer::loadVideo(MainForm *mainForm, const std::string &path, PREVIEWTYPE type, bool hasAR)
	{
		return loadVideo(mainForm, path, type, hasAR, false, -1, false);
	}

	bool VideoPlayer::loadVideo(MainForm *mainForm, const std::string &path, PREVIEWTYPE type, bool hasAR, bool inlineAvs, int startFrame, bool originalSize)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
	...
	void VideoPlayer::originalSizeButton_Click(object *sender, EventArgs *e)
	{
		bOriginalSize = true;
	...
	void VideoPlayer::btnFitScreen_Click(object *sender, EventArgs *e)
	{
		bOriginalSize = false;
	...
	void VideoPlayer::zoomInButton_Click(object *sender, EventArgs *e)
	{
		if (zoomFactor > 100)
			return;
	...
	void VideoPlayer::zoomOutButton_Click(object *sender, EventArgs *e)
	{
		if (zoomFactor > 100)
		{
			btnFitScreen_Click(0, 0);
	...
	void VideoPlayer::formResized(object *sender, EventArgs *e)
	{
		if (!sizeLock)
		{
	...
	void VideoPlayer::OnClosing(CancelEventArgs *e)
	{
	...
	void VideoPlayer::positionSlider_Scroll(object *sender, System::EventArgs *e)
	{
	...
	void VideoPlayer::playButton_Click(object *sender, System::EventArgs *e)
	{
	...
	void VideoPlayer::previousFrameButton_Click(object *sender, System::EventArgs *e)
	{
		videoPreview->OffsetPosition(-1);
	}

	void VideoPlayer::nextFrameButton_Click(object *sender, System::EventArgs *e)
	{
		videoPreview->OffsetPosition(1);
	}

	void VideoPlayer::fwdButton_Click(object *sender, System::EventArgs *e)
	{
		videoPreview->OffsetPosition(-25);
	}

	void VideoPlayer::ffButton_Click(object *sender, System::EventArgs *e)
	{
		videoPreview->OffsetPosition(25);
	}

	void VideoPlayer::creditsStartButton_Click(object *sender, System::EventArgs *e)
	{
		IntroCreditsFrameSet(getCurrentFrame(), true);
	}

	void VideoPlayer::introEndButton_Click(object *sender, System::EventArgs *e)
	{
		IntroCreditsFrameSet(getCurrentFrame(), false);
	}

	void VideoPlayer::zoneEndButton_Click(object *sender, System::EventArgs *e)
	{
	...
	void VideoPlayer::zoneStartButton_Click(object *sender, System::EventArgs *e)
	{
		int pos = getCurrentFrame();
	...
	void VideoPlayer::setZoneButton_Click(object *sender, System::EventArgs *e)
	{
		if (ZoneSet != 0)
		{
	...
	void VideoPlayer::chapterButton_Click(object *sender, System::EventArgs *e)
	{
		ChapterSet(getCurrentFrame());
	}

	void VideoPlayer::mnuIntroEnd_Click(object *sender, System::EventArgs *e)
	{
		setCurrentFrame(introEndFrame);

	}

	void VideoPlayer::mnuCreditsStart_Click(object *sender, System::EventArgs *e)
	{
		this->positionSlider->Value = this->creditsStartFrame;
		positionSlider_Scroll(0, 0);
	}

	void VideoPlayer::mnuZoneStart_Click(object *sender, System::EventArgs *e)
	{
		 setCurrentFrame(creditsStartFrame);
	}

	void VideoPlayer::mnuZoneEnd_Click(object *sender, System::EventArgs *e)
	{
		setCurrentFrame(zoneEnd);
	}

	void VideoPlayer::contextMenu1_Popup(object *sender, System::EventArgs *e)
	{
		if (this->introEndFrame > -1)
	...
	void VideoPlayer::showPAR_CheckedChanged(object *sender, EventArgs *e)
	{
		SetMaxZoomWidth();
		resize(videoWindowWidth, showPAR->Checked);
	}

	void VideoPlayer::goToFrameButton_Click(object *sender, EventArgs *e)
	{
		decimal val;
		bool bTopMost = this->TopMost;
		this->TopMost = false;
		if (NumberChooser::ShowDialog("Enter a frame number:", "Go to frame", 0, positionSlider->Minimum, positionSlider->Maximum, positionSlider->Value, val) == System::Windows::Forms::DialogResult::OK)
		{
	...
	void VideoPlayer::arChooser_SelectionChanged(object *sender, const std::string &val)
	{
		SetMaxZoomWidth();
		resize(videoWindowWidth, showPAR->Checked);
	}

	void VideoPlayer::nextFrameButton_KeyDown(object *sender, KeyEventArgs *e)
	{
		if (e->KeyCode == Keys::Right)
			videoPreview->OffsetPosition(1);
	}

	void VideoPlayer::previousFrameButton_KeyDown(object *sender, KeyEventArgs *e)
	{
		if (e->KeyCode == Keys::Right)
			videoPreview->OffsetPosition(-1);
	}

	void VideoPlayer::videoPreview_PositionChanged(object *sender, EventArgs *e)
	{
		positionSlider->Value = videoPreview->getPosition();
		setTitleText();
	}

	void VideoPlayer::btnReloadVideo_Click(object *sender, EventArgs *e)
	{
	...
	void VideoPlayer::VideoPlayer_Shown(object *sender, EventArgs *e)
	{
		Size *oSizeScreen = Screen::GetWorkingArea(this)->Size;
