...
namespace MeGUI
{
	...
	void UpdateCacher::flushOldCachedFilesAsync(std::vector<std::string> &urls, UpdateWindow *oUpdate)
	{
		std::string updateCache = MainForm::Instance->getSettings()->getMeGUIUpdateCache();
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
	...
	UpdateWindow::ErrorState UpdateCacher::DownloadFile(const std::string &url, Uri *serverAddress, Stream *&str, DownloadProgressChangedEventHandler *wc_DownloadProgressChanged, UpdateWindow *oUpdate)
	{
		ensureSensibleCacheFolderExists();
		UpdateWindow::ErrorState er = UpdateWindow::Successful;
		std::string updateCache = MainForm::Instance->getSettings()->getMeGUIUpdateCache();
		std::string localFilename = Path::Combine(updateCache, url);
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	void UpdateCacher::FlushFile(const std::string &p, UpdateWindow *oUpdate)
	{
		std::string localFilename = Path::Combine(MainForm::Instance->getSettings()->getMeGUIUpdateCache(), p);
		try
		{
