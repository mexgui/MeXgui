...
namespace MeGUI
{
	...
	public:
		...
		public:
			...
			void setSavePath(const std::string &value);
		private:
			std::string saveFolder;
		public:
			const std::string &getSaveFolder() const;
			void setSaveFolder(const std::string &value);
		...
		private:
			std::string requiredNET;
		public:
			const std::string &getRequiredNET() const;
			void setRequiredNET(const std::string &value);

			std::string treeViewID;
			const std::string &getTreeViewID() const;
			void setTreeViewID(const std::string &value);
	...
	public:
		...
		public:
			...
			iUpgradeable *FindByName(const std::string &name);
	...
	public:
		...
		public:
			...
			ProfilesFile(const std::string &treeviewid, const std::string &name, MeGUI::MainForm *mainForm);
	...
	public:
		...
		private:
			AviSynthFile();
		public:
			AviSynthFile(const std::string &treeviewid, const std::string &name);
	...
	public:
		...
		public:
			MeGUIFile(const std::string &treeViewID, const std::string &name);
	...
	public:
		...
		public:
			...
			ProgramFile(const std::string &treeViewID, const std::string &name); // Constructor

		private:
			const std::string &getMeGUIFilePath() const;
			void setMeGUIFilePath(const std::string &value);
	...
	public:
		...
		public:
			Version();
			Version(const std::string &version, const std::string &url);
		private:
...
//[EnumTitle("The requirements for this file are not met")]
	...
	private:
		...
		typedef void (*BeginParseUpgradeXml)(XmlNode *node, XmlNode *groupNode, const std::string &path);
...
//C# TO C++ CONVERTER WARNING: Unlike C#, there is no automatic call to this finalizer method in native C++:
	...
	public:
		void AddTextToLog(const std::string &text, ImageType oLogType);
	...
	private:
		std::vector<std::string> getUpdateServerList(std::string& serverList[]);
	...
	public:
		...
		/// </summary>
		ErrorState GetUpdateXML(const std::string &strServerAddress);
		/// <summary>
	...
	private:
		...
		bool FindFileInUpdateXML(XmlNodeList *xnList, const std::string &strFile);
		...
		void ParseUpgradeXml(XmlNode *currentNode, XmlNode *groupNode, const std::string &path);
		...
		void ParseFileData(XmlNode *node, XmlNode *groupNode, const std::string &treePath);
		void DisplayItems(bool bShowAllFiles);
		void listViewDetails_ItemCheck(object *sender, ItemCheckEventArgs *e);

		void listViewDetails_MouseClick(object *sender, MouseEventArgs *e);

		void setIgnoreValue_Click(object *sender, EventArgs *e);
	...
	private:
		...
		void InstallFiles(SortedDictionary<unsigned int, std::vector<iUpgradeable*>*> *groups);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	private:
		ErrorState manageBackups(std::string savePath, std::string name, bool bCopyFile)
		{
			try
			{
	...
	public:
		...
		void UpdateUploadDate(std::string name, std::string strDate)
		{
	...
	private:
		...
		void uncheckToolStripMenuItem_Click(object sender, EventArgs e)
		{
			ToolStripMenuItem *ts = static_cast<ToolStripMenuItem*>(sender);
			for (unknown::const_iterator item = listViewDetails::SelectedItems.begin(); item != listViewDetails::SelectedItems.end(); ++item)
				(*item)->Checked = false;
		...
		void reinstallToolStripMenuItem_Click(object sender, EventArgs e)
		{
			for (unknown::const_iterator item = listViewDetails::SelectedItems.begin(); item != listViewDetails::SelectedItems.end(); ++item)
			{
		...
		void chkShowAllFiles_CheckedChanged(object sender, EventArgs e)
		{
			DisplayItems(chkShowAllFiles->Checked);
		}

		void UpdateWindow_Move(object sender, EventArgs e)
		{
			if (this->WindowState != FormWindowState::Minimized && this->Visible == true)
				mainForm::Settings->UpdateFormLocation = this->Location;
		}

		void UpdateWindow_Resize(object sender, EventArgs e)
		{
			if (this->WindowState != FormWindowState::Minimized && this->Visible == true)
		...
		void listViewDetails_ColumnWidthChanged(object sender, ColumnWidthChangedEventArgs e)
		{
