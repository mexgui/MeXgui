...
namespace MeGUI
{
	namespace core
	{
		namespace gui
		{
			...
			void CountdownWindow::CountdownWindow_Load(object *sender, EventArgs *e)
			{
				//bool b = this.TopMost;
				this->Activate();
				this->Focus();
			...
			void CountdownWindow::SetButtonText(const std::string &text)
			{
				...
				{
					this->cancelButton->Text = std::string::Format("Cancel ({0})", text);
				}
			}

			void CountdownWindow::TimerTick(object *sender, System::EventArgs *e)
			{
				remain--;
				if (remain > 0)
				{
