...
namespace MeGUI
{
	...
	const std::string &MainForm::getLogFile() const
	{
		return strLogFile;
	}

	const Semaphore &MainForm::getLogLock() const
	{
		return logLock;
	}

	void MainForm::setLogLock(const Semaphore &value)
	{
		logLock = value;
	}
	...
	const MeGUI::core::util::LogItem &MainForm::getOneClickLog() const
	{
		return _oneClickLog;
	}
	...
	const MeGUI::core::util::LogItem &MainForm::getAVSScriptCreatorLog() const
	{
	...
	const MeGUI::core::util::LogItem &MainForm::getFileIndexerLog() const
	{
	...
	const MeGUI::core::util::LogItem &MainForm::getUpdateLog() const
	{
		return _updateLog;
	}
	...
	const MeGUI::core::util::LogItem &MainForm::getEac3toLog() const
	{
		return _eac3toLog;
	}
	...
	void MainForm::DeleteOnClosing(const std::string &file)
	{
		filesToDeleteOnClosing.push_back(file);
	}

	void MainForm::mnuGuide_Click(object *sender, EventArgs *e)
	{
		System::Diagnostics::Process::Start("http://mewiki.project357.com/wiki/Main_Page");
	}

	void MainForm::menuItem2_Click(object *sender, EventArgs *e)
	{

	}

	void MainForm::mnuChangelog_Click(object *sender, EventArgs *e)
	{
		std::string strChangeLog = Path::Combine(Application::StartupPath, "changelog.txt");
	...
	MainForm::MainForm()
	{
		...
		std::string strMeGUILogPath = System::IO::Path::GetDirectoryName(System::Windows::Forms::Application::ExecutablePath) + "\\logs";
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
	...
	const MeGUI::core::details::JobControl &MainForm::getJobs() const
	{
		return jobControl1;
	}
	...
	void MainForm::OnClosing(System::ComponentModel::CancelEventArgs *e)
	{
	...
	void MainForm::resetButton_Click(object *sender, System::EventArgs *e)
	{
		videoEncodingComponent1->Reset();
		audioEncodingComponent1->Reset();
	}

	void MainForm::autoEncodeButton_Click(object *sender, System::EventArgs *e)
	{
		RunTool("AutoEncode");
	}

	void MainForm::RunTool(const std::string &p)
	{
		try
		{
	...
	std::string MainForm::verifyOutputFile(const std::string &filename)
	{
		try
		{
	...
	std::string MainForm::verifyInputFile(const std::string &filename)
	{
		try
		{
			filename = Path::GetFullPath(filename); // this will throw ArgumentException if invalid
	...
	{
		XmlSerializer *ser = 0;
		std::string fileName = this->path + "\\settings.xml";
...
//		using (Stream s = File.Open(fileName, System.IO.FileMode.Create, System.IO.FileAccess.Write))
		...
		try
		{
			try
			{
				ser = new XmlSerializer(MeGUISettings::typeid);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	{
		std::string fileName = Path::Combine(path, "settings.xml");
		if (File::Exists(fileName))
		{
			XmlSerializer *ser = 0;
//C# TO C++ CONVERTER NOTE: The following 'using' block is replaced by its C++ equivalent:
//			using (Stream s = File.OpenRead(fileName))
			...
			try
			{
				...
				try
				{
					this->settings = static_cast<MeGUISettings*>(ser->Deserialize(s));
				}
				catch (...)
				{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	const MeGUI::JobUtil &MainForm::getJobUtil() const
	{
		return jobUtil;
	}

	const std::string &MainForm::getTitleText() const
	{
		return this->Text;
	}

	void MainForm::setTitleText(const std::string &value)
	{
		this->Text = value;
		trayIcon->Text = value;
	}
...
//			using (CountdownWindow countdown = new CountdownWindow(30))
			...
			try
			{
				if (countdown->ShowDialog() == System::Windows::Forms::DialogResult::OK)
				{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
		...
		else if (getJobs()->getCurrentAfterEncoding() == CloseMeGUI)
		{
			this->CloseSilent();
			Application::Exit();
		}
		...
		{
			std::string filename = getMeGUIPath() + "\\after_encoding.bat";
			try
			{
//C# TO C++ CONVERTER NOTE: The following 'using' block is replaced by its C++ equivalent:
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	const MeGUI::core::util::LogItem &MainForm::getLog() const
	{
		return logTree1->Log;
	}
	...
	{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		std::string text = getLog()->ToString();
		try
		{
			logLock->WaitOne(10000, false);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void MainForm::exitMeGUIToolStripMenuItem_Click(object *sender, EventArgs *e)
	{
		Close();
	}
	...
	void MainForm::mnuFileOpen_Click(object *sender, EventArgs *e)
	{
	...
	void MainForm::mnuViewMinimizeToTray_Click(object *sender, EventArgs *e)
	{
		formsToReopen.clear();
		this->Visible = false;
	...
	void MainForm::mnuFileExit_Click(object *sender, System::EventArgs *e)
	{
		this->Close();
	}

	void MainForm::mnuToolsSettings_Click(object *sender, System::EventArgs *e)
	{
//C# TO C++ CONVERTER NOTE: The following 'using' block is replaced by its C++ equivalent:
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void MainForm::mnuTool_Click(object *sender, System::EventArgs *e)
	{
	...
	void MainForm::mnuOptions_Click(object *sender, System::EventArgs *e)
	{
	...
	void MainForm::mnuMuxer_Click(object *sender, System::EventArgs *e)
	{
	...
	void MainForm::mnuView_Popup(object *sender, System::EventArgs *e)
	{
		std::vector<Pair<std::string, bool>*> workers = getJobs()->ListProgressWindows();
		...
		for (std::vector<Pair<std::string, bool>*>::const_iterator p = workers.begin(); p != workers.end(); ++p)
		{
	...
	void MainForm::mnuProgress_Click(object *sender, EventArgs *e)
	{
	...
	void MainForm::mnuViewProcessStatus_Click(object *sender, System::EventArgs *e)
	{
	}

	const MeGUISettings &MainForm::getSettings() const
	{
		return settings;
	}

	const MeGUI::MediaFileFactory &MainForm::getMediaFileFactory() const
	{
		return mediaFileFactory;
	}

	void MainForm::trayIcon_MouseDoubleClick(object *sender, MouseEventArgs *e)
	{
		// Activate the form.
		this->Show();
		this->Activate();
	...
	void MainForm::openMeGUIToolStripMenuItem_Click(object *sender, EventArgs *e)
	{
		trayIcon->Visible = false;
		this->Visible = true;
	}

	void MainForm::openOtherVideoFile(const std::string &fileName)
	{
	...
	void MainForm::openIndexableFile(const std::string &fileName)
	{
	...
	bool MainForm::openFile(const std::string &file, bool openVideo)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void MainForm::importProfiles(const std::string &file)
	{
		(new ProfileImporter(this, file, false))->ShowDialog();
	}

	void MainForm::MeGUI_DragDrop(object *sender, DragEventArgs *e)
	{
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	void MainForm::MeGUI_DragEnter(object *sender, DragEventArgs *e)
	{
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	void MainForm::mnuFileImport_Click(object *sender, EventArgs *e)
	{
		try
		{
	...
	void MainForm::mnuFileExport_Click(object *sender, EventArgs *e)
	{
		(new ProfileExporter(this))->ShowDialog();
	}

	void MainForm::mnuToolsAdaptiveMuxer_Click(object *sender, EventArgs *e)
	{
		AdaptiveMuxWindow *amw = new AdaptiveMuxWindow(this);
		amw->Show();
	}

	void MainForm::MeGUI_Load(object *sender, EventArgs *e)
	{
		RegisterForm(this);
	}
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	void MainForm::mnuUpdate_Click(object *sender, EventArgs *e)
	{
		UpdateWindow *update = new UpdateWindow(this, false);
		update->ShowDialog();
	}

	void MainForm::AddFileToReplace(const std::string &iUpgradeableName, const std::string &tempFilename, const std::string &filename, const std::string &newUploadDate)
	{
	...
	void MainForm::Application_ApplicationExit(object *sender, EventArgs *e)
	{
		MeGUI::Properties::Settings::getDefault()->Save();
	}
	...
	{
		for (std::vector<std::string>::const_iterator file = filesToDeleteOnClosing.begin(); file != filesToDeleteOnClosing.end(); ++file)
		{
			try
			{
	...
	const MeGUI::MuxProvider &MainForm::getMuxProvider() const
	{
		return muxProvider;
	}
	...
	{
		for (std::vector<std::string>::const_iterator file = parser->failedUpgrades.begin(); file != parser->failedUpgrades.end(); ++file)
		...
		{
			UpdateWindow *update = new UpdateWindow(this, false);
			for (std::map<std::string, std::string>::const_iterator file = parser->upgradeData.begin(); file != parser->upgradeData.end(); ++file)
	...
	void MainForm::constructMeGUIInfo()
	{
	...
	void MainForm::fillMenus()
	{
		...
		int index = 1;
		for (MeGUI::GenericRegisterer<IMuxing*>::const_iterator muxer = getPackageSystem()->getMuxerProviders()->begin(); muxer != getPackageSystem()->getMuxerProviders()->end(); ++muxer)
		{
		...
		std::vector<MenuItem*> toolsItems = std::vector<MenuItem*>();
		std::vector<Shortcut*> usedShortcuts = std::vector<Shortcut*>();
		toolsItems.push_back(mnutoolsD2VCreator);
		toolsItems.push_back(mnuMuxers);
		usedShortcuts.push_back(mnuMuxers->Shortcut);

		for (MeGUI::GenericRegisterer<ITool*>::const_iterator tool = getPackageSystem()->getTools()->begin(); tool != getPackageSystem()->getTools()->end(); ++tool)
		{
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
		...
		index = 0;
		for (std::vector<MenuItem*>::const_iterator m = toolsItems.begin(); m != toolsItems.end(); ++m)
		{
			(*m)->Index = index;
			index++;
		...
		std::vector<MenuItem*> optionsItems = std::vector<MenuItem*>();
		std::vector<Shortcut*> usedShortcuts2 = std::vector<Shortcut*>();
		optionsItems.push_back(mnuOptionsSettings);
		usedShortcuts2.push_back(mnuOptionsSettings->Shortcut);
		for (MeGUI::GenericRegisterer<IOption*>::const_iterator option = getPackageSystem()->getOptions()->begin(); option != getPackageSystem()->getOptions()->end(); ++option)
		{
			MenuItem *newMenuItem = new MenuItem();
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
		...
		index = 0;
		for (std::vector<MenuItem*>::const_iterator m = optionsItems.begin(); m != optionsItems.end(); ++m)
		{
			(*m)->Index = index;
			index++;
	...
	void MainForm::addPackages()
	{
		...
		getPackageSystem()->getJobProcessors()->Register(MeGUI::packages::tools::besplitter::Joiner::Factory);
		getPackageSystem()->getJobProcessors()->Register(MeGUI::packages::tools::besplitter::Splitter::Factory);
		...
		getPackageSystem()->getTools()->Register(new MeGUI::packages::tools::cutter::CutterTool());
		...
		getPackageSystem()->getTools()->Register(new MeGUI::packages::tools::besplitter::BesplitterTool());
		...
		getPackageSystem()->getTools()->Register(new MeGUI::packages::tools::hdbdextractor::HdBdExtractorTool());
...
Mutex *MainForm::mySingleInstanceMutex = new Mutex(true, "MeGUI_mutex");

	void MainForm::Main(std::string& args[])
	{
		// Parse for the need to run the program elevated
		bool bRunElevated = false, bForceAdmin = false;
		for (std::string::const_iterator strParam = args->begin(); strParam != args->end(); ++strParam)
		{
	...
	void MainForm::CurrentDomain_UnhandledException(object *sender, UnhandledExceptionEventArgs *e)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		...
		MessageBox::Show("MeGUI encountered a fatal error and may not be able to proceed. Reason: " + e.what(), "Fatal error", MessageBoxButtons::OK, MessageBoxIcon::Error);
	}

	void MainForm::Application_ThreadException(object *sender, System::Threading::ThreadExceptionEventArgs *e)
	{
	...
	void MainForm::runRestarter()
	{
		...
		for (std::map<std::string, CommandlineUpgradeData*>::const_iterator file = filesToReplace.begin(); file != filesToReplace.end(); ++file)
		{
			pstart->Arguments += std::string::Format("--component \"{0}\" \"{1}\" ", file->first, filesToReplace[file->first]->newUploadDate);
			for (int i = 0; i < filesToReplace[file]->filename->size(); i++)
			{
				pstart->Arguments += std::string::Format("\"{0}\" \"{1}\" ", filesToReplace[file->first]->filename[i], filesToReplace[file->first]->tempFilename[i]);
			}
		}
		if (restart)
	...
	const MeGUI::core::details::PackageSystem &MainForm::getPackageSystem() const
	{
	...
	const MeGUI::DialogManager &MainForm::getDialogManager() const
	{
		return dialogManager;
	}

	const std::string &MainForm::getMeGUIPath() const
	{
		return this->path;
	}
	...
	void MainForm::showAllWorkers_Click(object *sender, EventArgs *e)
	{
		getJobs()->ShowAllWorkers();
	}

	void MainForm::hideAllWorkers_Click(object *sender, EventArgs *e)
	{
		getJobs()->HideAllWorkers();
	}

	void MainForm::showAllWorkers_Popup(object *sender, EventArgs *e)
	{
		viewSummary->Checked = getJobs()->getSummaryVisible();

		std::vector<Pair<std::string, bool>*> workers = getJobs()->ListWorkers();
	...
	void MainForm::mnuWorker_Click(object *sender1, EventArgs *e)
	{
		MenuItem *sender = static_cast<MenuItem*>(sender1);
		getJobs()->SetWorkerVisible(sender->Text, !sender->Checked);
	}

	void MainForm::viewSummary_Click(object *sender, EventArgs *e)
	{
	...
	void MainForm::createNewWorker_Click(object *sender, EventArgs *e)
	{
		getJobs()->RequestNewWorker();
	}

	void MainForm::showAllProgressWindows_Click(object *sender, EventArgs *e)
	{
		getJobs()->ShowAllProcessWindows();
	}

	void MainForm::hideAllProgressWindows_Click(object *sender, EventArgs *e)
	{
		getJobs()->HideAllProcessWindows();
	}

	void MainForm::mnuForum_Click(object *sender, EventArgs *e)
	{
		System::Diagnostics::Process::Start("http://forum.doom9.org/forumdisplay.php?f=78");
	}

	void MainForm::mnuHome_Click(object *sender, EventArgs *e)
	{
		System::Diagnostics::Process::Start("http://sourceforge.net/projects/megui");
	}

	void MainForm::mnuBugTracker_Click(object *sender, EventArgs *e)
	{
		System::Diagnostics::Process::Start("http://sourceforge.net/tracker/?group_id=156112&atid=798476");
	}

	void MainForm::mnuFeaturesReq_Click(object *sender, EventArgs *e)
	{
		System::Diagnostics::Process::Start("http://sourceforge.net/tracker/?group_id=156112&atid=798479");
	}

	void MainForm::mnuDoc_Click(object *sender, EventArgs *e)
	{
		System::Diagnostics::Process::Start("http://mewiki.project357.com/wiki/Main_Page");
	}

	void MainForm::mnuOptionsSettings_Click(object *sender, EventArgs *e)
	{
//C# TO C++ CONVERTER NOTE: The following 'using' block is replaced by its C++ equivalent:
//		using (SettingsForm sform = new SettingsForm())
		SettingsForm *sform = new SettingsForm();
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void MainForm::getVersionInformation()
	{
	...
	#endif
		i->LogValue("Operating System", std::string::Format("{0}{1} ({2}.{3}.{4}.{5})", OSInfo::GetOSName(), OSInfo::GetOSServicePack(), OSInfo::getOSMajorVersion(), OSInfo::getOSMinorVersion(), OSInfo::getOSRevisionVersion(), OSInfo::getOSBuildVersion()));
		i->LogValue(".Net Framework", std::string::Format("{0}", OSInfo::DotNetVersionFormated(OSInfo::GetDotNetVersion())));

		VideoUtil::getAvisynthVersion(i);

		std::string devil = Path::Combine(Path::GetDirectoryName(Application::ExecutablePath), "devil.dll");
		...
		try
		{
			...
			std::string fileName = "splitter.ax";
	#endif
	#if defined(x64)
			Type *comtype = Type::GetTypeFromCLSID(new Guid("55DA30FC-F16B-49FC-BAA5-AE59FC65F82D"));
			std::string fileName = "splitter.x64.ax";
	#endif
			object *comobj = Activator::CreateInstance(comtype);
			FileUtil::GetFileInformation("Haali Matroska Splitter", Path::Combine(MeGUISettings::getHaaliMSPath(), fileName), i);
		}
		catch (std::exception &e1)
		{
		...
		FileUtil::GetFileInformation("Haali DSS2", Path::Combine(MeGUISettings::getHaaliMSPath(), "avss.dll"), i);
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	bool MainForm::AskToInstallComponents(bool bRestartRequired)
	{
		...
		if (MessageBox::Show(strQuestionText, "MeGUI component(s) missing", MessageBoxButtons::YesNo, MessageBoxIcon::Warning) == System::Windows::Forms::DialogResult::Yes)
			return true;
		else
			return false;
	}

	void MainForm::OneClickEncButton_Click(object *sender, EventArgs *e)
	{
		RunTool("one_click");
	}

	void MainForm::HelpButton_Click(object *sender, EventArgs *e)
	{
		System::Diagnostics::Process::Start("http://mewiki.project357.com/wiki/Main_Page");
	}

	void MainForm::menuItem5_Click(object *sender, EventArgs *e)
	{
		FileIndexerWindow *d2vc = new FileIndexerWindow(this);
		d2vc->Show();
	}

	void MainForm::MainForm_Move(object *sender, EventArgs *e)
	{
		if (this->WindowState != FormWindowState::Minimized && this->Visible == true)
			settings->setMainFormLocation(this->Location);
	}

	void MainForm::MainForm_Resize(object *sender, EventArgs *e)
	{
		if (this->WindowState != FormWindowState::Minimized && this->Visible == true)
	...
	void MainForm::InitializeComponent()
	{
		...
		// trayMenu
		// 
		this->trayMenu->Items->AddRange(new System::Windows::Forms::ToolStripItem[3] {this->openMeGUIToolStripMenuItem, this->toolStripSeparator1, this->exitMeGUIToolStripMenuItem});
		...
		this->openMeGUIToolStripMenuItem->Click += new System::EventHandler(this, &MainForm::openMeGUIToolStripMenuItem_Click);
		// 
		// toolStripSeparator1
		// 
		...
		this->exitMeGUIToolStripMenuItem->Click += new System::EventHandler(this, &MainForm::exitMeGUIToolStripMenuItem_Click);
		// 
		// MainForm
		// 
		this->AllowDrop = true;
		...
		this->Load += new System::EventHandler(this, &MainForm::MeGUI_Load);
		this->Shown += new System::EventHandler(this, &MainForm::MainForm_Shown);
		this->DragDrop += new System::Windows::Forms::DragEventHandler(this, &MainForm::MeGUI_DragDrop);
		this->DragEnter += new System::Windows::Forms::DragEventHandler(this, &MainForm::MeGUI_DragEnter);
	...
	{
		filesToDeleteOnClosing = std::vector<std::string>();
		allForms = std::vector<Form*>();
		formsToReopen = std::vector<Form*>();
		bImportProfileSuccessful = false;
		restart = false;
		filesToReplace = std::map<std::string, CommandlineUpgradeData*>();
		packageSystem = new PackageSystem();
		settings = new MeGUISettings();
		delete components;
	}
}
