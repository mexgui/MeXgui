...
namespace MeGUI
{
	namespace core
	{
		namespace gui
		{
			...
			void SimpleProfilesControl::SelectProfile(const std::string &fqname)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			void SimpleProfilesControl::SetProfileNameOrWarn(const std::string &fqname)
			{
				if (fqname.empty())
					return;
				try
			...
			void SimpleProfilesControl::ProfilesChanged(object *_, EventArgs *__)
			{
				RefreshProfiles();
			}

			const std::string &SimpleProfilesControl::getProfileSet() const
			{
				return profileSet;
			}

			void SimpleProfilesControl::setProfileSet(const std::string &value)
			{
				if (!profileSet.empty())
			...
			void SimpleProfilesControl::comboBox1_SelectedIndexChanged(object *sender, EventArgs *e)
			{
