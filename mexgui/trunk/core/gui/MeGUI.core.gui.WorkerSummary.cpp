...
namespace MeGUI
{
	namespace core
	{
		namespace gui
		{
			...
			void WorkerSummary::Rename(const std::string &workerName, const std::string &newName)
			{
			...
			void WorkerSummary::RefreshInfo()
			{
				...
				if (!Visible)
					return;
				for (std::map<std::string, IndividualWorkerSummary*>::const_iterator i = displays.begin(); i != displays.end(); ++i)
			...
			void WorkerSummary::Remove(const std::string &name)
			{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			void WorkerSummary::WorkerSummary_VisibleChanged(object *sender, EventArgs *e)
			{
				if (Visible)
					RefreshInfo();
			}

			void WorkerSummary::RefreshInfo(const std::string &name)
			{
				if (InvokeRequired)
				{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			void WorkerSummary::OnClosing(CancelEventArgs *e)
			{
				e->Cancel = true;
				Hide();
			}

			void WorkerSummary::newWorkerToolStripMenuItem_Click(object *sender, EventArgs *e)
			{
			...
			{
				displays = std::map<std::string, IndividualWorkerSummary*>();
				delete components;
			}
		}
	}
