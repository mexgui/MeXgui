...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			{
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
				std::vector<TreeNode*> subNodes = log->getSubEvents().ConvertAll<TreeNode*>(delegate(LogItem e)
				{
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			void LogTree::ofIndividualNodeToolStripMenuItem_Click(object *sender, EventArgs *e)
			{
				show(getselectedLogItem(), false);
			}

			void LogTree::ofBranchToolStripMenuItem_Click(object *sender, EventArgs *e)
			{
				show(getselectedLogItem(), true);
			}

			void LogTree::editLog_Click(object *sender, EventArgs *e)
			{
				show(Log, true);
			}
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			void LogTree::saveLog_Click(object *sender, EventArgs *e)
			{
				save(Log);
			}

			void LogTree::saveBranch_Click(object *sender, EventArgs *e)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			void LogTree::expandOrCollapseAll(LogItem *i, bool expand)
			{
				 ...
				 for (std::vector<LogItem*>::const_iterator i2 = i->getSubEvents().begin(); i2 != i->getSubEvents().end(); ++i2)
			...
			void LogTree::expandLog_Click(object *sender, EventArgs *e)
			{
				expandAll(Log);
			}

			void LogTree::expandBranch_Click(object *sender, EventArgs *e)
			{
				expandAll(getselectedLogItem());
			}

			void LogTree::collapseLog_Click(object *sender, EventArgs *e)
			{
				collapseAll(Log);
			}

			void LogTree::collapseBranch_Click(object *sender, EventArgs *e)
			{
				collapseAll(getselectedLogItem());
			}

			void LogTree::LogTree_Load(object *sender, EventArgs *e)
			{
			...
			void LogTree::resetOverlayIcon_Click(object *sender, EventArgs *e)
			{
			   MainForm::Instance->setOverlayIcon(0);
			}

			void LogTree::contextMenu_Opening(object *sender, CancelEventArgs *e)
			{
