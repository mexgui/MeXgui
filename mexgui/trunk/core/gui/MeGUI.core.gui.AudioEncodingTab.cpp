...
namespace MeGUI
{
	namespace core
	{
		namespace gui
		{
			...
			const std::string &AudioEncodingTab::getQueueButtonText() const
			{
				return queueAudioButton->Text;
			}

			void AudioEncodingTab::setQueueButtonText(const std::string &value)
			{
				queueAudioButton->Text = value;
			}

			const std::string &AudioEncodingTab::getAudioContainer() const
			{
				return audioContainer->Text;
			}

			void AudioEncodingTab::setAudioContainer(const std::string &value)
			{
			...
			const std::string &AudioEncodingTab::getAudioInput() const
			{
				return audioInput->getFilename();
			}

			void AudioEncodingTab::setAudioInput(const std::string &value)
			{
				audioInput->setFilename(value);
			}

			const std::string &AudioEncodingTab::getAudioOutput() const
			{
				return audioOutput->getFilename();
			}

			void AudioEncodingTab::setAudioOutput(const std::string &value)
			{
			...
			std::string AudioEncodingTab::verifyAudioSettings()
			{
				...
				{
					if (MessageBox::Show(std::string::Format("The output filename suggests the audio delay is {0}ms. " + "However, the input delay is {1}ms and the delay correction is {2}ms, " + "so a delay of {3}ms is more appropriate. Do you want MeGUI to correct " + "the output filename before adding the job?", delay.Value, inputDelay, this->delay->Value, (inputDelay - static_cast<int>(this->delay->Value))), "Output filename suggests wrong delay", MessageBoxButtons::YesNo, MessageBoxIcon::Question) == DialogResult::Yes)
				...
				std::string fileErr = MainForm::verifyOutputFile(this->getAudioOutput());
				if (fileErr != "")
				{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Replace' method:
			...
			void AudioEncodingTab::audioInput_FileSelected(FileBar *sender, FileBarEventArgs *args)
			{
			...
			void AudioEncodingTab::audioContainer_SelectedIndexChanged(object *sender, EventArgs *e)
			{
			...
			void AudioEncodingTab::deleteAudioButton_Click(object *sender, EventArgs *e)
			{
				Reset();
			}

			void AudioEncodingTab::queueAudioButton_Click(object *sender, EventArgs *e)
			{
				std::string settingsError = verifyAudioSettings();
				if (settingsError != "")
				{
			...
			void AudioEncodingTab::openAudioFile(const std::string &fileName)
			{
			...
			void AudioEncodingTab::audioProfile_SelectedProfileChanged(object *sender, EventArgs *e)
			{
