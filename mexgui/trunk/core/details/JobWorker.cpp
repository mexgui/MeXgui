...
#if defined(NONE)
			const std::string &NameTakenException::getName() const
			{
				return name;
			}
#endif
...
#if defined(NONE)
			NameTakenException::NameTakenException(const std::string &name) : MeGUIException("Worker name '" + name + "' is already in use.")
			{
				this->name = name;
			}
#endif
...
JobPostProcessor *const JobWorker::DeleteIntermediateFilesPostProcessor = new JobPostProcessor(new Processor(&deleteIntermediateFiles), "DeleteIntermediateFiles");
			...
			void JobWorker::deleteIntermediateFiles(MainForm *mainForm, Job *job)
			{
				if (mainForm->getSettings()->getDeleteIntermediateFiles())
				{
					...
					for (std::vector<std::string>::const_iterator file = job->FilesToDelete.begin(); file != job->FilesToDelete.end(); ++file)
					{
						mainForm->addToLog("Found intermediate output file '" + (static_cast<std::string>(*file)) + "', deleting...");
						try
						{
							File::Delete(*file);
...
#if defined(NONE)
			void JobWorker::addToLog(const std::string &p)
			{
				mainForm->addToLog(p);
			}
#endif
...
#if defined(NONE)
			MeGUI::Job *JobWorker::getNextJob()
			{
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
			...
			{
#if defined(NONE)
				localJobs = std::map<std::string, Job*>();
				localJobs = std::map<std::string, Job*>();
#endif
			}
		}
