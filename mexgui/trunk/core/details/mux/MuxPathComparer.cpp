...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
		lock (this)
		{
			...
			for (std::vector<PreferencedComparer>::const_iterator comparer = comparisonMethods.begin(); comparer != comparisonMethods.end(); ++comparer)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
		lock (this)
		{
			for (std::vector<CheckMuxPath>::const_iterator checker = checkingMethods.begin(); checker != checkingMethods.end(); ++checker)
			{
				if (checker(path))
					return true;
			}
	...
	MuxPath *MuxPathComparer::GetBestMuxPath(std::vector<MuxPath*> &muxPaths)
	{
	...
	int MuxPathComparer::CompareByAACContainer(MuxPath *x, MuxPath *y)
	{
		...
		for (std::vector<MuxableType*>::const_iterator initialType = x->getInitialInputTypes().begin(); initialType != x->getInitialInputTypes().end(); ++initialType)
		{
			if ((*initialType)->outputType->Equals(AudioType::RAWAAC))
				xInitialRawAAC++;
		}
		for (std::vector<MuxableType*>::const_iterator initialType = y->getInitialInputTypes().begin(); initialType != y->getInitialInputTypes().end(); ++initialType)
		{
			if ((*initialType)->outputType->Equals(AudioType::RAWAAC))
				yInitialRawAAC++;
		}
	...
	bool MuxPathComparer::CheckForUnsupportedCodecs(MuxPath *x)
	{
		...
		{
			for (std::vector<MuxableType*>::const_iterator type = x[i].handledInputTypes.begin(); type != x[i].handledInputTypes.end(); ++type)
			{
	...
	bool MuxPathComparer::CheckForVFWMatroska(MuxPath *x)
	{
		...
		for (MeGUI::MuxPath::const_iterator mpl = x->begin(); mpl != x->end(); ++mpl)
		{
	...
	{
		comparisonMethods = std::vector<PreferencedComparer>();
		checkingMethods = std::vector<CheckMuxPath>();
		comparisonSorted = false;
	}
}
