...
namespace MeGUI
{
	...
	VideoJob *JobUtil::generateVideoJob(const std::string &input, const std::string &output, VideoCodecSettings *settings, Nullable<Dar> dar, Zone zones[])
	{
		return generateVideoJob(input, output, settings, false, dar, zones);
	}

	VideoJob *JobUtil::generateVideoJob(const std::string &input, const std::string &output, VideoCodecSettings *settings, bool skipVideoCheck, Nullable<Dar> dar, Zone zones[])
	{
	...
	void JobUtil::checkVideo(const std::string &p)
	{
	...
	{
		std::string nbProc = System::Environment::GetEnvironmentVariable("NUMBER_OF_PROCESSORS");
		if (!nbProc.empty())
		{
			try
			{
	...
	MeGUI::core::details::JobChain *JobUtil::GenerateMuxJobs(VideoStream *video, Nullable<decimal> framerate, MuxStream audioStreamsArray[], MuxableType audioTypes[], MuxStream subtitleStreamsArray[], MuxableType subTypes[], const std::string &chapterFile, MuxableType *chapterInputType, ContainerType *container, const std::string &output, Nullable<FileSize> splitSize, std::vector<std::string> &inputsToDelete, const std::string &deviceType, MuxableType *deviceOutputType, bool alwaysMuxOutput)
	{
		...
		std::vector<MuxableType*> tempTypes = std::vector<MuxableType*>();
		std::vector<MuxableType*> duplicateTypes = std::vector<MuxableType*>();
		...
		for (std::vector<MuxableType*>::const_iterator oType = tempTypes.begin(); oType != tempTypes.end(); ++oType)
		{
			bool bFound = false;
		...
		while (duplicateTypes.size() > 0)
		{
			...
			for (int i = 0; i < muxPath->getLength(); i++)
				for (std::vector<MuxableType*>::const_iterator oType = muxPath[i].handledInputTypes.begin(); oType != muxPath[i].handledInputTypes.end(); ++oType)
		...
		std::vector<MuxJob*> jobs = std::vector<MuxJob*>();
		std::vector<MuxStream*> subtitleStreams = std::vector<MuxStream*>(subtitleStreamsArray);
		std::vector<MuxStream*> audioStreams = std::vector<MuxStream*>(audioStreamsArray);
		int index = 0;
		int tempNumber = 1;
		std::string previousOutput = "";
		for (MeGUI::MuxPath::const_iterator mpl = muxPath->begin(); mpl != muxPath->end(); ++mpl)
		{
			std::vector<std::string> filesToDeleteThisJob = std::vector<std::string>();
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			std::string fpsFormated = std::string::Format("{0:##.###}", framerate); // this formating is required for mkvmerge at least to avoid fps rounding error
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to this .NET Convert class method:
			...
			std::string tempOutputName = Path::Combine(Path::GetDirectoryName(output), Path::GetFileNameWithoutExtension(output) + tempNumber + ".");
			tempNumber++;
			for (std::vector<MuxableType*>::const_iterator o = mpl.handledInputTypes.begin(); o != mpl.handledInputTypes.end(); ++o)
			{
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			for (std::vector<MuxStream*>::const_iterator s = mjob->getSettings()->getAudioStreams().begin(); s != mjob->getSettings()->getAudioStreams().end(); ++s)
			{
				audioStreams.Remove(*s);
			}
			for (std::vector<MuxStream*>::const_iterator s = mjob->getSettings()->getSubtitleStreams().begin(); s != mjob->getSettings()->getSubtitleStreams().end(); ++s)
			{
	...
	MeGUI::core::details::JobChain *JobUtil::AddVideoJobs(const std::string &movieInput, const std::string &movieOutput, VideoCodecSettings *settings, int introEndFrame, int creditsStartFrame, Nullable<Dar> dar, bool prerender, bool checkVideo, Zone zones[])
	{
		JobChain *jobs = 0;
	...
	MeGUI::core::details::JobChain *JobUtil::prepareVideoJob(const std::string &movieInput, const std::string &movieOutput, VideoCodecSettings *settings, Nullable<Dar> dar, bool prerender, bool checkVideo, Zone zones[])
	{
		...
		std::string hfyuFile = "";
		std::string inputAVS = movieInput;
		if (prerender)
		{
		...
		if (checkVideo)
		{
			...
			std::string error = vUtil->checkVideo(movieInput);
			if (error != "")
			{
		...
		if (job != 0)
		{
			...
			std::vector<Job*> jobList = std::vector<Job*>();
			if (prerenderJob != 0)
			{
	...
	bool JobUtil::getInputProperties(unsigned long long &nbOfFrames, double &framerate, const std::string &video)
	{
		int d1, d2, d3, d4;
		Dar d;
	...
	bool JobUtil::GetAllInputProperties(unsigned long long &nbOfFrames, double &framerate, int &framerate_n, int &framerate_d, int &hRes, int &vRes, Dar &dar, const std::string &video)
	{
		nbOfFrames = 0;
		hRes = vRes = 0;
		framerate = 0.0;
...
//			using (AvsFile avi = AvsFile.OpenScriptFile(video))
			...
			{
//				checked
					nbOfFrames = static_cast<unsigned long long>(avi->getVideoInfo()->FrameCount);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	bool JobUtil::GetAllInputProperties(unsigned long long &nbOfFrames, double &framerate, int &hRes, int &vRes, Dar &dar, const std::string &video)
	{
		int fn, fd;
		return GetAllInputProperties(nbOfFrames, framerate, fn, fd, hRes, vRes, dar, video);
	}

	unsigned long long JobUtil::getNumberOfFrames(const std::string &path)
	{
	...
	double JobUtil::getFramerate(const std::string &path)
	{
		unsigned long long retval = 0;
		double framerate = 0.0;
		bool succ = getInputProperties(retval, framerate, path);
	...
	bool JobUtil::getFinalZoneConfiguration(VideoCodecSettings *vSettings, int introEndFrame, int creditsStartFrame, Zone zones[])
	{
		...
		unsigned long long nbOfFrames = getNumberOfFrames(mainForm->getVideo()->getVideoInput());
