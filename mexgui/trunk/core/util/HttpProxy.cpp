...
namespace MeGUI
{
	namespace core
	{
		namespace util
		{
			...
			IWebProxy *HttpProxy::GetProxy(MeGUISettings *settings)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no equivalent to implicit typing in C++ unless the C++0x inferred typing option is selected:
				var address = settings->getHttpProxyPort().empty() ? settings->getHttpProxyAddress() : std::string::Format("{0}:{1}", settings->getHttpProxyAddress(), settings->getHttpProxyPort());
