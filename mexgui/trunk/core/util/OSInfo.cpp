...
namespace MeGUI
{
	...
	std::string OSInfo::GetOSServicePack()
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	std::string OSInfo::GetDotNetVersion()
	{
		return GetDotNetVersion("");
	}

	std::string OSInfo::GetDotNetVersion(const std::string &getSpecificVersion)
	{
		std::string fv = "unknown";
		std::string componentsKeyName = "SOFTWARE\\Microsoft\\NET Framework Setup\\NDP\\";
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
				std::string *instComps = componentsKey->GetSubKeyNames();
				ArrayList *versions = new ArrayList();

				for (std::string::const_iterator instComp = instComps->begin(); instComp != instComps->end(); ++instComp)
				{
					Microsoft::Win32::RegistryKey *key = componentsKey->OpenSubKey(*instComp);
					std::string version = static_cast<std::string>(key->GetValue("Version"));
					...
					else
					{
						for (unknown::const_iterator strRegKey = key->GetSubKeyNames().begin(); strRegKey != key->GetSubKeyNames().end(); ++strRegKey)
						{
							...
							std::string strVersion = static_cast<std::string>(strKey->GetValue("Version"));
							if (!strVersion.empty())
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	std::string OSInfo::DotNetVersionFormated(const std::string &dotNetVersion)
	{
		std::string dnvf = "unknown";
		std::string major = "";
		std::string minor = "";
		std::string build = "";
		std::string revision = "";
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
				std::string *versions = dotNetVersion.Split('.');
...
//ORIGINAL LINE: case "51641":
	...
	std::string OSInfo::GetMOStuff(const std::string &queryObject)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
					...
					{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
						long long freespace = long long::Parse(*mo["FreeSpace"]->ToString()) / 1073741824;
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
						long long totalsize = long long::Parse(*mo["Size"]->ToString()) / 1073741824;
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	const std::string &OSInfo::getOSVersion() const
	{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
