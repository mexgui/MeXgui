...
namespace MeGUI
{
	namespace core
	{
		namespace util
		{
			...
			MeGUI::CropValues *Autocrop::autocrop(IVideoReader *reader)
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
			...
			MeGUI::CropValues *Autocrop::getFinalAutocropValues(CropValues values[])
			{
				CropValues *retval = values[0]->Clone();
				std::map<int, int> topValues = std::map<int, int>();
				std::map<int, int> leftValues = std::map<int, int>();
				std::map<int, int> rightValues = std::map<int, int>();
				std::map<int, int> bottomValues = std::map<int, int>();
				...
				for (std::map<int, int>::const_iterator kvp = leftValues.begin(); kvp != leftValues.end(); ++kvp)
				{
				...
				for (std::map<int, int>::const_iterator kvp = topValues.begin(); kvp != topValues.end(); ++kvp)
				{
					if (kvp->second > sizeof(values) / sizeof(values[0]) / 2) // we have more than 50% matching values, use value found
					{
				...
				for (std::map<int, int>::const_iterator kvp = rightValues.begin(); kvp != rightValues.end(); ++kvp)
				{
					if (kvp->second > sizeof(values) / sizeof(values[0]) / 2) // we have more than 50% matching values, use value found
					{
				...
				for (std::map<int, int>::const_iterator kvp = bottomValues.begin(); kvp != bottomValues.end(); ++kvp)
				{
					if (kvp->second > sizeof(values) / sizeof(values[0]) / 2) // we have more than 50% matching values, use value found
					{
			...
			MeGUI::CropValues *Autocrop::getAutoCropValues(Bitmap *b)
			{
