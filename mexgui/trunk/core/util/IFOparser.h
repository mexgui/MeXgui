...
namespace MeGUI
{
	namespace core
	{
		namespace util
		{
			...
			public:
				static std::string DetermineMovieIFO(const std::string &inputPath);

				static unsigned char *GetFileBlock(const std::string &strFile, long long pos, int count);
			...
			public:
				static long long ToFilePosition(unsigned char bytes[]);

			private:
				static long long GetTotalFrames(TimeSpan *time, int fps);

				static std::string TwoLong(int val);
			...
			public:
				static long long GetPCGIP_Position(const std::string &ifoFile);

				static int GetProgramChains(const std::string &ifoFile, long long pcgitPosition);

				static unsigned int GetChainOffset(const std::string &ifoFile, long long pcgitPosition, int programChain);

				static int GetNumberOfPrograms(const std::string &ifoFile, long long pcgitPosition, unsigned int chainOffset);

				static Nullable<TimeSpan*> ReadTimeSpan(const std::string &ifoFile, long long pcgitPosition, unsigned int chainOffset, double &fps);
				...
				static std::string getAudioLanguage(const std::string &FileName, int count);
				...
				static std::string *GetSubtitlesStreamsInfos(const std::string &FileName, int iPGC, bool bGetAllStreams);
				...
				static unsigned int getPGCnb(const std::string &FileName);
			...
			public:
				static std::string GetVideoAR(const std::string &FileName);
			};
		}
	}
}
