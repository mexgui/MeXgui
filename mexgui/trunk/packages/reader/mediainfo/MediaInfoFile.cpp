...
namespace MeGUI
{
	...
	IMediaFile *MediaInfoFileFactory::Open(const std::string &file)
	{
		return new MediaInfoFile(file);
	}

	int MediaInfoFileFactory::HandleLevel(const std::string &file)
	{
		return 5;
	}

	const std::string &MediaInfoFileFactory::getID() const
	{
		return "MediaInfo/DirectShowSource";
	}

	MediaFile *MediaInfoFile::Open(const std::string &file)
	{
		try
		{
			...
			// tracks
			std::vector<MediaTrack*> tracks = std::vector<MediaTrack*>();
			for (unknown::const_iterator t = m->Video.begin(); t != m->Video.end(); ++t)
			{
				...
				unsigned long long width = unsigned long long::Parse((*t)->Width);
				unsigned long long height = unsigned long long::Parse((*t)->Height);
				unsigned long long frameCount = unsigned long long::Parse((*t)->FrameCount);
			...
			for (std::string::const_iterator t = m->Text->begin(); t != m->Text->end(); ++t)
			{
...
Regex *MediaInfoFile::chaptersRegex = new Regex("^(?<num>\\d+)\\s*:\\s*(?<hours>\\d+):(?<mins>\\d+):(?<secs>\\d+).(?<ms>\\d+) (?<name>.*)$", RegexOptions::Multiline | RegexOptions::Compiled);
	...
	SubtitleCodec *MediaInfoFile::getSubtitleCodec(const std::string &p)
	{
		try
		{
			return 0;
...
CultureInfo *const MediaInfoFile::culture = new CultureInfo("en-us");
	...
	const std::string &MediaInfoFile::getContainerFileTypeString() const
	{
		return _strContainer;
	}

	MediaInfoFile::MediaInfoFile(const std::string &file, LogItem *&oLog)
	{
		InitializeInstanceFields();
	}

	MediaInfoFile::MediaInfoFile(const std::string &file, LogItem *&oLog, int iPGCNumber)
	{
		InitializeInstanceFields();
		GetSourceInformation(file, oLog, iPGCNumber);
	}

	MediaInfoFile::MediaInfoFile(const std::string &file)
	{
		InitializeInstanceFields();
	}

	MediaInfoFile::MediaInfoFile(const std::string &file, int iPGCNumber)
	{
		InitializeInstanceFields();
		GetSourceInformation(file, 0, iPGCNumber);
	}

	void MediaInfoFile::GetSourceInformation(const std::string &file, LogItem *oLog, int iPGCNumber)
	{
...
//ORIGINAL LINE: case "ES":
			...
			//subtitle detection
			int i = 0;
			for (std::string::const_iterator oTextTrack = info->Text->begin(); oTextTrack != info->Text->end(); ++oTextTrack)
			{
				int mmgTrackID = 0;
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
				...
				std::string strCodec = oTrack->getCodec().ToUpper(System::Globalization::CultureInfo::InvariantCulture);
...
//				checked
					...
					_VideoInfo->Width = static_cast<unsigned long long>(easyParseInt(track->Width).Value);
					_VideoInfo->Height = static_cast<unsigned long long>(easyParseInt(track->Height).Value);
					Nullable<int> tempVar = easyParseInt(track.FrameCount);
					_VideoInfo->FrameCount = static_cast<unsigned long long>(tempVar::HasValue ? tempVar : 0);
...
//C# TO C++ CONVERTER TODO TASK: End of the original C# 'checked' block
	...
	void MediaInfoFile::WriteSourceInformation(MediaInfo *oInfo, const std::string &strFile, LogItem *infoLog)
	{
		try
		{
			// general track
			...
			// text track
			for (std::string::const_iterator t = oInfo->Text->begin(); t != oInfo->Text->end(); ++t)
			{
	...
	void MediaInfoFile::CorrectSourceInformation(MediaInfo *&oInfo, const std::string &strFile, LogItem *infoLog, int iPGCNumber)
	{
		try
		{
			if (oInfo->Video->Count > 0 && Path::GetExtension(strFile.ToLower(System::Globalization::CultureInfo::InvariantCulture)) == ".vob")
			{
				...
				if (File::Exists(ifoFile))
				{
					...
					std::string strResult = IFOparser::GetVideoAR(ifoFile);
					if (!strResult.empty())
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
					std::string *arrSubtitle = IFOparser::GetSubtitlesStreamsInfos(ifoFile, _VideoInfo->PGCNumber, false);
					for (std::string::const_iterator strSubtitle = arrSubtitle->begin(); strSubtitle != arrSubtitle->end(); ++strSubtitle)
					{
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
						std::string *strLanguage = (*strSubtitle).Split('-');
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			else if (oInfo->General[0]->FormatString->ToLower(System::Globalization::CultureInfo::InvariantCulture)->Equals("blu-ray playlist"))
			{
				...
				const std::string tempVector[] = {strFile};
				_Eac3toInfo = new Eac3toInfo(std::vector<std::string>(tempVector, tempVector + sizeof(tempVector) / sizeof(tempVector[0])), this, infoLog);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	int MediaInfoFile::getEac3toChaptersTrack()
	{
		...
		if (_Eac3toInfo == 0)
		{
			const std::string tempVector2[] = {_file};
			_Eac3toInfo = new Eac3toInfo(std::vector<std::string>(tempVector2, tempVector2 + sizeof(tempVector2) / sizeof(tempVector2[0])), this, _Log);
		}
	...
	bool MediaInfoFile::extractMKVChapters(const std::string &strChapterFile)
	{
		if (cType != ContainerType::MKV)
			return false;
	...
	bool MediaInfoFile::recommendIndexer(std::vector<std::string> &arrIndexer)
	{
		FileIndexerWindow::IndexType oType = FileIndexerWindow::NONE;
		for (std::vector<std::string>::const_iterator strIndexer = arrIndexer.begin(); strIndexer != arrIndexer.end(); ++strIndexer)
		{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	Nullable<int> MediaInfoFile::easyParseInt(const std::string &value)
	{
		try
		{
	...
	Nullable<double> MediaInfoFile::easyParseDouble(const std::string &value)
	{
		try
		{
	...
	Nullable<decimal> MediaInfoFile::easyParseDecimal(const std::string &value)
	{
		try
		{
	...
	ContainerType *MediaInfoFile::getContainerType(const std::string &codec, const std::string &description)
	{
		...
		for (std::map<std::string, ContainerType*>::const_iterator knownDescription = knownContainerDescriptions.begin(); knownDescription != knownContainerDescriptions.end(); ++knownDescription)
			if (description.find(knownDescription->first) != string::npos)
				return knownContainerDescriptions[knownDescription->first];
		return 0;
	}

	AudioCodec *MediaInfoFile::getAudioCodec(const std::string &description)
	{
		description = description.ToLower(System::Globalization::CultureInfo::InvariantCulture);
		for (std::map<std::string, AudioCodec*>::const_iterator knownDescription = knownAudioDescriptions.begin(); knownDescription != knownAudioDescriptions.end(); ++knownDescription)
			if (description.find(knownDescription->first) != string::npos)
	...
	VideoCodec *MediaInfoFile::getVideoCodec(const std::string &description)
	{
		description = description.ToLower(System::Globalization::CultureInfo::InvariantCulture);
		for (std::map<std::string, VideoCodec*>::const_iterator knownDescription = knownVideoDescriptions.begin(); knownDescription != knownVideoDescriptions.end(); ++knownDescription)
			if (description.find(knownDescription->first) != string::npos)
				return knownVideoDescriptions[knownDescription->first];
		return 0;
	}

	VideoType *MediaInfoFile::getVideoType(VideoCodec *codec, ContainerType *cft, const std::string &filename)
	{
		std::string extension = Path::GetExtension(filename)->ToLower(System::Globalization::CultureInfo::InvariantCulture);
		for (MeGUI::GenericRegisterer<VideoType*>::const_iterator t = ContainerManager::VideoTypes->begin(); t != ContainerManager::VideoTypes->end(); ++t)
		{
	...
	AudioType *MediaInfoFile::getAudioType(AudioCodec *codec, ContainerType *cft, const std::string &filename)
	{
		std::string extension = Path::GetExtension(filename)->ToLower(System::Globalization::CultureInfo::InvariantCulture);
		ContainerType *type = 0;
		if (cft != 0)
			type = cft;
		for (MeGUI::GenericRegisterer<AudioType*>::const_iterator t = ContainerManager::AudioTypes->begin(); t != ContainerManager::AudioTypes->end(); ++t)
		{
	...
	{
		knownVideoDescriptions = std::map<std::string, VideoCodec*>();
		...
		knownAudioDescriptions = std::map<std::string, AudioCodec*>();
		...
		knownContainerTypes = std::map<std::string, ContainerType*>();
		...
		knownContainerDescriptions = std::map<std::string, ContainerType*>();
	}
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	const std::string &MediaInfoFile::getFileName() const
	{
		return _file;
	}
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	VideoInformation::VideoInformation(bool hasVideo, unsigned long long width, unsigned long long height, Dar dar, unsigned long long frameCount, double fps, int fps_n, int fps_d)
	{
		HasVideo = hasVideo;
		Width = width;
		Height = height;
	...
	const std::string &VideoInformation::getScanType() const
	{
		return _strVideoScanType;
	}

	void VideoInformation::setScanType(const std::string &value)
	{
	...
	const std::vector<AudioTrackInfo*> &AudioInformation::getTracks() const
	{
		return _arrAudioTracks;
	}

	void AudioInformation::setTracks(const std::vector<AudioTrackInfo*> &value)
	{
	...
	{
		_arrAudioTracks = std::vector<AudioTrackInfo*>();
	}
	...
	const std::vector<SubtitleTrackInfo*> &SubtitleInformation::getTracks() const
	{
		return _arrSubtitleTracks;
	}

	void SubtitleInformation::setTracks(const std::vector<SubtitleTrackInfo*> &value)
	{
	...
	{
		_arrSubtitleTracks = std::vector<SubtitleTrackInfo*>();
	}
}
