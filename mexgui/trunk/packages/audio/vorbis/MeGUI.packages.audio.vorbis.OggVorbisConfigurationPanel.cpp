#include "MeGUI.packages.audio.vorbis.OggVorbisConfigurationPanel.h"
	...
	namespace packages
	{
		namespace audio
		{
			namespace vorbis
			{
				...
				OggVorbisConfigurationPanel::OggVorbisConfigurationPanel() : MeGUI::core::details::audio::AudioConfigurationPanel()
				{
				...
				const MeGUI::AudioCodecSettings &OggVorbisConfigurationPanel::getCodecSettings() const
				{
				...
				void OggVorbisConfigurationPanel::vQuality_ValueChanged(object *sender, EventArgs *e)
				{
					Decimal q = (static_cast<Decimal>(vQuality->Value)) * 10.0 / vQuality->Maximum;
					label1->Text = std::string::Format("Variable Bitrate (Q={0}) ", q);
				}

				const MeGUI::OggVorbisSettings &OggVorbisConfigurationPanel::getSettings() const
				{
