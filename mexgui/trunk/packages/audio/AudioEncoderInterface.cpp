...
System::Text::RegularExpressions::Regex *const AviSynthAudioEncoder::_cleanUpStringRegex = new System::Text::RegularExpressions::Regex("\\n[^\\n]+\\r", System::Text::RegularExpressions::RegexOptions::Compiled | System::Text::RegularExpressions::RegexOptions::CultureInvariant);
	...
	void AviSynthAudioEncoder::writeTempTextFile(const std::string &filePath, const std::string &text)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	{
		for (std::vector<std::string>::const_iterator filePath = _tempFiles.begin(); filePath != _tempFiles.end(); ++filePath)
			safeDelete(*filePath);

	}

	void AviSynthAudioEncoder::safeDelete(const std::string &filePath)
	{
		try
		{
	...
	void AviSynthAudioEncoder::createTemporallyEqFiles(const std::string &tempPath)
	{
	...
	void AviSynthAudioEncoder::raiseEvent(const std::string &s)
	{
		su->setStatus(s);
		raiseEvent();
	}

	AviSynthAudioEncoder::AviSynthAudioEncoder(MeGUISettings *settings) : _uniqueId(Guid::NewGuid()->ToString("N"))
	{
	...
	void AviSynthAudioEncoder::ProcessLine(const std::string &line, StreamType stream, ImageType oType)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
	...
	{
		std::string fileErr = MainForm::verifyInputFile(this->audioJob->Input);
		if (fileErr != "")
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void AviSynthAudioEncoder::createEncoderProcess(AviSynthClip *a)
	{
		try
		{
			...
			info->Arguments = std::string::Format(_encoderCommandLine, audioJob->Output, a->getAudioSampleRate(), a->getBitsPerSample(), a->getChannelsCount(), a->getSamplesCount(), a->getAudioSizeInBytes());
			...
			stdoutLog = _log->Info(std::string::Format("[{0:G}] {1}", DateTime::Now, "Standard output stream"));
			stderrLog = _log->Info(std::string::Format("[{0:G}] {1}", DateTime::Now, "Standard error stream"));
	...
	void AviSynthAudioEncoder::writeHeader(Stream *target, AviSynthClip *a)
	{
		...
		bool useFaadTrick = a->getAudioSizeInBytes() >= (static_cast<long long>(unsigned int::MaxValue) - WAV_HEADER_SIZE);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void AviSynthAudioEncoder::createAviSynthScript()
	{
		...
		std::string id = _uniqueId;
		std::string tmp = Path::Combine(Path::GetTempPath(), id);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void AviSynthAudioEncoder::InitializeInstanceFields()
	{
		...
		delete _readFromStdErrThread;
		delete _settings;
		_tempFiles = std::vector<std::string>();
	}
}
