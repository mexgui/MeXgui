...
namespace MeGUI
{
	...
	AviSynthWindow::AviSynthWindow(MainForm *mainForm)
	{
		...
		this->controlsToDisable = std::vector<Control*>();
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	void AviSynthWindow::ProfileChanged(object *sender, EventArgs *e)
	{
		this->setSettings(GetProfileSettings());
	}

	AviSynthWindow::AviSynthWindow(MainForm *mainForm, const std::string &videoInput)
	{
		InitializeInstanceFields();
		scriptRefresh--;
	...
	AviSynthWindow::AviSynthWindow(MainForm *mainForm, const std::string &videoInput, const std::string &indexFile)
	{
		InitializeInstanceFields();
		scriptRefresh--;
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	void AviSynthWindow::openDLLButton_Click(object *sender, System::EventArgs *e)
	{
		this->openFilterDialog->InitialDirectory = MainForm::Instance->getSettings()->getAvisynthPluginsPath();
		if (this->openFilterDialog->ShowDialog() == System::Windows::Forms::DialogResult::OK)
		{
			dllPath->Text = openFilterDialog->FileName;
			std::string temp = avisynthScript->Text;
			script = new StringBuilder();
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void AviSynthWindow::previewButton_Click(object *sender, System::EventArgs *e)
	{
	...
	void AviSynthWindow::saveButton_Click(object *sender, System::EventArgs *e)
	{
		std::string fileName = videoOutput->getFilename();
	...
	std::string AviSynthWindow::generateScript()
	{
		script = new StringBuilder();
		...
		std::string deinterlaceLines = "#deinterlace";
		std::string denoiseLines = "#denoise";
		std::string cropLine = "#crop";
		std::string resizeLine = "#resize";
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		...
		std::string newScript = ScriptServer::CreateScriptFromTemplate(GetProfileSettings()->getTemplate(), inputLine, cropLine, resizeLine, denoiseLines, deinterlaceLines);

		if (this->signalAR->Checked && suggestedDar.HasValue)
			newScript = std::string::Format("# Set DAR in encoder to {0} : {1}. The following line is for automatic signalling\r\nglobal MeGUI_darx = {0}\r\nglobal MeGUI_dary = {1}\r\n", suggestedDar.Value::X, suggestedDar.Value::Y) + newScript;
		...
		if (this->SubtitlesPath->Text != "")
		{
			...
			{
				std::string charset = CharsetValue();
	...
	void AviSynthWindow::showScript(bool bForce)
	{
		...
		std::string oldScript = avisynthScript->Text;
		avisynthScript->Text = this->generateScript();
		if (!oldScript.Equals(avisynthScript->Text))
			chAutoPreview_CheckedChanged(0, 0);
	}

	void AviSynthWindow::openVideoSource(const std::string &videoInput, const std::string &indexFileTemp)
	{
		std::string ext, projectPath, fileNameNoPath;
...
//ORIGINAL LINE: case ".vdr":
	...
	void AviSynthWindow::writeScript(const std::string &path)
	{
		try
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void AviSynthWindow::checkNVCompatibleFile(const std::string &input)
	{
		bool flag = false;
//C# TO C++ CONVERTER NOTE: The following 'using' block is replaced by its C++ equivalent:
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void AviSynthWindow::openDirectShow(const std::string &fileName)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
				...
				tempAvs = std::string::Format("DirectShowSource(\"{0}\", audio=false{1}, convertfps=true){2}{3}", fileName, frameRateString == "" ? "" : (", fps=" + frameRateString), VideoUtil::getAssumeFPS(0, fileName), this->flipVertical->Checked ? ".FlipVertical()" : "");
	...
	void AviSynthWindow::openVDubFrameServer(const std::string &fileName)
	{
		if (!File::Exists(fileName))
		{
			MessageBox::Show(fileName + " could not be found","File Not Found",MessageBoxButtons::OK);
	...
	void AviSynthWindow::openAVSScript(const std::string &fileName)
	{
		if (!File::Exists(fileName))
		{
			MessageBox::Show(fileName + " could not be found", "File Not Found", MessageBoxButtons::OK);
	...
	{
		for (std::vector<Control*>::const_iterator ctrl = this->controlsToDisable.begin(); ctrl != this->controlsToDisable.end(); ++ctrl)
			(*ctrl)->Enabled = enable;
	...
	void AviSynthWindow::openVideo(const std::string &videoInput)
	{
		if (indexFile.empty())
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	void AviSynthWindow::openVideo(const std::string &videoInputScript, const std::string &strSourceFileName, bool inlineAvs)
	{
		this->crop->Checked = false;
	...
	void AviSynthWindow::calcAspectError()
	{
		...
		lblAspectError->Text = std::string::Format("{0:0.00000%}", aspectError);
	...
	void AviSynthWindow::deinterlace_CheckedChanged(object *sender, EventArgs *e)
	{
	...
	void AviSynthWindow::noiseFilter_CheckedChanged(object *sender, EventArgs *e)
	{
	...
	void AviSynthWindow::autoCropButton_Click(object *sender, EventArgs *e)
	{
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	void AviSynthWindow::analyseButton_Click(object *sender, EventArgs *e)
	{
		if (input->getFilename().length() > 0)
		{
			if (detector == 0) // We want to start the analysis
			{
				std::string source = ScriptServer::GetInputLine(input->getFilename(), indexFile, false, sourceType, false, false, false, 25, false);
				if (nvDeInt->Enabled)
					source += ")";
...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	void AviSynthWindow::finishedAnalysis(SourceInfo *info, bool error, const std::string &errorMessage)
	{
		if (error)
		{
			detector->stop();
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
	...
	void AviSynthWindow::deintSourceType_SelectedIndexChanged(object *sender, EventArgs *e)
	{
	...
	void AviSynthWindow::reopenOriginal_Click(object *sender, EventArgs *e)
	{
	...
	void AviSynthWindow::chAutoPreview_CheckedChanged(object *sender, EventArgs *e)
	{
		if (chAutoPreview->Checked)
	...
	void AviSynthWindow::nvDeInt_CheckedChanged(object *sender, EventArgs *e)
	{
		if (nvDeInt->Checked)
	...
	void AviSynthWindow::nvDeInt_Click(object *sender, EventArgs *e)
	{
		// just to be sure
		checkNVCompatibleFile(input->getFilename());
	}

	void AviSynthWindow::openSubtitlesButton_Click(object *sender, EventArgs *e)
	{
		if (this->openSubsDialog->ShowDialog() != System::Windows::Forms::DialogResult::OK)
			return;

		if (this->SubtitlesPath->Text != openSubsDialog->FileName)
		{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			std::string ext = Path::GetExtension(openSubsDialog->FileName)->ToString()->ToLower(System::Globalization::CultureInfo::InvariantCulture);
	...
	std::string AviSynthWindow::CharsetValue()
	{
		std::string c = "";
	...
	void AviSynthWindow::inputDARChanged(object *sender, const std::string &val)
	{
		updateEverything(sender != 0, false, false);
	}

	void AviSynthWindow::updateEverything(object *sender, EventArgs *e)
	{
		updateEverything(sender != 0, false, false);
	}
	...
	void AviSynthWindow::refreshScript(object *sender, EventArgs *e)
	{
		if (sender != 0 && e != 0)
			showScript(false);
	}

	void AviSynthWindow::AviSynthWindow_Shown(object *sender, EventArgs *e)
	{
		if (player != 0 && !player->Visible)
			player->Show();
	}

	void AviSynthWindow::resize_CheckedChanged(object *sender, EventArgs *e)
	{
	...
	void AviSynthWindow::InitializeComponent()
	{
		...
		this->input->FileSelected += new MeGUI::FileBarEventHandler(this, &AviSynthWindow::input_FileSelected);
		// 
		// avsProfile
		// 
		...
		this->arChooser->SelectionChanged += new MeGUI::StringChanged(this, &AviSynthWindow::inputDARChanged);
		// 
		// AviSynthWindow
		// 
