#include "MeGUI.core.gui.AviSynthProfileConfigPanel.h"
	...
	namespace core
	{
		namespace gui
		{
			...
			AviSynthProfileConfigPanel::AviSynthProfileConfigPanel()
			{
				...
				if (!MeGUISettings::getHaaliMSPath().empty() && System::IO::File::Exists(System::IO::Path::Combine(MeGUISettings::getHaaliMSPath(), "avss.dll")))
					dss2->Enabled = true;
			}

			const MeGUI::AviSynthSettings &AviSynthProfileConfigPanel::getSettings() const
			{
			...
			void AviSynthProfileConfigPanel::signalAR_CheckedChanged(object *sender, EventArgs *e)
			{
				mod16Box->Enabled = signalAR->Checked;
			}

			void AviSynthProfileConfigPanel::insert_Click(object *sender, EventArgs *e)
			{
				std::string text = dynamic_cast<std::string>((dynamic_cast<Control*>(sender))->Tag);
				std::string avsScript = avisynthScript->Text;
				std::string avsScriptA = avsScript.substr(0, avisynthScript->SelectionStart);
				std::string avsScriptB = avsScript.substr(avisynthScript->SelectionStart + avisynthScript->SelectionLength);
			...
			void AviSynthProfileConfigPanel::noiseFilter_CheckedChanged(object *sender, EventArgs *e)
			{
				noiseFilterType->Enabled = noiseFilter->Checked;
			}

			void AviSynthProfileConfigPanel::resize_CheckedChanged(object *sender, EventArgs *e)
			{
			...
			void AviSynthProfileConfigPanel::InitializeComponent()
			{
				...
				this->dllBar->FileSelected += new MeGUI::FileBarEventHandler(this, &AviSynthProfileConfigPanel::dllBar_FileSelected);
				// 
				// flowLayoutPanel1
				// 
