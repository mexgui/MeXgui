...
namespace MeGUI
{
	...
	void QuantizerMatrixDialog::QuantizerMatrixDialog_Load(object *sender, System::EventArgs *e)
	{
	...
	void QuantizerMatrixDialog::predefinedMatrix_SelectedIndexChanged(object *sender, System::EventArgs *e)
	{
	...
	void QuantizerMatrixDialog::matrixSize_SelectedIndexChanged(object *sender, System::EventArgs *e)
	{
		if (doEvents)
		{
	...
	void QuantizerMatrixDialog::matrixType_SelectedIndexChanged(object *sender, System::EventArgs *e)
	{
		if (doEvents)
		{
			if (predefinedMatrix->SelectedIndex == 3) // save previous matrix before showing the new one
	...
	std::string QuantizerMatrixDialog::convertMatrixToString(int matrix[][])
	{
		std::string retval = "";
	...
	int **QuantizerMatrixDialog::convertStringToMatrix(const std::string &matrix, int size)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
		std::string *quantizers = matrix.Split(splitChars);
		int column = 0;
		int line = 0;
		for (std::string::const_iterator quantizer = quantizers->begin(); quantizer != quantizers->end(); ++quantizer)
		{
			int val = 16;
			try
			{
			...
			catch (std::exception &e)
			{
				...
				_oLog->LogValue("convertStringToMatrix: " + matrix, e, MeGUI::core::util::Error);
	...
	void QuantizerMatrixDialog::saveMatrixButton_Click(object *sender, System::EventArgs *e)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
			...
			{
				MeGUI::core::util::LogItem *_oLog = MainForm::Instance->getLog()->Info("Error");
				_oLog->LogValue("Error", f, MeGUI::core::util::Error);
			}
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void QuantizerMatrixDialog::loadMatrixButton_Click(object *sender, System::EventArgs *e)
	{
		if (openFileDialog->ShowDialog() == System::Windows::Forms::DialogResult::OK)
		{
			StreamReader *sr = 0;
			std::string matrix = "", line = "";
			try
			{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	const std::string &CQMEditorTool::getName() const
	{
	...
	const std::string &CQMEditorTool::getID() const
	{
		return "cqmEditor";
	}
}
