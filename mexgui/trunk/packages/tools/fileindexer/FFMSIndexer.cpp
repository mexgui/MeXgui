...
JobProcessorFactory *const FFMSIndexer::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "FFMSIndexer");
	...
	FFMSIndexer::FFMSIndexer(const std::string &executableName)
	{
		executable = executableName;
	}

	void FFMSIndexer::ProcessLine(const std::string &line, StreamType stream, ImageType oType)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Split' method:
	...
	const std::string &FFMSIndexer::getCommandline() const
	{
	...
	void FFMSIndexer::doExitConfig()
	{
		if (job->getDemuxMode() > 0 && !su->getHasError() && !su->getWasAborted() && job->getAudioTracks().size() > 0)
		{
			...
			for (int iCurrentTrack = 0; iCurrentTrack <= 29; iCurrentTrack++) // hard limit to max. 30 tracks
			{
				...
				std::string strDLLPath = Path::Combine(Path::GetDirectoryName(MainForm::Instance->getSettings()->getFFMSIndexPath()), "ffms2.dll");
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
				...
				for (std::vector<AudioTrackInfo*>::const_iterator oAudioTrack = job->getAudioTracks().begin(); oAudioTrack != job->getAudioTracks().end(); ++oAudioTrack)
				{
					if ((*oAudioTrack)->getTrackIndex() != iCurrentAudioTrack)
						continue;

					// write avs file
					std::string strAudioAVSFile;
