...
namespace BDInfo
{
	...
	std::string TSPlaylistFile::ToString()
	{
		return Name;
	}

	const unsigned long long &TSPlaylistFile::getFileSize() const
	{
		unsigned long long size = 0;
		for (std::vector<TSStreamClip*>::const_iterator clip = StreamClips.begin(); clip != StreamClips.end(); ++clip)
		{
			size += (*clip)->FileSize;
		}
		return size;
	...
	const unsigned long long &TSPlaylistFile::getTotalSize() const
	{
		unsigned long long size = 0;
		for (std::vector<TSStreamClip*>::const_iterator clip = StreamClips.begin(); clip != StreamClips.end(); ++clip)
		{
	...
	const unsigned long long &TSPlaylistFile::getAngleSize() const
	{
		unsigned long long size = 0;
		for (std::vector<TSStreamClip*>::const_iterator clip = StreamClips.begin(); clip != StreamClips.end(); ++clip)
		{
	...
	const unsigned long long &TSPlaylistFile::getTotalBitRate() const
	{
		if (getTotalLength() > 0)
		{
			return static_cast<unsigned long long>(Math::Round(((getTotalSize() * 8.0) / getTotalLength())));
		}
		return 0;
	}

	const unsigned long long &TSPlaylistFile::getAngleBitRate() const
	{
		if (getAngleLength() > 0)
		{
			return static_cast<unsigned long long>(Math::Round(((getAngleSize() * 8.0) / getAngleLength())));
		}
		return 0;
	}
		...
		//Dictionary<string, TSStreamClipFile> streamClipFiles)
	{
		...
		std::map<std::string, TSStreamClipFile*> streamClipFiles = std::map<std::string, TSStreamClipFile*>();
		...
		try
		{
	...
	#if defined(DEBUG)
			Debug::WriteLine(std::string::Format("\tStreamFileCount: {0}", streamFileCount));
	#endif
			std::vector<TSStreamClip*> chapterClips = std::vector<TSStreamClip*>();
			...
			for (int streamFileIndex = 0; streamFileIndex < streamFileCount; streamFileIndex++)
			{
				...
				std::string streamClipFileName = std::string::Format("{0}.CLPI", ASCIIEncoding::ASCII::GetString(streamFileNameData));
				std::string streamClipFilePath = Path::Combine(Path::Combine(FileInfo->Directory->Parent->FullName, "CLIPINF"), streamClipFileName);
				...
				if (streamClipFile == 0)
				{
					throw std::exception(std::string::Format("Playlist {0} referenced missing file {1}.", FileInfo->Name, streamClipFileName));
				}
	...
	#if defined(DEBUG)
				Debug::WriteLine(std::string::Format("\t{0} {1} {2} {3}", streamClip->Name, streamClip->TimeIn.TotalSeconds, streamClip->TimeOut.TotalSeconds, streamClip->Length.TotalSeconds));
	#endif
				if ((playlistData[streamFileOffset + 12] & 0x10) > 0)
				{
					...
					for (int angle = 0; angle < (angleCount - 1); angle++)
					{
						...
						std::string angleClipFileName = std::string::Format("{0}.CLPI", ASCIIEncoding::ASCII::GetString(angleFileNameData));
						...
						if (angleClipFile == 0)
						{
							throw std::exception(std::string::Format("Playlist {0} referenced missing angle file {1}.", FileInfo->Name, angleClipFileName));
						}
	...
	#if defined(DEBUG)
						Debug::WriteLine(std::string::Format("\t\t{0}", angleFileName));
	#endif
					}
				}
	...
	#if defined(DEBUG)
					Debug::WriteLine(std::string::Format("\t{0} {1} {2}", chapterIndex, streamClip->Name, chapter::TotalSeconds));
	#endif
				}
				chapterOffset += 14;
			}
	#if defined(DEBUG)
			Debug::WriteLine(std::string::Format("\tLength: {0}", Length::TotalSeconds));
			Debug::WriteLine(std::string::Format("\tAngleLength: {0}", getAngleLength().TotalSeconds));
	#endif
			//LoadStreamClips();
			IsInitialized = true;
		}
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void TSPlaylistFile::InitializeInstanceFields()
	{
		...
		StreamClips = std::vector<TSStreamClip*>();
		AngleStreams = std::vector<std::map<unsigned short, TSStream*>*>();
		AngleCount = 0;
	}
}
