...
namespace MeGUI
{
	...
	ChapterCreator::ChapterCreator(MainForm *mainForm)
	{
		...
		pgc = new ChapterInfo(new object[] {Chapters = std::vector<Chapter>(), FramesPerSecond = 25.0, LangCode = ""});
	}
	...
	void ChapterCreator::showChapters(Chapter chaps[])
	{
		...
		for (MeGUI::Chapter::const_iterator chap = chaps->begin(); chap != chaps->end(); ++chap)
		{
			ListViewItem *item = new ListViewItem(new std::string[] {(*chap).timecode, (*chap).name});
			item->Tag = *chap;
	...
	void ChapterCreator::FreshChapterView()
	{
		...
		try
		{
			this->chapterListView->Items->Clear();
			//fill list
			for (std::vector<Chapter>::const_iterator c = pgc->getChapters().begin(); c != pgc->getChapters().end(); ++c)
			{
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
				ListViewItem *item = new ListViewItem(new std::string[] {(*c).getTime()->ToString(), (*c).getName()});
				chapterListView->Items->Add(item);
				if (item->Index % 2 != 0)
					item->BackColor = Color::White;
				else
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void ChapterCreator::removeZoneButton_Click(object *sender, System::EventArgs *e)
	{
	...
	void ChapterCreator::clearZonesButton_Click(object *sender, System::EventArgs *e)
	{
	...
	void ChapterCreator::chapterListView_SelectedIndexChanged(object *sender, System::EventArgs *e)
	{
	...
	void ChapterCreator::addZoneButton_Click(object *sender, System::EventArgs *e)
	{
		Chapter c;
	...
	void ChapterCreator::saveButton_Click(object *sender, System::EventArgs *e)
	{
		...
		{
			std::string ext = Path::GetExtension(saveFileDialog->FileName)->ToLower(System::Globalization::CultureInfo::InvariantCulture);
	...
	void ChapterCreator::btInput_Click(object *sender, EventArgs *e)
	{
...
//					using (frmStreamSelect frm = new frmStreamSelect(ex))
					...
					try
					{
						...
						if (frm->getChapterCount() == 1 || frm->ShowDialog(this) == System::Windows::Forms::DialogResult::OK)
						{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	void ChapterCreator::showVideoButton_Click(object *sender, System::EventArgs *e)
	{
	...
	void ChapterCreator::setVideoInput(const std::string &value)
	{
	...
	void ChapterCreator::player_Closed(object *sender, EventArgs *e)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void ChapterCreator::ChapterCreator_Load(object *sender, EventArgs *e)
	{
	...
	void ChapterCreator::chapterName_TextChanged(object *sender, EventArgs *e)
	{
		try
		{
			if (chapterListView->SelectedIndices->Count == 0)
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	const std::string &ChapterCreatorTool::getName() const
	{
	...
	const std::string &ChapterCreatorTool::getID() const
	{
