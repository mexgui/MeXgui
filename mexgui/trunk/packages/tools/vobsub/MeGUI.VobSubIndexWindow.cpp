#include "MeGUI.VobSubIndexWindow.h"
...
namespace MeGUI
{
	...
	void VobSubIndexWindow::queueButton_Click(object *sender, EventArgs *e)
	{
		if (Drives::ableToWriteOnThisDrive(Path::GetPathRoot(output->getFilename())))
		{
			...
			MessageBox::Show("MeGUI cannot write on " + Path::GetPathRoot(output->getFilename()) + ". Please, select another output path.", "Configuration Incomplete", MessageBoxButtons::OK);
	}

	void VobSubIndexWindow::openVideo(const std::string &fileName)
	{
	...
	{
		std::vector<int> trackIDs = std::vector<int>();
	...
	void VobSubIndexWindow::setConfig(const std::string &input, const std::string &output, bool indexAllTracks, std::vector<int> &trackIDs, int pgc)
	{
		...
		else
		{
			...
			int index = 0;
			std::vector<int> checkedItems = std::vector<int>();
			...
			for (std::vector<int>::const_iterator idx = checkedItems.begin(); idx != checkedItems.end(); ++idx)
			{
	...
	void VobSubIndexWindow::chkShowAllStreams_CheckedChanged(object *sender, EventArgs *e)
	{
	...
	void VobSubIndexWindow::pgc_ValueChanged(object *sender, EventArgs *e)
	{
		if (input->getFilename().empty())
			return;
		openVideo(input->getFilename());
	...
	void VobSubIndexWindow::InitializeComponent()
	{
		...
		this->output->FileSelected += new MeGUI::FileBarEventHandler(this, &VobSubIndexWindow::output_FileSelected);
		// 
		// input
		// 
		...
		this->input->setTitle("");
		this->input->FileSelected += new MeGUI::FileBarEventHandler(this, &VobSubIndexWindow::input_FileSelected);
		// 
		// VobSubIndexWindow
		// 
