...
JobProcessorFactory *const HDStreamExtractorIndexer::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "HDStreamExtIndexer");
	...
	HDStreamExtractorIndexer::HDStreamExtractorIndexer(const std::string &executablePath)
	{
		this->executable = executablePath;
	}

	Nullable<decimal> HDStreamExtractorIndexer::getPercentage(const std::string &line)
	{
		try
		{
	...
	{
		for (std::vector<std::string>::const_iterator strSource = job->getSource().begin(); strSource != job->getSource().end(); ++strSource)
			Util::ensureExists(*strSource);
	}

	void HDStreamExtractorIndexer::ProcessLine(const std::string &line, StreamType stream, ImageType oType)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
	...
	const std::string &HDStreamExtractorIndexer::getCommandline() const
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
				sb->Append(std::string::Format("\"{0}\" {1}) {2}", job->Input.substr(0, job->Input.find("BDMV")), job->getFeatureNb(), job->getArgs().Trim() + " -progressnumbers"));
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
			else if (job->Input.ToLower(System::Globalization::CultureInfo::InvariantCulture)->EndsWith(".evo"))
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
				sb->Append(std::string::Format("\"{0}\" {1}) {2}", job->Input.substr(0, job->Input.find("HVDVD_TS")), job->getFeatureNb(), job->getArgs().Trim() + " -progressnumbers"));
			else
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
				sb->Append(std::string::Format("\"{0}\" {1}) {2}", job->Input, job->getFeatureNb(), job->getArgs().Trim() + " -progressnumbers"));
		}
		else
		{
			...
			std::string strSource = std::string::Format("\"{0}\"", job->getSource()[0]);
			for (int i = 1; i < job->getSource().size(); i++)
				strSource += std::string::Format("+\"{0}\"", job->getSource()[i]);
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
			sb->Append(std::string::Format("{0} {1}", strSource, job->getArgs().Trim() + " -progressnumbers"));
		}
