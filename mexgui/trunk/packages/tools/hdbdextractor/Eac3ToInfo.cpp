	...
	namespace packages
	{
		namespace tools
		{
			namespace hdbdextractor
			{
				...
				const std::string &eac3toArgs::geteac3toPath() const
				{
					return privateeac3toPath;
				}

				void eac3toArgs::seteac3toPath(const std::string &value)
				{
					privateeac3toPath = value;
				}

				const std::string &eac3toArgs::getworkingFolder() const
				{
					return privateworkingFolder;
				}

				void eac3toArgs::setworkingFolder(const std::string &value)
				{
					privateworkingFolder = value;
				}

				const std::string &eac3toArgs::getfeatureNumber() const
				{
					return privatefeatureNumber;
				}

				void eac3toArgs::setfeatureNumber(const std::string &value)
				{
					privatefeatureNumber = value;
				}

				const std::string &eac3toArgs::getargs() const
				{
					return privateargs;
				}

				void eac3toArgs::setargs(const std::string &value)
				{
					privateargs = value;
				}

				const MeGUI::packages::tools::hdbdextractor::ResultState &eac3toArgs::getresultState() const
				{
				...
				Eac3toInfo::Eac3toInfo(std::vector<std::string> &input, MediaInfoFile *iFile, LogItem *oLog)
				{
					// create log instance
					if (oLog == 0)
					{
					...
					for (std::vector<std::string>::const_iterator strPath = input.begin(); strPath != input.end(); ++strPath)
				...
				const std::vector<Feature*> &Eac3toInfo::getFeatures() const
				{
					return features;
				}
...
//					using (Process compiler = new Process())
					...
					try
					{
						std::string strSource = std::string::Format("\"{0}\"", input[0]);
						for (int i = 1; i < input.size(); i++)
							strSource += std::string::Format("+\"{0}\"", input[i]);

						compiler->StartInfo->FileName = args.geteac3toPath();
						switch (args.getresultState())
						{
							case FeatureCompleted:
								compiler->StartInfo->Arguments = std::string::Format("{0}", strSource);
								break;
							case StreamCompleted:
								if (args.getargs() == "")
									compiler->StartInfo->Arguments = std::string::Format("{0}", strSource);
								else
									compiler->StartInfo->Arguments = std::string::Format("{0} {1}) {2}", strSource, args.getargs(), "-progressnumbers");
								break;
							case ExtractCompleted:
								if (oMode == FileBased)
									compiler->StartInfo->Arguments = std::string::Format("{0} {1}", strSource, args.getargs() + " -progressnumbers");
								else
									compiler->StartInfo->Arguments = std::string::Format("{0} {1}) {2}", strSource, args.getfeatureNumber(), args.getargs() + "-progressnumbers");
								break;
						}

						if (_log != 0)
							_log->LogEvent(std::string::Format("Arguments: {0}", compiler->StartInfo->Arguments));
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
				...
				void Eac3toInfo::backgroundWorker_Exited(object *sender, EventArgs *e)
				{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'TrimStart' method:
						...
						if (!data.empty())
						{
							...
							else if (Regex::IsMatch(data, "^\".+\"$", RegexOptions::Compiled))
							{
								...
								if (oMode == FileBased)
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
									features[0]->getStreams()[features[0]->getStreams()->size() - 1]->setName(Extensions::CapitalizeAll(data.Trim((std::string("\" ."))->ToCharArray())));
								else
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
									features[features.size() - 1]->setName(Extensions::CapitalizeAll(data.Trim((std::string("\" ."))->ToCharArray())));
								return;
							}
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
								for (unknown::const_iterator file = Regex::Match(data, "\\[.+\\]")->Value->Trim((std::string("[]"))->ToCharArray())->Split((std::string("+"))->ToCharArray()).begin(); file != Regex::Match(data, "\\[.+\\]")->Value->Trim((std::string("[]"))->ToCharArray())->Split((std::string("+"))->ToCharArray()).end(); ++file)
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
							...
							else if (Regex::IsMatch(data, "^analyze: [0-9]{1,3}%$", RegexOptions::Compiled))
							{
								if (backgroundWorker->IsBusy)
									backgroundWorker->ReportProgress(int::Parse(Regex::Match(data, "[0-9]{1,3}")->Value), std::string::Format("Analyzing ({0}%)", int::Parse(Regex::Match(data, "[0-9]{1,3}")->Value)));
							...
							{
								if (backgroundWorker->IsBusy)
									backgroundWorker->ReportProgress(int::Parse(Regex::Match(data, "[0-9]{1,3}")->Value), std::string::Format("Processing ({0}%)", int::Parse(Regex::Match(data, "[0-9]{1,3}")->Value)));
							...
							{
								if (backgroundWorker->IsBusy)
									backgroundWorker->ReportProgress(int::Parse(Regex::Match(data, "[0-9]{1,3}")->Value), std::string::Format("Progress ({0}%)", int::Parse(Regex::Match(data, "[0-9]{1,3}")->Value)));
							...
							{
								if (_log != 0)
									_log->Warn(std::string::Format("Unknown line: \"{0}\"", data));
							}
						}
					}
				}
