...
//C# TO C++ CONVERTER WARNING: C# to C++ Converter converted the original 'null' assignment to a call to 'delete', but you should review memory allocation of all pointer variables in the converted code:
	...
	void OneClickPostProcessing::safeDelete(const std::string &filePath)
	{
		try
		{
	...
	void OneClickPostProcessing::raiseEvent(const std::string &s)
	{
		su->setStatus(s);
		raiseEvent();
	}
...
//C# TO C++ CONVERTER TODO TASK: Lambda expressions and anonymous methods are not converted to native C++ unless the option to convert to C++0x lambdas is selected:
			...
			std::vector<std::string> arrAudioFilesDelete = std::vector<std::string>();
			audioFiles = std::map<int, std::string>();
			std::vector<AudioTrackInfo*> arrAudioTracks = std::vector<AudioTrackInfo*>();
			std::vector<AudioJob*> arrAudioJobs = std::vector<AudioJob*>();
			std::vector<MuxStream*> arrMuxStreams = std::vector<MuxStream*>();
			std::vector<std::string> intermediateFiles = std::vector<std::string>();

			FileUtil::ensureDirectoryExists(job->getPostprocessingProperties()->getWorkingDirectory());

			// audio handling
			for (std::vector<OneClickAudioTrack*>::const_iterator oAudioTrack = job->getPostprocessingProperties()->getAudioTracks().begin(); oAudioTrack != job->getPostprocessingProperties()->getAudioTracks().end(); ++oAudioTrack)
			{
			...
			if (audioFiles.empty() && job->getPostprocessingProperties()->getIndexType() != FileIndexerWindow::NONE && !job->getPostprocessingProperties()->getEac3toDemux())
				audioFiles = vUtil->getAllDemuxedAudio(arrAudioTracks, std::vector<AudioTrackInfo*>(), arrAudioFilesDelete, job->getIndexFile(), _log);
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
				if (job->getPostprocessingProperties()->getChapterFile().StartsWith("<") || job->getPostprocessingProperties()->getChapterExtracted())
				{
					...
					std::string strTempFile = job->getPostprocessingProperties()->getChapterFile();
			...
			for (std::vector<std::string>::const_iterator file = arrAudioFilesDelete.begin(); file != arrAudioFilesDelete.end(); ++file)
			...
			for (std::vector<std::string>::const_iterator file = job->getPostprocessingProperties()->getFilesToDelete().begin(); file != job->getPostprocessingProperties()->getFilesToDelete().end(); ++file)
				intermediateFiles.push_back(*file);

			// subtitle handling
			if (!avsFile.empty() || !job->getPostprocessingProperties()->getVideoFileToMux().empty())
			{
				std::vector<MuxStream*> subtitles = std::vector<MuxStream*>();
				if (job->getPostprocessingProperties()->getSubtitleTracks().size() > 0)
				{
					for (std::vector<OneClickStream*>::const_iterator oTrack = job->getPostprocessingProperties()->getSubtitleTracks().begin(); oTrack != job->getPostprocessingProperties()->getSubtitleTracks().end(); ++oTrack)
					{
						if ((*oTrack)->getTrackInfo()->ExtractMKVTrack)
						{
							//demuxed MKV
							std::string trackFile = Path::GetDirectoryName(job->getIndexFile()) + "\\" + (*oTrack)->getTrackInfo()->DemuxFileName;
	...
	void OneClickPostProcessing::fillInAudioInformation(std::vector<AudioJob*> &arrAudioJobs, std::vector<MuxStream*> &arrMuxStreams)
	{
		for (std::vector<MuxStream*>::const_iterator m = arrMuxStreams.begin(); m != arrMuxStreams.end(); ++m)
			(*m)->path = convertTrackNumberToFile((*m)->path, (*m)->delay);

		std::vector<AudioJob*> tempList = std::vector<AudioJob*>();
		for (std::vector<AudioJob*>::const_iterator a = arrAudioJobs.begin(); a != arrAudioJobs.end(); ++a)
		{
	...
	std::string OneClickPostProcessing::convertTrackNumberToFile(const std::string &input, int &delay)
	{
		if (input.empty())
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
		...
		{
			std::string sub = input.substr(2, input.length() - 4);
			try
			{
				int t = int::Parse(sub);
				std::string s = audioFiles[t];
			...
			{
				_log->Warn(std::string::Format("Couldn't find audio file for track {0}. Skipping track.", input));
				return 0;
			}
		}
	...
	std::string OneClickPostProcessing::createAVSFile(const std::string &indexFile, const std::string &inputFile, Nullable<Dar> AR, int desiredOutputWidth, bool signalAR, LogItem *_log, AviSynthSettings *avsSettings, bool autoDeint, VideoCodecSettings *settings, Nullable<Dar> &dar, bool autoCrop, bool keepInputResolution, bool useChaptersMarks)
	{
		...
		{
			std::string tempAvs = "AVISource(\"" + inputFile + "\", audio=false)" + VideoUtil::getAssumeFPS(0, inputFile);
		...
		if (!AR.HasValue)
		{
			...
			if (customDAR.getAR() <= 0)
			{
				...
				_log->Warn(std::string::Format("No aspect ratio found, defaulting to {0}.", customDAR));
			}
		}
		else
			customDAR = AR.Value;
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
		...
		std::string newScript = ScriptServer::CreateScriptFromTemplate(avsSettings->getTemplate(), inputLine, cropLine, resizeLine, denoiseLines, deinterlaceLines);

		if (dar.HasValue)
			newScript = std::string::Format("global MeGUI_darx = {0}\r\nglobal MeGUI_dary = {1}\r\n{2}", dar.Value::X, dar.Value::Y, newScript);
		else
		{
			if (xTargetDevice != 0 && xTargetDevice->getBluRay())
			{
				std::string strResolution = outputWidthIncludingPadding + "x" + outputHeightIncludingPadding;
	...
	void OneClickPostProcessing::finishedAnalysis(SourceInfo *info, bool error, const std::string &errorMessage)
	{
		if (error || info == 0)
		{
