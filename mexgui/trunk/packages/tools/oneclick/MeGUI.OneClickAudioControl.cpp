...
namespace MeGUI
{
	...
	OneClickAudioControl::OneClickAudioControl()
	{
		...
		InitializeComponent();
		language->Items->AddRange((std::vector<std::string>(LanguageSelectionContainer::getLanguages().Keys))->ToArray());
	...
	void OneClickAudioControl::SetProfileNameOrWarn(const std::string &fqname)
	{
	...
	const std::string &OneClickAudioControl::getLanguage() const
	{
		return language->Text;
	}

	void OneClickAudioControl::setLanguage(const std::string &value)
	{
	...
	void OneClickAudioControl::cbEncodingMode_SelectedIndexChanged(object *sender, EventArgs *e)
	{
		encoderProfile->Enabled = !cbEncodingMode->SelectedText->Equals("never");
	}

	void OneClickAudioControl::language_SelectedIndexChanged(object *sender, EventArgs *e)
	{
