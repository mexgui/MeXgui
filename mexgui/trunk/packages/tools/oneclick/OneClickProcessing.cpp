	...
	namespace packages
	{
		namespace tools
		{
			namespace oneclick
			{
				...
				OneClickProcessing::OneClickProcessing(OneClickWindow *oWindow, const std::string &strFileOrFolderName, OneClickSettings *oSettings, LogItem *oLog)
				{
				...
				OneClickProcessing::OneClickProcessing(OneClickWindow *oWindow, std::vector<OneClickFilesToProcess*> &arrFilesToProcess, OneClickSettings *oSettings, LogItem *oLog)
				{
					this->oOneClickWindow = oWindow;
					this->_log = oLog;

					std::vector<OneClickFilesToProcess*> arrFilesToProcessNew = std::vector<OneClickFilesToProcess*>();
					MediaInfoFile *iFile = 0;

					for (std::vector<OneClickFilesToProcess*>::const_iterator oFileToProcess = arrFilesToProcess.begin(); oFileToProcess != arrFilesToProcess.end(); ++oFileToProcess)
					{
						if (iFile == 0)
						{
					...
					if (iFile != 0)
						...
						oOneClickWindow->setInputData(0, std::vector<OneClickFilesToProcess*>()); // not demuxable
				}
...
//					using (frmStreamSelect frm = new frmStreamSelect(ex, SelectionMode.MultiExtended))
					...
					try
					{
						...
						if (frm->getChapterCount() == 1 || (frm->getChapterCount() > 1 && frm->ShowDialog() == DialogResult::OK))
						{
							std::vector<ChapterInfo*> oChapterList = frm->getSelectedMultipleChapterInfo();
							if (oChapterList.size() > 0)
							{
								...
								for (std::vector<ChapterInfo*>::const_iterator oChapterInfo = oChapterList.begin(); oChapterInfo != oChapterList.end(); ++oChapterInfo)
								{
									std::string strVOBFile = Path::Combine(path, (*oChapterInfo)->getTitle() + "_1.VOB");
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
				...
				{
					std::string path = this->strInput;
					while (!Path::GetFullPath(path)->Equals(Path::GetPathRoot(path)))
					{
						std::string bdmvPath = Path::Combine(path, "BDMV\\PLAYLIST");
					...
					try
					{
						...
						if (frm->getChapterCount() == 1 || (frm->getChapterCount() > 1 && frm->ShowDialog() == DialogResult::OK))
						{
							...
							if (oChapterList.size() > 0)
							{
								...
								for (std::vector<ChapterInfo*>::const_iterator oChapterInfo = oChapterList.begin(); oChapterInfo != oChapterList.end(); ++oChapterInfo)
								{
									std::string strFile = path + "\\BDMV\\PLAYLIST\\" + (*oChapterInfo)->getSourceName();
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
				...
				{
					oOneClickWindow->setInputData(iFile, std::vector<OneClickFilesToProcess*>());
					return true;
				}
			}
		}
