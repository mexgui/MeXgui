...
namespace MeGUI
{
	...
	AutoEncodeWindow::AutoEncodeWindow(VideoStream *videoStream, std::vector<AudioJob*> &audioStreams, MainForm *mainForm, bool prerender, VideoInfo *vInfo)
	{
		InitializeInstanceFields();
		this->vInfo = vInfo;
	...
	{
		std::vector<AudioEncoderType*> aTypes = std::vector<AudioEncoderType*>();
//ORIGINAL LINE: AudioEncoderType[] audioTypes;
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
		AudioEncoderType *audioTypes;
		for (std::vector<AudioJob*>::const_iterator stream = this->audioStreams.begin(); stream != this->audioStreams.end(); ++stream)
		{
		...
		std::vector<ContainerType*> supportedOutputTypes = this->muxProvider->GetSupportedContainers(this->videoStream->getSettings()->getEncoderType(), audioTypes);
		...
		std::vector<DeviceType*> supportedOutputDeviceTypes = this->muxProvider->GetSupportedDevices(static_cast<ContainerType*>(container->SelectedItem));
		...
		std::string muxedName = FileUtil::AddToFileName(vInfo->getVideoOutput(), "-muxed");
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void AutoEncodeWindow::InitializeComponent()
	{
		...
		this->targetSize->SelectionChanged += new MeGUI::StringChanged(this, &AutoEncodeWindow::targetSize_SelectionChanged);
		// 
		// noTargetRadio
		// 
	...
	void AutoEncodeWindow::container_SelectedIndexChanged(object *sender, EventArgs *e)
	{
		...
		if (!muxedOutput->getFilename().empty())
		{
			...
			std::vector<DeviceType*> supportedOutputDeviceTypes = this->muxProvider->GetSupportedDevices(cot);
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	void AutoEncodeWindow::outputFormat_CheckedChanged(object *sender, System::EventArgs *e)
	{
		ContainerType *cot = dynamic_cast<ContainerType*>(this->container->SelectedItem);
	...
	void AutoEncodeWindow::separateEncodableAndMuxableAudioStreams(AudioJob encodable[], MuxStream muxable[], AudioEncoderType muxTypes[])
	{
		...
		int j = 0;
		for (MeGUI::AudioJob::const_iterator stream = encodable->begin(); stream != encodable->end(); ++stream)
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	MeGUI::packages::tools::calculator::CalcData *AutoEncodeWindow::GetCalcData()
	{
		CalcData *data = new CalcData(static_cast<long long>(videoStream->getNumberOfFrames()), videoStream->getFramerate(), static_cast<ContainerType*>(container->SelectedItem), videoStream->getSettings()->getCodec(), videoStream->getSettings()->getNbBframes() > 0, getAudioStreamsForBitrate());
		return data;
	}

	AudioBitrateCalculationStream *AutoEncodeWindow::getAudioStreamsForBitrate()
	{
		std::vector<AudioBitrateCalculationStream*> streams = std::vector<AudioBitrateCalculationStream*>();
		for (std::vector<AudioJob*>::const_iterator s = audioStreams.begin(); s != audioStreams.end(); ++s)
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
	...
	{
		long long sizes[this->audioStreams.size()];
		int index = 0;
		for (std::vector<AudioJob*>::const_iterator stream = this->audioStreams.begin(); stream != this->audioStreams.end(); ++stream)
		{
			if (!(*stream)->Output->empty()) // if we don't have the video length or the audio is not fully configured we can give up now
			{
				...
				long long sizeInBytes = static_cast<long long>(lengthInSeconds * bytesPerSecond);
	...
	void AutoEncodeWindow::queueButton_Click(object *sender, System::EventArgs *e)
	{
		if (!this->muxedOutput->getFilename().empty())
		{
			...
			if (FileSizeRadio->Checked)
				...
				log->LogValue("Projected Bitrate ", std::string::Format("{0}kbps", projectedBitrateKBits->Text));
...
//C# TO C++ CONVERTER WARNING: Since the array size is not known in this declaration, C# to C++ Converter has converted this array to a pointer.  You will need to call 'delete[]' where appropriate:
			...
			std::string chapters = "";
			std::string videoInput = vInfo->getVideoInput();
			std::string videoOutput = vInfo->getVideoOutput();
			std::string muxedOutput = this->muxedOutput->getFilename();
			ContainerType *cot = dynamic_cast<ContainerType*>(this->container->SelectedItem);

			// determine audio language 
			for (MeGUI::core::details::MuxStream::const_iterator stream = audio->begin(); stream != audio->end(); ++stream)
			{
			...
			if (addSubsNChapters->Checked)
			{
				...
				if (amw->ShowDialog() == System::Windows::Forms::DialogResult::OK)
	...
	{
		std::vector<MuxStream*> newAudio = std::vector<MuxStream*>();
		for (MeGUI::core::details::MuxStream::const_iterator stream = audio->begin(); stream != audio->end(); ++stream)
		{
			bool matchFound = false;
			for (MeGUI::AudioJob::const_iterator a = aStreams->begin(); a != aStreams->end(); ++a)
			{
	...
	void AutoEncodeWindow::containerOverhead_ValueChanged(object *sender, System::EventArgs *e)
	{
		if (isBitrateMode)
	...
	void AutoEncodeWindow::projectedBitrate_TextChanged(object *sender, System::EventArgs *e)
	{
		if (!this->isBitrateMode)
			this->setTargetSize();
	}

	void AutoEncodeWindow::calculationMode_CheckedChanged(object *sender, System::EventArgs *e)
	{
	...
	void AutoEncodeWindow::targetSize_SelectionChanged(object *sender, const std::string &val)
	{
		if (isBitrateMode)
			this->setVideoBitrate();
	}
	...
	const std::string &AutoEncodeTool::getName() const
	{
		return "AutoEncode";
	}
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to the exception 'finally' clause:
	...
	const std::string &AutoEncodeTool::getID() const
	{
		return "AutoEncode";
	}
}
