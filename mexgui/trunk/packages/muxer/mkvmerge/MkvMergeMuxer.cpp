...
JobProcessorFactory *const MkvMergeMuxer::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "MkvMergeMuxer");
	...
	MkvMergeMuxer::MkvMergeMuxer(const std::string &executablePath)
	{
		this->executable = executablePath;
	}

	Nullable<decimal> MkvMergeMuxer::getPercentage(const std::string &line)
	{
		try
		{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
	...
	void MkvMergeMuxer::ProcessLine(const std::string &line, StreamType stream, ImageType oType)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'StartsWith' method:
	...
	const std::string &MkvMergeMuxer::getCommandline() const
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		...
		{
			std::string inputFile = settings->getVideoInput();
			if (settings->getVideoInput().empty())
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'EndsWith' method:
			else if (stream->path.ToLower(System::Globalization::CultureInfo::InvariantCulture)->EndsWith(".idx"))
			{
				...
				for (std::vector<SubtitleInfo*>::const_iterator strack = subTracks.begin(); strack != subTracks.end(); ++strack)
				{
					for (std::map<std::string, std::string>::const_iterator strLanguage = LanguageSelectionContainer::getLanguages().begin(); strLanguage != LanguageSelectionContainer::getLanguages().end(); ++strLanguage)
					{
