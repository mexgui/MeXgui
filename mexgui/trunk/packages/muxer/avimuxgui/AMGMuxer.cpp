...
JobProcessorFactory *const AMGMuxer::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "AMGMuxer");
	...
	AMGMuxer::AMGMuxer(const std::string &executablePath)
	{
	...
	const std::string &AMGMuxer::getCommandline() const
	{
		return "\"" + script_filename + "\"";
	}

	std::string AMGMuxer::writeScript(MuxJob *job)
	{
		...
		if (settings->getSplitSize().HasValue || settings->getDeviceType().empty() || settings->getDeviceType() == "Standard")
		{
			...
			if ((settings->getDeviceType().empty() || settings->getDeviceType() == "Standard") && (!settings->getSplitSize().HasValue || (static_cast<MeGUI::core::util::FileSize>(settings->getSplitSize())).getMB() > 2000))
				script->AppendFormat("SET OPTION MAXFILESIZE {0}{1}", 2000, "\r\n");
			else
				script->AppendFormat("SET OPTION MAXFILESIZE {0}{1}", (static_cast<MeGUI::core::util::FileSize>(settings->getSplitSize())).getMB(), "\r\n");
		}
		else
		{
		...
		std::string filename = Path::ChangeExtension(job->Output, ".mux");
