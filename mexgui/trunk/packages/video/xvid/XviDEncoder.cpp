...
JobProcessorFactory *const XviDEncoder::Factory = new JobProcessorFactory(new ProcessorFactory(&init), "XviDEncoder");
	...
	XviDEncoder::XviDEncoder(const std::string &exePath) : CommandlineVideoEncoder()
	{
		executable = exePath;
	}

	void XviDEncoder::ProcessLine(const std::string &line, StreamType stream, ImageType oType)
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the .NET String 'Trim' method:
	...
	const std::string &XviDEncoder::getCommandline() const
	{
		return genCommandline(job->Input, job->Output, job->getDAR(), dynamic_cast<xvidSettings*>(job->getSettings()), hres, vres, job->getZones());
	}

	std::string XviDEncoder::genCommandline(const std::string &input, const std::string &output, Nullable<Dar> d, xvidSettings *xs, int hres, int vres, Zone zones[])
	{
...
//C# TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'ToString':
		...
		{
			std::string extension = Path::GetExtension(output)->ToLower(System::Globalization::CultureInfo::InvariantCulture);
